...
#ifndef ACLRUNTIME_INCLUDE_MODELINFER_MODELINFERENCEPROCESSOR_H_
#define ACLRUNTIME_INCLUDE_MODELINFER_MODELINFERENCEPROCESSOR_H_
...
struct DynamicInfo {
    ...
    std::unique_ptr<DyDimsInfo> dyDims;
    std::unique_ptr<DyShapeInfo> dyShape;
};

struct ModelDesc {
    std::vector<Base::TensorDesc>
        inTensorsDesc;  // 所有 intensors信息 不包括dynamic index
...
struct InferSummaryInfo {
    std::vector<std::pair<float, float>> execTimeList;
    std::chrono::steady_clock::time_point zero_point =
        std::chrono::steady_clock::now();
};

constexpr size_t DYNAMIC_INDEX_NONE = SIZE_MAX;
...
public:
     ...
     * @return APP_ERROR error code
     */
    APP_ERROR Inference(const std::vector<TensorBase>& feeds,
                        std::vector<std::string> outputNames,
                        std::vector<TensorBase>& outputTensors);

    APP_ERROR Inference(const std::map<std::string, TensorBase>& feeds,
                        std::vector<std::string> outputNames,
                        std::vector<TensorBase>& outputTensors);

    APP_ERROR Inference(const std::vector<BaseTensor>& feeds,
                        std::vector<std::string>& outputNames,
                        std::vector<TensorBase>& outputTensors);

    APP_ERROR FirstInference(const std::vector<BaseTensor>& feeds,
                             std::vector<std::string>& outputNames,
                             std::vector<TensorBase>& outputTensors);
    ...
    APP_ERROR FirstInferenceInner(std::vector<BaseTensor>& inputs,
                                  std::vector<std::string> outputNames,
                                  std::vector<TensorBase>& outputTensors);
    APP_ERROR ModelInference_Inner(std::vector<BaseTensor>& inputs,
                                   std::vector<std::string> outputNames,
                                   std::vector<TensorBase>& outputTensors);
    ...
    InferSummaryInfo& GetMutableSumaryInfo();
...
private:
    ...
    APP_ERROR AddOutTensors(std::vector<MemoryData>& outputs,
    ...
    APP_ERROR CheckInVectorAndFillBaseTensor(
    ...
    APP_ERROR CheckInMapAndFillBaseTensor(
...
#endif  // ACLRUNTIME_INCLUDE_MODELINFER_MODELINFERENCEPROCESSOR_H_
