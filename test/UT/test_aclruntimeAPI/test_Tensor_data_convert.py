...
class TestClass:
    ...
    def get_model_path(cls, kind: str):
        return os.path.join(
            TestCommonClass.base_path,
            cls.model_name,
            "model",
            f"{cls.model_name}_{kind}.om",
        )
    ...
    def init(self):
        ...
        self.session = aclruntime.InferenceSession(
            self.get_model_path(self.model_kind), self.device_id, self.options
        )
