...
SampleProcess::SampleProcess(const char *data, const char *outFolder) : deviceId_(0), context_(nullptr), stream_(nullptr), thread_(),filePath(string(data)+"/vdec_h265_1frame_rabbit_1280x720.h265"),
    outFolder_(outFolder), picDesc_({}), enType_(H265_MAIN_LEVEL), format_(PIXEL_FORMAT_YUV_SEMIPLANAR_420)
{
}
...
Result SampleProcess::DoModelProcess()
{
    ...
    int modelInputWidth = 1280;
    int modelInputHeight = 720;

    INFO_LOG("modelInputWidth %d modelInputHeight %d",modelInputWidth,modelInputHeight);
    ...
    for (size_t frameId = 0; frameId < fileList.size(); frameId++) {
        ...
        std::string fileNameSave = std::string(outFolder_) + "/" + fileList[frameId];
        INFO_LOG("read file %s \n", fileNameSave.c_str());
        ...
        // dvpp process
        ret = dvppProcess.Process(dvppOutputBuffer, dvppOutputSize);
        if (ret != SUCCESS) {
            ERROR_LOG("init dvpp resource failed");
            return FAILED;
        }
