...
class DummyStreamClient(BaseStreamClient):
    ...
    def construct_request_body(self, inputs, parameters=None):
        return {'dummy': inputs}
...
def test_preprocess_cur_line_timeout(patch_logger):
    client = DummyStreamClient()
    with pytest.raises(AisBenchClientException):
        client.preprocess_cur_line('engine callback timeout.')
...
def test_request_success(monkeypatch):
    ...
    inputs = type('M', (), {'input_data': 'data', 'start_time': None, 'end_time': None, 'req_latency': None})()
    res = client.request(inputs, parameters=None)
    assert res == json.dumps(payload)


def test_request_error(monkeypatch, patch_logger):
    # simulate do_request raising TimeoutError
    client = DummyClient()
    monkeypatch.setattr(DummyClient, 'do_request', lambda self, body, method: (_ for _ in ()).throw(__import__('urllib3').exceptions.TimeoutError()))
    inputs = type('M', (), {'input_data': 'data'})()
    with pytest.raises(AisBenchClientException) as excinfo:
