...
class TestClass:
    ...
    def get_resnet50_model_path(cls, kind: str):
        return os.path.join(
            TestCommonClass.base_path, "resnet50", "model", f"pth_resnet50_{kind}.om"
        )

    @classmethod
    def get_add_model_path(cls, kind: str):
        return os.path.join(
            TestCommonClass.base_path, "add_model", "model", f"add_model_{kind}.om"
        )
    ...
    def test_resnet50_get_inputs(self):
        ...
        session = aclruntime.InferenceSession(
            self.get_resnet50_model_path("bs1"), self.device_id, options
        )
        inTensorDescList = session.get_inputs()

        assert isinstance(inTensorDescList, list)
    ...
    def test_resnet50_get_outputs(self):
        ...
        assert isinstance(outTensorDescList, list)
    ...
    def test_add_model_get_inputs(self):
        options = aclruntime.session_options()
        session = aclruntime.InferenceSession(
            self.get_add_model_path("bs1"), self.device_id, options
        )
        inTensorDescList = session.get_inputs()
    ...
    def test_add_model_get_outputs(self):
        ...
        assert len(outTensorDescList) == 1
        for i, outTensorDesc in enumerate(outTensorDescList):
            assert outTensorDescList[i].name[-6:] == "output"
            assert outTensorDesc.datatype in self.TensorDataType
