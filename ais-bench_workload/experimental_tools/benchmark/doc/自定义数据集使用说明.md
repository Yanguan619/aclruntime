...
|参数|说明|样例|
| ----- | ----- | ---- |
|--models|和普通数据集使用方式一致，指定模型推理后端任务名称（对应ais_bench/benchmark/configs/models路径下一个已经实现的默认模型配置文件），支持传入多个任务名称，支持的任务范围请参考父级目录的README中使用普通数据集为例的方式，"命令行指定模型和数据集" ([父级目录README.md](../README.md)中的"命令行指定模型和数据集"章节) 方式的必选配置，与"配置文件指定模型和数据集" ([父级目录README.md](../README.md)中的"配置文件指定模型和数据集"章节) 方式的"config"参数二选一|--models vllm_api_general|
