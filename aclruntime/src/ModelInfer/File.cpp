...
bool File::IsPathCharactersValid(const std::string& path) {
    ...
    for (size_t i = 0; i < path.size(); ++i) {
        auto it =
            std::find(extendedPattern.begin(), extendedPattern.end(), path[i]);
...
bool File::IsPathDepthValid(const std::string& path) {
    return static_cast<size_t>(std::count(path.begin(), path.end(),
                                          PATH_SEPARATOR)) <= PATH_DEPTH_MAX;
}

bool File::CheckFileRWX(const std::string& path,
...
bool File::CheckFileSuffixAndSize(const std::string& path, FileType type) {
    static const std::map<FileType, std::pair<std::string, size_t>>
        FileTypeCheckTbl = {
...
bool File::OpenFile(const std::string& path, std::ifstream& ifs,
...
bool File::OpenFile(const std::string& path, std::ofstream& ofs,
                    std::ios::openmode mode) {
    std::string absPath = GetAbsPath(path);
    if (absPath.empty()) {
        ERROR_LOG("path is empty");
    ...
    int fd = open(absPath.c_str(), CREATE_FILE_MODE_DEFAULT,
                  NORMAL_FILE_MODE_DEFAULT);
    if (fd == -1) {
    ...
    if (!tmpofs.is_open()) {
        ERROR_LOG("ofstream open file failed");
        return false;
    }
    ofs = std::move(tmpofs);
    return true;
...
bool File::CheckFileBeforeRead(const std::string& path, FileType type) {
    ...
    if (!IsFileReadable(absPath) ||
        (GetFilePermissions(absPath) & S_IRUSR) == 0) {
...
bool File::CheckFileBeforeCreateOrWrite(const std::string& path,
                                        bool overwrite) {
    std::string absPath = GetAbsPath(path);
    if (absPath.empty()) {
        ERROR_LOG("path is empty");
