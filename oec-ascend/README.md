...
```
#### 网络问题
如果wget遇到证书相关的错误，可以手动下载安装包上传安装，或者增加`--no-check-certifiate`参数规避（**不建议**）
...
### 使用
#### 必要步骤
1. 准备一个安装有昇腾NPU硬件的环境并安装昇腾驱动软件
2. 安装Ascend-cann-Toolkit，Ascend-cann-kernels，Ascend-cann-nnal软件

昇腾驱动固件和CANN软件安装包，可以访问 [昇腾社区资源中心](https://www.hiascend.com/developer/download/community) 获取。软件安装指南请参考 [昇腾社区文档](https://www.hiascend.com/document)。
