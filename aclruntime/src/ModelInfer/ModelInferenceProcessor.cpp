...
APP_ERROR ModelInferenceProcessor::Execute()
{
    ...
    float start_time = std::chrono::duration<float, std::milli>(start - zp).count();
...
APP_ERROR ModelInferenceProcessor::SetDynamicDims(std::string dymdimsStr)
{
    ...
    Result ret =  processModel->CheckDynamicDims(dynamicInfo_.dyDims->dym_dims, dym_gear_count_, dims);
    if (ret != SUCCESS) {
    ...
    for (const auto& kv : namedimsmap) {
        ...
        modelDesc_.inTensorsDesc[inindex].realsize = static_cast<size_t>(kv.second) * aclDataTypeSize(tmpDataType);
    }
...
APP_ERROR ModelInferenceProcessor::SetDynamicShape(std::string dymshapeStr)
{
    ...
    for (const auto& kv : namedimsmap) {
        ...
        size_t inindex = it->second;
        auto tmpDataType = static_cast<aclDataType>(modelDesc_.inTensorsDesc[inindex].datatype);
        modelDesc_.inTensorsDesc[inindex].realsize = static_cast<size_t>(kv.second) *
    ...
    switch (dynamicInfo_.dynamicType) {
        ...
        case DYNAMIC_DIMS:
            if (dynamicInfo_.dyDims == nullptr) {
                ...
                CHECK_RET_EQ(processModel->SetDynamicDims(dynamicInfo_.dyDims->dym_dims), SUCCESS);
            }
            break;
        case DYNAMIC_SHAPE:
            if (dynamicInfo_.dyShape == nullptr) {
                ...
                CHECK_RET_EQ(processModel->SetDynamicShape(
                    dynamicInfo_.dyShape->dym_shape_map, dynamicInfo_.dyShape->dims_num), SUCCESS);
            }
            break;
        default:
            break;
