...
PyInferenceSession::PyInferenceSession(const std::string& modelPath,
  ...
  if (options->loop <= 0 || options->loop > LOOP_MAX_SIZE) {
    ERROR_LOG(
        "loop size out of range: loop must be greater than 0 and less than or "
        "equal to 100000.");
  ...
  if (options->log_level < LOG_DEBUG_LEVEL ||
    ...
    ERROR_LOG(
        "log level out of range: log level must be greate than 0 and less than "
        "5");
...
int PyInferenceSession::SetDynamicBatchsize(int batchsize) {
  ...
  if (batchsize <= 0 || batchsize > BATCHSIZE_MAX) {
    ERROR_LOG(
        "dynamic batchsize must be greater than 0 and less than or equal to "
        "%d.",
        BATCHSIZE_MAX);
...
int PyInferenceSession::SetDynamicHW(int width, int height) {
  ...
  if (width <= 0 || width > HW_MAX) {
    ERROR_LOG(
        "width of dymHW must be greater than 0 and less than or equal to %d.",
        HW_MAX);
    throw std::runtime_error("width of dymHW out of range. Please check.");
  }
  if (height <= 0 || height > HW_MAX) {
    ERROR_LOG(
        "height of dymHW must be greater than 0 and less than or equal to %d.",
        HW_MAX);
...
int PyInferenceSession::SetCustomOutTensorsSize(
  ...
  for (size_t outSize : customOutSize) {
    if (outSize <= 0 || outSize > CUSTOME_SIZE_MAX_SIZE) {
      ERROR_LOG(
          "custom size out of range: custom size must be greater than 0 and "
...
void PyInferenceSession::InferPipeline(
  ...
  for (size_t i = 0; i < numThreads; i++) {
    ...
    prepareThreadGroup.emplace_back(FuncPrepare, std::ref(h2dQueues[i]),
                                    session, std::ref(infilesList), inferOption,
                                    numThreads, i);
