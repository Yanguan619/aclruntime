...
constexpr size_t DEVICE_ID_MAX = 255;
constexpr size_t CUSTOME_SIZE_COUNT_MAX = 256;
constexpr int BATCHSIZE_MAX = 4096;
constexpr int HW_MAX = 65536;
...
PyInferenceSession::PyInferenceSession(const std::string& modelPath,
    ...
    if (options->log_level < LOG_DEBUG_LEVEL ||
        options->log_level > LOG_ERROR_LEVEL) {
        ERROR_LOG(
            "log level out of range: log level must be greate than 0 and less "
            "than "
            "5");
    ...
    if ((options->aclJsonPath != "") &&
        (!File::CheckFileBeforeRead(options->aclJsonPath, FileType::JSON))) {
...
void PyInferenceSession::SetContext() {
    APP_ERROR ret =
        TensorContext::GetInstance()->SetContext(deviceId_, contextIndex_);
    if (ret != APP_ERR_OK) {
...
APP_ERROR PyInferenceSession::EnsureInputPool(
    const std::vector<Base::BaseTensor>& feeds) {
    if (inputMemPool_.size() < feeds.size()) {
        inputMemPool_.resize(feeds.size());
    }
    for (size_t i = 0; i < feeds.size(); ++i) {
        if (inputMemPool_[i].ptrData != nullptr &&
            inputMemPool_[i].size >= feeds[i].size) {
        ...
        MemoryData mem(feeds[i].size, MemoryData::MemoryType::MEMORY_DEVICE,
                       deviceId_);
        auto ret = MemoryHelper::MxbsMalloc(mem);
        if (ret != APP_ERR_OK) {
            ERROR_LOG("input pool malloc failed i:%zu size:%zu ret:%d", i,
                      feeds[i].size, ret);
            return ret;
        }
...
int PyInferenceSession::FreeResource() {
    ...
    ret =
        TensorContext::GetInstance()->DestroyContext(deviceId_, contextIndex_);
    if (ret != APP_ERR_OK) {
...
PyInferenceSession::~PyInferenceSession() { Destroy(); }

void PyInferenceSession::Init(const std::string& modelPath,
                              std::shared_ptr<SessionOptions> options) {
    ...
    // create context
    DEBUG_LOG("[MEM_CHECK] Before CreateContext: RSS=%zuMB",
              GetSystemMemoryUsedMB());
    APP_ERROR ret =
        TensorContext::GetInstance()->CreateContext(deviceId_, contextIndex_);
    if (ret != APP_ERR_OK) {
        throw std::runtime_error(GetError(ret));
    }
    SetContext();
...
std::vector<TensorBase> PyInferenceSession::InferMap(
...
std::vector<TensorBase> PyInferenceSession::InferVector(
...
std::string GetTensorDesc(Base::TensorDesc desc) {
    return (GetShapeDesc(desc.shape) + "  " +
            Base::GetTensorDataTypeDesc(desc.datatype) + "  " +
            std::to_string(desc.size) + "  " + std::to_string(desc.realsize));
}

uint32_t PyInferenceSession::GetDeviceId() const { return deviceId_; }
...
std::string PyInferenceSession::GetModelPath() { return modelPath_; }
...
const InferSummaryInfo& PyInferenceSession::GetSumaryInfo() const {
    return modelInfer_.GetSumaryInfo();
}

void PyInferenceSession::MergeSummaryInfo(const InferSummaryInfo& summaryInfo) {
    InferSummaryInfo& lhsSummaryInfo = modelInfer_.GetMutableSumaryInfo();
    const auto& src = summaryInfo.execTimeList;
    auto& dst = lhsSummaryInfo.execTimeList;
...
int PyInferenceSession::SetDynamicBatchsize(int batchsize) {
    ...
    if (batchsize <= 0 || batchsize > BATCHSIZE_MAX) {
        ERROR_LOG(
            "dynamic batchsize must be greater than 0 and less than or equal "
            "to "
            "%d.",
            BATCHSIZE_MAX);
        throw std::runtime_error(
            "dynamic batchsize out of range. Please check.");
    }
...
int PyInferenceSession::SetDynamicHW(int width, int height) {
    ...
    if (width <= 0 || width > HW_MAX) {
        ERROR_LOG(
            "width of dymHW must be greater than 0 and less than or equal to "
            "%d.",
            HW_MAX);
    ...
    if (height <= 0 || height > HW_MAX) {
        ERROR_LOG(
            "height of dymHW must be greater than 0 and less than or equal to "
            "%d.",
            HW_MAX);
...
int PyInferenceSession::SetDynamicDims(std::string dymdimsStr) {
    ...
    if (!Utils::IsLegalDymString(dymdimsStr)) {
        throw std::runtime_error(
            "the format of dynamic dims string is illegal, please check");
    }
...
int PyInferenceSession::SetDynamicShape(std::string dymshapeStr) {
    ...
    if (!Utils::IsLegalDymString(dymshapeStr)) {
        throw std::runtime_error(
            "the format of dynamic shape string is illegal, please check");
    }
...
int PyInferenceSession::SetCustomOutTensorsSize(
    std::vector<size_t> customOutSize) {
    if (customOutSize.size() > CUSTOME_SIZE_COUNT_MAX) {
        ERROR_LOG("custom size count is over max permitted count %zu",
                  CUSTOME_SIZE_COUNT_MAX);
        throw std::runtime_error(
            "length of custom size list out of range. Please check.");
    }
    for (size_t outSize : customOutSize) {
        if (outSize <= 0 || outSize > CUSTOME_SIZE_MAX_SIZE) {
            ERROR_LOG(
                "custom size out of range: custom size must be greater than 0 "
                "and "
                "less than or equal to 16GB.");
            throw std::runtime_error(
                "custom size num out of range. Please check.");
        }
    }
    SetContext();
...
std::vector<TensorBase> PyInferenceSession::InferBaseTensorVector(
    std::vector<std::string>& output_names,
    ...
    for (size_t i = 0; i < feeds.size(); ++i) {
        ...
        MemoryData src(feeds[i].buf, feeds[i].size,
...
std::vector<TensorBase> PyInferenceSession::FirstInnerInfer(
    std::vector<std::string>& output_names,
    std::vector<Base::BaseTensor>& feeds) {
    SetContext();
...
std::vector<TensorBase> PyInferenceSession::InnerInfer(
    const std::vector<int>& in_out_list, std::vector<std::string>& output_names,
    const bool get_outputs, const bool mem_copy) {
    SetContext();
    std::vector<TensorBase> outputs = {};
    APP_ERROR ret = modelInfer_.RepeatInference(in_out_list, output_names,
                                                outputs, get_outputs, mem_copy);
    if (ret != APP_ERR_OK) {
        throw std::runtime_error(GetError(ret));
    }
    return outputs;
...
void PyInferenceSession::OnlyInfer(std::vector<BaseTensor>& inputs,
                                   std::vector<std::string>& output_names,
                                   std::vector<TensorBase>& outputs) {
    SetContext();
    APP_ERROR ret = modelInfer_.Inference(inputs, output_names, outputs);
...
bool CheckExtraSession(
    size_t contextIndex,
    const std::vector<std::shared_ptr<PyInferenceSession>>& extraSession) {
    std::set<size_t> contextSet{contextIndex};
    for (const auto& session : extraSession) {
        auto newContextIndex = session->GetContextIndex();
        if (!contextSet.insert(newContextIndex).second) {
            return false;
        }
    }
    return true;
...
std::vector<std::vector<TensorBase>>
PyInferenceSession::InferPipelineBaseTensor(
    ...
    std::thread h2dThread(FuncH2d, std::ref(h2dQueue), std::ref(computeQueue),
                          this);
    std::thread computeThread(FuncCompute, std::ref(computeQueue),
                              std::ref(d2hQueue), this, nullptr);
    std::thread d2hThread(FuncD2h, std::ref(d2hQueue), std::ref(saveQueue),
                          this);
...
void PyInferenceSession::InferPipeline(
    ...
    if (!CheckExtraSession(contextIndex_, extraSession)) {
        ERROR_LOG(
            "infer wiht pipeline failed: cannot have session in same context");
        throw std::runtime_error(
            "pipeline failed: cannot have session in same context");
    }
    ...
    std::vector<InferSummaryInfo> summaryInfoGroup(numThreads - 1);

    for (size_t i = 0; i < numThreads; i++) {
        Base::PyInferenceSession* session = this;
        InferSummaryInfo* inferSummary = nullptr;
        if (i != 0) {
            session = extraSession[i - 1].get();
            inferSummary = &(summaryInfoGroup[i - 1]);
            session->modelInfer_.GetMutableSumaryInfo().zero_point =
                this->GetSumaryInfo().zero_point;
        }
        prepareThreadGroup.emplace_back(FuncPrepare, std::ref(h2dQueues[i]),
                                        session, std::ref(infilesList),
                                        inferOption, numThreads, i);
        h2dThreadGroup.emplace_back(FuncH2d, std::ref(h2dQueues[i]),
                                    std::ref(computeQueues[i]), session);
        computeThreadGroup.emplace_back(FuncCompute, std::ref(computeQueues[i]),
                                        std::ref(d2hQueues[i]), session,
                                        inferSummary);
        d2hThreadGroup.emplace_back(FuncD2h, std::ref(d2hQueues[i]),
                                    std::ref(saveQueues[i]), session);
        saveThreadGroup.emplace_back(FuncSave, std::ref(saveQueues[i]),
                                     inferOption);
    }
...
TensorBase PyInferenceSession::CreateTensorFromFilesList(
    ...
    for (uint32_t i = 0; i < filesList.size(); i++) {
        ...
        if (ret != SUCCESS) {
            ERROR_LOG("TensorBase malloc i:%d file:%s failed ret:%d", i,
                      filesList[i].c_str(), ret);
            throw std::runtime_error(GetError(ret));
        }
    }
...
namespace {
std::shared_ptr<Base::PyInferenceSession> CreateModelInstance(
    const std::string& modelPath, const uint32_t& deviceId,
    std::shared_ptr<Base::SessionOptions> options) {
    return std::make_shared<Base::PyInferenceSession>(modelPath, deviceId,
                                                      options);
}
}  // namespace
...
void RegistOptions(py::module& m) {
    ...
    py::class_<Base::SessionOptions, std::shared_ptr<Base::SessionOptions>>(
        m, "session_options")
        .def(
            py::init([]() { return std::make_shared<Base::SessionOptions>(); }))
        ...
        .def_readwrite("workspace_mem_optimize", &Base::SessionOptions::workspaceMemOptimize);
...
void RegistInferenceSession(py::module& m) {
    ...
    py::class_<Base::InferSummaryInfo>(m, "Summary")
        .def(py::init<>())
        .def_readwrite("exec_time_list", &Base::InferSummaryInfo::execTimeList);

    auto model = py::class_<Base::PyInferenceSession,
                            std::shared_ptr<Base::PyInferenceSession>>(
        m, "InferenceSession");
    model.def(
        py::init<std::string&, int, std::shared_ptr<Base::SessionOptions>>());
    ...
    model.def("run_pipeline",
              &Base::PyInferenceSession::InferPipelineBaseTensor);
    ...
    model.def("sumary", &Base::PyInferenceSession::GetSumaryInfo,
              py::return_value_policy::reference);
    model.def("summary", &Base::PyInferenceSession::GetSumaryInfo,
              py::return_value_policy::reference);
    model.def("get_inputs", &Base::PyInferenceSession::GetInputs,
              py::return_value_policy::reference);
    ...
    model.def("set_dynamic_batchsize",
              &Base::PyInferenceSession::SetDynamicBatchsize);
    ...
    model.def("set_custom_outsize",
              &Base::PyInferenceSession::SetCustomOutTensorsSize);

    model.def("create_tensor_from_fileslist",
              &Base::PyInferenceSession::CreateTensorFromFilesList);
    ...
    m.def("model", &CreateModelInstance, "modelPath"_a, "deviceId"_a = 0,
          "options"_a = py::none());
}

void RegistAippConfig(
    py::class_<Base::PyInferenceSession,
               std::shared_ptr<Base::PyInferenceSession>>& model) {
    using namespace pybind11::literals;

    model.def("get_max_dym_batchsize",
              &Base::PyInferenceSession::GetMaxDymBatchsize);
    model.def("get_dym_aipp_input_exist",
              &Base::PyInferenceSession::GetDymAIPPInputExist);
    model.def("check_dym_aipp_input_exist",
              &Base::PyInferenceSession::CheckDymAIPPInputExist);
    model.def("set_dym_aipp_info_set",
              &Base::PyInferenceSession::SetDymAIPPInfoSet);

    model.def("aipp_set_max_batch_size",
              &Base::PyInferenceSession::AippSetMaxBatchSize);
    model.def("aipp_set_input_format",
              &Base::PyInferenceSession::SetInputFormat);
    model.def("aipp_set_src_image_size",
              &Base::PyInferenceSession::SetSrcImageSize);
    model.def("aipp_set_rbuv_swap_switch",
              &Base::PyInferenceSession::SetRbuvSwapSwitch);
    model.def("aipp_set_ax_swap_switch",
              &Base::PyInferenceSession::SetAxSwapSwitch);
    ...
    model.def("aipp_set_padding_params",
              &Base::PyInferenceSession::SetPaddingParams);
    model.def("aipp_set_dtc_pixel_mean",
              &Base::PyInferenceSession::SetDtcPixelMean);
    model.def("aipp_set_dtc_pixel_min",
