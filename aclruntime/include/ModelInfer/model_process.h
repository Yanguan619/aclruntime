...
#ifndef ACLRUNTIME_INCLUDE_MODELINFER_MODEL_PROCESS_H_
#define ACLRUNTIME_INCLUDE_MODELINFER_MODEL_PROCESS_H_
#include <chrono>
#include <string>
#include <vector>
...
enum class MemoryPolicy { FREE_MEMORY, RETAIN_MEMORY };
...
public:
     ...
     * @return result
     */
    UtilsResult::Result LoadModelFromFile(
        const std::string& modelPath, const std::string& weightDir = "",
        std::shared_ptr<Base::SessionOptions> options = nullptr);

    UtilsResult::Result LoadModelFromMem(const void* modelData,
                                         size_t modelSize);
     ...
     */
    UtilsResult::Result CheckDynamicDims(std::vector<std::string> dym_dims,
                                         size_t gearCount, aclmdlIODims* dims);

    /**
     * @brief check dynamic input batch valid
     */
    UtilsResult::Result CheckDynamicBatchSize(uint64_t dymbatch,
                                              bool& is_dymbatch);

    /**
     * @brief check dynamic input image size valid
     */
    UtilsResult::Result CheckDynamicHWSize(std::pair<int, int> dynamicPair,
                                           bool& is_dymHW);
     ...
     * @brief check dynamic input image size valid
     */
    UtilsResult::Result CheckDynamicShape(
     ...
     * @brief set dynamic input dims
     */
    UtilsResult::Result SetDynamicShape(
        std::map<std::string, std::vector<int64_t>> dym_shape_map,
        std::vector<int64_t>& dims_num);
    ...
    // ------------------分别配置具体AIPP参数-----------------
    UtilsResult::Result SetAIPPSrcImageSize(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet);

    UtilsResult::Result SetAIPPInputFormat(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet);

    UtilsResult::Result SetAIPPCscParams(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet);

    UtilsResult::Result SetAIPPRbuvSwapSwitch(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet);

    UtilsResult::Result SetAIPPAxSwapSwitch(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet);

    UtilsResult::Result SetAIPPDtcPixelMean(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet, size_t batchIndex);

    UtilsResult::Result SetAIPPDtcPixelMin(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet, size_t batchIndex);

    UtilsResult::Result SetAIPPPixelVarReci(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet, size_t batchIndex);

    UtilsResult::Result SetAIPPCropParams(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet, size_t batchIndex);

    UtilsResult::Result SetAIPPPaddingParams(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
        aclmdlAIPP* aippDynamicSet, size_t batchIndex);
    // ------------------分别配置具体AIPP参数-----------------
     ...
     */
    UtilsResult::Result GetDymAIPPConfigSet(
        std::shared_ptr<Base::DynamicAippConfig> dyAippCfg,
     ...
     */
    UtilsResult::Result GetAIPPIndexList(
     ...
     * @return result
     */
    UtilsResult::Result UpdateInputsReuse(
     ...
     * @return result
     */
    UtilsResult::Result UpdateInputsMemcpy(
        const std::vector<int>& inOutRelation);
     ...
     */
    UtilsResult::Result GetCurOutputDimsMul(
    ...
    UtilsResult::Result GetInTensorDesc(size_t i, std::string& name,
                                        int& datatype, size_t& format,
                                        std::vector<int64_t>& shape,
                                        size_t& size);
    UtilsResult::Result GetOutTensorDesc(size_t i, std::string& name,
                                         int& datatype, size_t& format,
                                         std::vector<int64_t>& shape,
                                         size_t& size);
    ...
    UtilsResult::Result GetCurOutputShape(size_t index, bool is_dymshape,
                                          std::vector<int64_t>& shape);
