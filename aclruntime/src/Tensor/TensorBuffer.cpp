...
APP_ERROR TensorBuffer::SetContext() const {
  if (IsDevice()) {
    APP_ERROR ret =
        TensorContext::GetInstance()->SetContext(deviceId, contextIndex);
    if (ret != APP_ERR_OK) {
...
APP_ERROR TensorBuffer::CheckCopyValid(const TensorBuffer &buffer1,
  ...
  if (buffer1.size != buffer2.size) {
    ERROR_LOG("param1 data size(%lu) not match to param2 size(%lu)",
...
TensorBufferCopyType TensorBuffer::GetBufferCopyType(
    const TensorBuffer &buffer1, const TensorBuffer &buffer2) {
...
APP_ERROR TensorBuffer::CopyBetweenHost(TensorBuffer &dst,
  ...
  try {
    std::copy((uint8_t *)src.data.get(), (uint8_t *)src.data.get() + src.size,
...
APP_ERROR TensorBuffer::CopyBetweenHostDevice(TensorBuffer &dst,
                                              const TensorBuffer &src) {
  APP_ERROR ret = CheckCopyValid(dst, src);
  if (ret != APP_ERR_OK) {
    ERROR_LOG("check copy valid failed. ret=%d", ret);
...
APP_ERROR TensorBuffer::CopyBetweenSameDevice(TensorBuffer &dst,
                                              const TensorBuffer &src) {
  APP_ERROR ret = CheckCopyValid(dst, src);
  if (ret != APP_ERR_OK) {
    ERROR_LOG("check copy valid failed. ret=%d", ret);
...
APP_ERROR TensorBuffer::CopyBetweenDiffDevice(TensorBuffer &dst,
                                              const TensorBuffer &src) {
  APP_ERROR ret = CheckCopyValid(dst, src);
  if (ret != APP_ERR_OK) {
    ERROR_LOG("check copy valid failed. ret=%d", ret);
  ...
  // Direct device-to-device copy via ACL runtime (handles cross-device
  // natively). Previously this routed through a host staging buffer (D2H +
  // H2D), which temporarily doubled device memory and wasted bandwidth.
  ret = src.SetContext();
  if (ret != APP_ERR_OK) {
    ERROR_LOG("set context failed. ret=%d", ret);
    return ret;
...
APP_ERROR TensorBuffer::TensorBufferCopy(TensorBuffer &dst,
                                         const TensorBuffer &src) {
