...
import shutil
import math
from ais_bench.infer.common.io_operations import (PURE_INFER_FAKE_FILE,
...
from ais_bench.infer.common.utils import list_split
...
class TestGetNarrayFromFileList(unittest.TestCase):
    ...
    def test_get_narray_from_files_list(self, mock_file_content, mock_concatenate, mock_exists):
...
class TestGetTensorFromFileList(unittest.TestCase):
    ...
    def test_get_tensor_from_files_list_given_valid_files_when_no_combine_tensor_mode_then_success(self, mock_get_narray):
        ...
        # 调用函数
        result = get_tensor_from_files_list(files_list, session, size, pure_data_type, no_combine_tensor_mode)

        # 验证结果
        self.assertEqual(result, "tensor")
        mock_get_narray.assert_called_once_with(files_list, size, pure_data_type, no_combine_tensor_mode)
...
class TestCreateInfileslistFromFileslist(unittest.TestCase):
    ...
    def test_create_infileslist_from_fileslist_given_matching_lengths_when_no_combine_tensor_mode_then_success(self, mock_list_split, mock_get_files_count_per_batch):
        ...
        # Act
        result = create_infileslist_from_fileslist(fileslist, intensors_desc, no_combine_tensor_mode)
        print(f"{result=}")
        # Assert
    ...
    @patch('ais_bench.infer.common.io_operations.get_files_count_per_batch')
    def test_create_infileslist_from_fileslist_given_mismatching_lengths_when_no_combine_tensor_mode_then_failure(self, mock_get_files_count_per_batch):
        # Arrange
        ...
        # Act and Assert
        with self.assertRaises(RuntimeError):
            create_infileslist_from_fileslist(fileslist, intensors_desc, no_combine_tensor_mode)

    @patch('ais_bench.infer.common.io_operations.get_files_count_per_batch')
    def test_create_infileslist_from_fileslist_given_mismatching_lengths_when_combine_tensor_mode_then_failure(self, mock_get_files_count_per_batch):
        # Arrange
        fileslist = ['file1', 'file2', 'file3']
        intensors_desc = [1, 2]
        no_combine_tensor_mode = True
...
class TestCreateIntensorsFromInfileslist(unittest.TestCase):
    ...
    def test_create_intensors_from_infileslist_given_valid_infileslist_then_success(self, mock_get_tensor_from_files_list):
        # Arrange
...
class TestCheckInputParameter(unittest.TestCase):
    ...
    @patch('os.path.isdir')
    @patch('os.readlink')
    def test_check_input_parameter_given_valid_files_then_success(self, mock_readlink, mock_isdir, mock_isfile):
        # Arrange
    ...
    @patch('os.path.isdir')
    @patch('os.readlink')
    def test_check_input_parameter_given_valid_dirs_then_success(self, mock_readlink, mock_isdir, mock_isfile):
        # Arrange
    ...
    @patch('os.path.isdir')
    @patch('os.readlink')
    def test_check_input_parameter_given_invalid_files_then_failure(self, mock_readlink, mock_isdir, mock_isfile):
        # Arrange
        inputs_list = ['file1', 'file2']
        intensors_desc = [TensorDesc(realsize=10)]
    ...
    @patch('os.path.isdir')
    @patch('os.readlink')
    def test_check_input_parameter_given_invalid_dirs_then_failure(self, mock_readlink, mock_isdir, mock_isfile):
        # Arrange
        inputs_list = ['dir1', 'dir2']
        intensors_desc = [TensorDesc(realsize=10), TensorDesc(realsize=20)]
    ...
    @patch('os.path.isdir')
    @patch('os.readlink')
    def test_check_input_parameter_given_valid_dirs_then_true(self, mock_readlink, mock_isdir, mock_isfile):
        # Arrange
        inputs_list = ['dir1', 'dir2']
        intensors_desc = [TensorDesc(realsize=10), TensorDesc(realsize=20)]
        mock_isdir.return_value = True
    ...
    def test_check_input_parameter_given_valid_dirs_then_true(self):
        # Arrange
        inputs_list = []
        intensors_desc = [TensorDesc(realsize=10), TensorDesc(realsize=20)]
        with self.assertRaises(RuntimeError):
...
class TestCreateInfileslistFromInputsList(unittest.TestCase):
    ...
    @patch('os.path.isfile')
    def test_create_infileslist_from_inputs_list_given_files_when_success_then_returns_infileslist(self, mock_isfile, mock_create_infileslist_from_fileslist):
        # Arrange
        mock_isfile.return_value = True
        inputs_list = ["file1.txt", "file2.txt", "file3.txt"]
        intensors_desc = [1, 2, 3]
        no_combine_tensor_mode = False
        mock_create_infileslist_from_fileslist.return_value = [["file1", "file2"], ["file3", "file4"]]

        # Act
        result = create_infileslist_from_inputs_list(inputs_list, intensors_desc, no_combine_tensor_mode)
    ...
    @patch('os.path.isfile')
    @patch('os.path.isdir')
    def test_create_infileslist_from_inputs_list_given_dirs_when_success_then_returns_infileslist(self, mock_isdir, mock_isfile, mock_create_infileslist_from_fileslist, mock_get_fileslist_from_dir):
        # Arrange
        mock_isfile.return_value = False
        mock_isdir.return_value = True
        inputs_list = ["file1.txt", "file2.txt", "file3.txt"]
    ...
    @patch('os.path.isfile')
    @patch('os.path.isdir')
    def test_create_infileslist_from_inputs_list_given_none_when_success_then_returns_infileslist(self, mock_isdir, mock_isfile):
        # Arrange
        mock_isfile.return_value = False
        mock_isdir.return_value = False
        inputs_list = ["file1.txt", "file2.txt", "file3.txt"]
    ...
    @patch('os.path.isfile')
    @patch('os.path.isdir')
    def test_create_infileslist_from_inputs_list_given_dirs_when_create_result_is_none(self, mock_isdir, mock_isfile, mock_create_infileslist_from_fileslist, mock_get_fileslist_from_dir):
        # Arrange
        mock_isfile.return_value = False
        mock_isdir.return_value = True
        inputs_list = ["file1.txt", "file2.txt", "file3.txt"]
...
class TestCheckPipelineFileslistMatchIntensors(unittest.TestCase):
    ...
    def test_check_pipeline_fileslist_match_intensors_given_matching_lengths_and_sizes_then_success(self, mock_get_file_datasize):
        # Arrange
        fileslist = [['file1'], ['file2'], ['file3']]
        intensors_desc = [
            MagicMock(realsize=1024, size=1024, shape=[10, 10], auto_dim_mode=False, auto_shape_mode=False),
            MagicMock(realsize=1024, size=1024, shape=[10, 10], auto_dim_mode=False, auto_shape_mode=False),
            MagicMock(realsize=1024, size=1024, shape=[10, 10], auto_dim_mode=False, auto_shape_mode=False)
        ]
    ...
    @patch('ais_bench.infer.common.io_operations.get_file_datasize')
    def test_check_pipeline_fileslist_match_intensors_given_mismatching_lengths_then_failure(self, mock_get_file_datasize):
        # Arrange
        fileslist = [['file1'], ['file2'], ['file3']]
        intensors_desc = [
            MagicMock(realsize=1024, size=1324, shape=[10, 10], auto_dim_mode=False, auto_shape_mode=False),
            MagicMock(realsize=1024, size=1324, shape=[10, 10], auto_dim_mode=False, auto_shape_mode=False)
        ]
...
class TestCreatePipelineFileslistFromInputsList(unittest.TestCase):
    ...
    @patch('ais_bench.infer.common.io_operations.check_pipeline_fileslist_match_intensors')
    @patch('os.path.isfile')
    def test_create_pipeline_fileslist_from_inputs_list_given_valid_file_inputs_then_success(self, mock_isfile, mock_check_pipeline_fileslist_match_intensors, mock_check_input_parameter, mock_list_split):
        # Arrange
        mock_isfile.return_value = True
        ...
        mock_check_pipeline_fileslist_match_intensors.assert_called_once_with(mock_list_split.return_value, intensors_desc)
    
    @patch('ais_bench.infer.common.io_operations.get_fileslist_from_dir')
    @patch('ais_bench.infer.common.io_operations.check_input_parameter')
    @patch('ais_bench.infer.common.io_operations.check_pipeline_fileslist_match_intensors')
    @patch('os.path.isdir')
    def test_create_pipeline_fileslist_from_inputs_list_given_valid_dir_inputs_then_success(self, mock_isdir, mock_check_pipeline_fileslist_match_intensors, mock_check_input_parameter, mock_get_fileslist_from_dir):
        # Arrange
        mock_isdir.return_value = True
        inputs_list = ['file1', 'file2', 'file3', 'file4']
        intensors_desc = [1, 2, 3, 4]
    ...
    @patch('os.path.isdir')
    @patch('os.path.isfile')
    def test_create_pipeline_fileslist_from_inputs_list_given_invalid_dir_inputs_then_failure(self, mock_isfile, mock_isdir, mock_check_input_parameter):
        # Arrange
        mock_isfile.return_value = False
        mock_isdir.return_value = False
        inputs_list = ['file1', 'file2', 'file3', 'file4']
    ...
    @patch('ais_bench.infer.common.io_operations.check_input_parameter')
    @patch('os.path.isdir')
    def test_create_pipeline_fileslist_from_inputs_list_given_valid_dir_inputs_then_failure(self, mock_isdir, mock_check_input_parameter, mock_get_fileslist_from_dir):
        # Arrange
        mock_isdir.return_value = True
        inputs_list = ['file1', 'file2', 'file3', 'file4']
        intensors_desc = [1, 2, 3, 4]
...
class TestSaveTensorsToFile(unittest.TestCase):
    ...
    def test_save_tensors_to_file_given_invalid_inputs_then_failure(self, mock_array_split, mock_array, mock_transpose, mock_basename, mock_join):
        # Arrange
        infiles_paths = [['file1.txt', 'file2.txt'], ['file3.txt', 'file4.txt']]
        outfmt = 'npy'
        index = 0
        output_batchsize_axis = 0
        out_file_info = OutFileInfo(output_prefix='/output', outputs=[np.array([1, 2, 3]), np.array([4, 5, 6])])
        mock_transpose.return_value = [['file1.txt', 'file3.txt'], ['file2.txt', 'file4.txt']]
        ...
        # Act
        with self.assertRaises(RuntimeError):
            save_tensors_to_file(infiles_paths, outfmt, index, output_batchsize_axis, out_file_info)
    ...
    @patch('numpy.array')
    @patch('numpy.array_split')
    def test_save_tensors_to_file_given_invalid_inputs_then_failure1(self, mock_array_split, mock_array, mock_transpose, mock_basename, mock_join):
        # Arrange
        infiles_paths = [['file1.txt', 'file2.txt'], ['file3.txt', 'file4.txt']]
        outfmt = 'npy'
        index = 0
    ...
    def test_save_tensors_to_file_given_valid_inputs_then_success(self, mock_save_data, mock_array_split, mock_array, mock_transpose, mock_basename, mock_join):
        # Arrange
