...
TEST(CnpyTest, ParseNpyHeader_ThrowsOnNullFile) {
    ...
    EXPECT_THROW(cnpy::ParseNpyHeader(nullptr, wordSize, shape, fortranOrder),
                 std::runtime_error);
}
...
TEST(CnpyTest, ParseNpyHeader_ThrowsOnShortFile) {
    ...
    std::vector<size_t> shape;
    bool fortranOrder = false;
    EXPECT_THROW(cnpy::ParseNpyHeader(fp, wordSize, shape, fortranOrder),
                 std::runtime_error);
    fclose(fp);
}
...
TEST(CnpyTest, ParseNpyHeader_Success) {
    ...
    std::string header =
        "{'fortran_order': False, 'shape': (2, 3), 'descr': '<f4'}";
...
TEST(CnpyTest, NpyLoad_Success) {
    ...
    std::string header =
        "{'fortran_order': False, 'shape': (2, 2), 'descr': '<f4'}";
    header.append(75 - header.size(), ' ');
    header.push_back('\n');
    unsigned char header_len = static_cast<unsigned char>(header.size());
    fwrite(&header_len, 1, 1, fp);
...
TEST(CnpyTest, ParseNpyHeader_ShapeReverseParenthesis) {
    ...
    fwrite("\x01\x00", 1, 2, fp);
    std::string header =
        "{'fortran_order': False, ')descr': '<f4', 'shape': (2, 3), }";
    header.append(75 - header.size(), ' ');
    header.push_back('\n');
    unsigned char header_len = static_cast<unsigned char>(header.size());
    fwrite(&header_len, 1, 1, fp);
...
TEST(CnpyTest, ParseNpyHeader_NegativeWordSize) {
    ...
    // descr: '<-4'
    std::string header =
        "{'fortran_order': False, 'shape': (2, 3), 'descr': '<-4'}";
    header.append(75 - header.size(), ' ');
    header.push_back('\n');
    unsigned char header_len = static_cast<unsigned char>(header.size());
    fwrite(&header_len, 1, 1, fp);
...
TEST(CnpyTest, BinLoad_OpenFileFail) {
    ...
    // 传入非法路径，OpenFile 会失败
    EXPECT_THROW(cnpy::BinLoad("/this/path/should/not/exist/abc.bin"),
                 std::runtime_error);
}
...
TEST(CnpyTest, BinLoad_ReadContentCorrect) {
    ...
    EXPECT_EQ(std::string(arr.dataHolder->begin(), arr.dataHolder->end()),
              content);
    remove(fname.c_str());
}
