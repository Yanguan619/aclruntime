...
protected:
    void SetUp() override {
        ...
        file.close();
        // 创建符号链接
        ASSERT_EQ(symlink(File::GetAbsPath(testRegularFile).c_str(),
                          testLink.c_str()),
                  0);
...
TEST_F(FileTest, TestGetAbsPath) {
    ...
    EXPECT_EQ(pwd + "/testpath",
              File::GetAbsPath("subdir/subdir/.././../testpath"));
    EXPECT_EQ(pwd + "/subdir/testpath",
              File::GetAbsPath("./subdir/.././/subdir/testpath"));
}
...
TEST_F(FileTest, TestIsPathDepthValid) {
    ...
    EXPECT_TRUE(
        File::IsPathDepthValid(std::string(PATH_DEPTH_MAX, PATH_SEPARATOR)));
    EXPECT_FALSE(File::IsPathDepthValid(
        std::string(PATH_DEPTH_MAX + 1, PATH_SEPARATOR)));
}
...
TEST_F(FileTest, TestCheckDir_PathIsEmpty) { EXPECT_FALSE(File::CheckDir("")); }
...
TEST_F(FileTest, OpenFile_Write_TooLongPath) {
    std::string longPath =
        testDir + "/" + std::string(FULL_PATH_LENGTH_MAX + 1, 'a');
    std::ofstream ofs;
...
TEST_F(FileTest, DeleteFile_EmptyPath) { EXPECT_TRUE(File::DeleteFile("")); }
...
TEST_F(FileTest, CheckFileSuffixAndSize_CommonType) {
    ...
    TEST_ExecShellCommand("truncate -s " +
                          std::to_string(MAX_FILE_SIZE_DEFAULT + 1) + " " +
                          filePath);
...
TEST_F(FileTest, CheckFileSuffixAndSize_KnownTypes) {
    ...
    for (const auto& c : cases) {
        ...
        TEST_ExecShellCommand("truncate -s " + std::to_string(c.maxSize + 1) +
                              " " + filePath);
...
TEST_F(FileTest, CheckFileSuffixAndSize_UnknownTypeOrFileNotExist) {
    ...
    ofs.close();
    EXPECT_FALSE(
        File::CheckFileSuffixAndSize(filePath, static_cast<FileType>(999)));
    // File not exist
    EXPECT_FALSE(
        File::CheckFileSuffixAndSize(testNotExistsFile, FileType::COMMON));
}
...
TEST_F(FileTest, CheckFileBeforeRead_SuffixAndSizeInvalid) {
    ...
    // Too large for CSV
    TEST_ExecShellCommand("truncate -s " + std::to_string(MAX_CSV_SIZE + 1) +
                          " " + filePath);
...
TEST_F(FileTest,
       CheckFileBeforeCreateOrWrite_Exist_RegularFile_Owner_Writable) {
    std::ofstream ofs(testRegularFile);
    ofs << "test";
    ofs.close();
