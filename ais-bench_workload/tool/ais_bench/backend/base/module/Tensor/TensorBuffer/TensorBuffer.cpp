...
APP_ERROR TensorBuffer::SetContext() const
{
    if (IsDevice()) {
        ...
        if (ret != APP_ERR_OK) {
            ERROR_LOG("SetContext failed. ret=%s", ret);
            return ret;
        }
    }
    return APP_ERR_OK;
...
APP_ERROR TensorBuffer::TensorBufferMalloc(TensorBuffer &buffer)
{
    ...
    if (ret != APP_ERR_OK) {
        ERROR_LOG("MemoryHelper::MxbsMalloc failed. ret=%s", ret);
        return ret;
    }
...
APP_ERROR TensorBuffer::CopyBetweenHost(TensorBuffer &dst, const TensorBuffer &src)
{
    ...
    if (ret != APP_ERR_OK) {
        ERROR_LOG("CheckCopyValid failed. ret=%d", ret);
        return ret;
    }
    try {
...
APP_ERROR TensorBuffer::CopyBetweenDiffDevice(TensorBuffer &dst, const TensorBuffer &src)
{
    ...
    if (ret != APP_ERR_OK) {
        ERROR_LOG("CopyBetweenHostDevice failed. ret=%d", ret);
        return ret;
    }
...
APP_ERROR TensorBuffer::TensorBufferCopy(TensorBuffer &dst, const TensorBuffer &src)
{
    ...
    if (copyType == TensorBufferCopyType::HOST_AND_HOST) {
        ...
        if (ret != APP_ERR_OK) {
            ERROR_LOG("CopyBetweenHost failed. ret=%d", ret);
            return ret;
        }
        return APP_ERR_OK;
    }
    ...
    if (ret != APP_ERR_OK) {
        ERROR_LOG("CopyBetweenDiffDevice failed. ret=%d", ret);
        return ret;
    }
    return APP_ERR_OK;
}
