...
#ifndef ACLRUNTIME_INCLUDE_MODELINFER_FILE_H_
#define ACLRUNTIME_INCLUDE_MODELINFER_FILE_H_
...
constexpr mode_t WRITE_FILE_NOT_PERMITTED =
    S_IWGRP | S_IWOTH | S_IROTH | S_IXOTH;
...
public:
    ...
    // 安全的创建目录
    static bool CreateDir(const std::string& path, bool recursion = false,
                          mode_t mode = NORMAL_DIR_MODE_DEFAULT);
    // 打开文件
    static bool OpenFile(const std::string& path, std::ifstream& ifs,
                         std::ios::openmode mode = std::ios::in);
    static bool OpenFile(const std::string& path, std::ofstream& ofs,
    ...
    static bool CheckFileRWX(const std::string& path,
    ...
    static bool CheckFileBeforeCreateOrWrite(const std::string& path,
                                             bool overwrite = false);
};

#endif  // ACLRUNTIME_INCLUDE_MODELINFER_FILE_H_
