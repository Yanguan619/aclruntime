...
public:
    ...
    virtual bool CheckFileBeforeRead(const std::string& path,
                                     FileType type) = 0;
};
...
protected:
    void SetUp() override {
        ...
        ON_CALL(*mockFileChecker,
                CheckFileBeforeRead("valid_model.om", FileType::OM))
            .WillByDefault(Return(true));
        ON_CALL(*mockFileChecker,
                CheckFileBeforeRead("valid_config.json", FileType::JSON))
            .WillByDefault(Return(true));
    }
...
TEST_F(PyInferenceSessionTest, InvalidLoopSize) {
    // loop=0
    options->loop = 0;
    EXPECT_THROW(PyInferenceSession("valid_model.om", 0, options),
                 std::runtime_error);
...
TEST_F(PyInferenceSessionTest, InvalidModelPath) {
    EXPECT_THROW(PyInferenceSession("invalid_model.om", 0, options),
                 std::runtime_error);
}
...
TEST_F(PyInferenceSessionTest, InvalidDeviceId) {
    // deviceId=-1
    EXPECT_THROW(PyInferenceSession("valid_model.om", -1, options),
                 std::runtime_error);

    // deviceId=256（假设DEVICE_ID_MAX=255）
    EXPECT_THROW(PyInferenceSession("valid_model.om", 256, options),
                 std::runtime_error);
}
}  // namespace AISBench_test
