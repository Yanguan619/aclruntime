...
struct NpyArray {
    NpyArray(const std::vector<size_t> &shape, size_t wordSize,
             bool fortranOrder)
        : shape(shape),
          wordSize(wordSize),
...
void NpySave(std::string fname, const T *data, const std::vector<size_t> shape,
    ...
    if (mode == "w") {
        ...
        int fd =
            open(fname.c_str(), O_EXCL | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP);
        close(fd);
    }
    if (mode == "a") {
    ...
    if (fp) {
        ...
        if (wordSize != sizeof(T)) {
            ERROR_LOG(
                "libnpy error: %s has word size %zu but saving npy appending "
                "data "
                "sized %zu\n",
        ...
        if (trueDataShape.size() != shape.size()) {
            ERROR_LOG(
                "libnpy error: saving npy attempting to append misdimensioned "
                "data "
                "to %s\n",
                fname.c_str());
        ...
        for (size_t i = 1; i < shape.size(); i++) {
            if (shape[i] != trueDataShape[i]) {
                ERROR_LOG(
                    "libnpy error: saving npy attempting to append misshaped "
                    "data to "
                    "%s",
                    fname.c_str());
