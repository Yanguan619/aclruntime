...
int main(int argc, char **argv)
{
    ...
    AsdSip::asdBlasHandle handle;
    AsdSip::asdBlasCreate(handle);

    size_t lwork = 0;
    void *buffer = nullptr;
    AsdSip::asdBlasMakeDotPlan(handle);
    AsdSip::asdBlasGetWorkspaceSize(handle, lwork);
    std::cout << "lwork = " << lwork << std::endl;
    if (lwork > 0) {
        ret = aclrtMalloc(&buffer, static_cast<int64_t>(lwork), ACL_MEM_MALLOC_HUGE_FIRST);
        CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("allocate workspace failed. ERROR: %d\n", ret); return ret);
    }
    AsdSip::asdBlasSetWorkspace(handle, buffer);
    AsdSip::asdBlasSetStream(handle, stream);
    auto xxx = AsdSip::asdBlasSdot(handle, n, inputX, incx, inputY, incy, result);
    std::cout<< "AsdSip::asdBlasSdot(handle, n, inputX, incx, inputY, incy, result) = " << xxx <<std::endl;
    // ASD_STATUS_CHECK(
    //     AsdSip::asdBlasSdot(handle, n, inputX, incx, inputY, incy, result)
    // );

    AsdSip::asdBlasSynchronize(handle);
    AsdSip::asdBlasDestroy(handle);
