...
#undef SET_DEFAULT_EXPECT
  ...
  void SetupCompleteModel(size_t numInputs = 2,
                          const vector<const char*>& inputNames = {"input1",
                                                                   "input2"}) {
    // 模拟加载模型成功
    LoadModelSuccess();
...
TEST_F(ModelProcessTest, TestCreateDymInput_Success) {
  ...
  EXPECT_CALL(
      *mockAcl,
      aclmdlDestroyDataset(reinterpret_cast<const aclmdlDataset*>(fakeDataset)))
      .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestCreateInput_CreateDatasetFailed) {
  ...
  // 设置模拟期望
  EXPECT_CALL(*mockAcl, aclmdlCreateDataset()).WillRepeatedly(Return(nullptr));
...
TEST_F(ModelProcessTest, TestCreateInput_AddDatasetBufferFailed) {
  ...
  EXPECT_CALL(*mockAcl, aclGetRecentErrMsg()).WillRepeatedly(Return(errorMsg));
...
TEST_F(ModelProcessTest, TestDestroyInput_GetBufferFailed) {
  ...
  // 期望释放内存
  EXPECT_CALL(*mockAcl, aclrtFree(memoryBuffer)).WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestExecute_Failed) {
  ...
  EXPECT_TRUE(logOutput.find("modelId is " + std::to_string(expectedModelId)) !=
              string::npos);
  EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
}
...
TEST_F(ModelProcessTest, TestUnload_SuccessWithModelDesc) {
  ...
  EXPECT_TRUE(logOutput.find("model Id is " +
                             std::to_string(expectedModelId)) != string::npos);
}
...
TEST_F(ModelProcessTest, TestCreateOutput_WithParams_CreateBufferFailed) {
  ...
  // 预期释放内存
  EXPECT_CALL(*mockAcl, aclrtFree(outputBuffer)).WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestSetInputAIPP_Success) {
  ...
  // 设置模拟行为
  EXPECT_CALL(*mockAcl, aclmdlSetInputAIPP(expectedModelId,
                                           modelProcess->input_, 0, dummyAipp))
