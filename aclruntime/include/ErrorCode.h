...
#ifndef ACLRUNTIME_INCLUDE_ERRORCODE_H_
#define ACLRUNTIME_INCLUDE_ERRORCODE_H_
...
enum {
    ...
    APP_ERR_COMM_CONNECTION_CLOSE =
        APP_ERR_COMM_BASE + 24,  // Out of connection, Communication shutdown
    ...
    APP_ERR_DVPP_PICDESC_FAIL =
        APP_ERR_DVPP_BASE + 12,  // DVPP: fail to create acldvppCreatePicDesc or
    APP_ERR_DVPP_CONFIG_FAIL =
        APP_ERR_DVPP_BASE + 13,  // DVPP: fail to set dvpp configuration,such as
    APP_ERR_DVPP_OBJ_FUNC_MISMATCH =
        APP_ERR_DVPP_BASE + 14,  // DVPP: DvppCommon object mismatch the function
    ...
    APP_ERR_PROTOBUF_NAME_MISMATCH =
        APP_ERR_DVPP_BASE + 18,  // DVPP: VpcReSize mismatch the protobuf name
    ...
    APP_ERR_INFER_FIND_MODEL_DESC_FAIL =
        APP_ERR_INFER_BASE + 7,  // Infer: find model description fail
    ...
    APP_ERR_INFER_DYNAMIC_IMAGE_SIZE_FAIL =
        APP_ERR_INFER_BASE + 10,                                 // Infer: find model weight fail
    ...
    APP_ERR_PLUGIN_TOOLKIT_METADATA_PLUGIN_NAME_KEY_ALREADY_EXIST =
        APP_ERR_PLUGIN_TOOLKIT_BASE + 15,
...
const std::map<int, std::string> APP_ERR_ACL_LOG_MAP = {
    {APP_ERR_OK, "Success"},
...
const std::map<int, std::string> APP_ERR_COMMON_LOG_MAP = {
    {0, "Success"},
    ...
    {APP_ERR_COMM_CONNECTION_CLOSE - APP_ERR_COMM_BASE,
...
const std::map<int, std::string> APP_ERR_DVPP_LOG_MAP = {
    {0, "Success"},
    ...
    {APP_ERR_DVPP_PICDESC_FAIL - APP_ERR_DVPP_BASE,
    ...
    {APP_ERR_DEVICE_ID_MISMATCH - APP_ERR_DVPP_BASE,
        "DVPP: DvppCommon object mismatch the function"},
    ...
    {APP_ERR_PROTOBUF_NAME_MISMATCH - APP_ERR_DVPP_BASE,
...
const std::map<int, std::string> APP_ERR_INFER_LOG_MAP = {
    {0, "Success"},
    ...
    {APP_ERR_INFER_FIND_MODEL_DESC_FAIL - APP_ERR_INFER_BASE,
...
const std::map<int, std::string> APP_ERR_COMMANDER_MAP = {
    {0, "Success"},
    {APP_ERR_COMMANDER_SPLIT_PARA_ERROR - APP_ERR_COMMANDER_BASE,
    ...
    {APP_ERR_COMMANDER_INFER_RESULT_ERROR - APP_ERR_COMMANDER_BASE,
        "Commander subtask inference result error."},
};

const std::map<int, std::string> APP_ERR_QUEUE_LOG_MAP = {
    {0, "Success"},
...
const std::map<int, std::string> APP_ERR_STREAM_LOG_MAP = {
    {0, "Success"},
    ...
    {APP_ERR_PIPELINE_PROPERTY_CONFIG_ERROR - APP_ERR_STREAM_BASE,
        "pipeline property config error"},
};

const std::map<int, std::string> APP_ERR_PLUGIN_TOOLKIT_LOG_MAP = {
    {0, "Success"},
    {APP_ERR_PLUGIN_TOOLKIT_CREATE_NODE_FAILED - APP_ERR_PLUGIN_TOOLKIT_BASE,
        "plugin toolkit creat node fail"},
    {APP_ERR_PLUGIN_TOOLKIT_NODE_ALREADY_EXIST - APP_ERR_PLUGIN_TOOLKIT_BASE,
        "plugin toolkit node already exist"},
    {APP_ERR_PLUGIN_TOOLKIT_MESSAGE_NOT_MATCH - APP_ERR_PLUGIN_TOOLKIT_BASE,
        "plugin toolkit message not match"},
    {APP_ERR_PLUGIN_TOOLKIT_PARENT_NOT_MATCH - APP_ERR_PLUGIN_TOOLKIT_BASE,
        "plugin toolkit parent not match"},
    {APP_ERR_PLUGIN_TOOLKIT_NOT_INITIALIZED - APP_ERR_PLUGIN_TOOLKIT_BASE,
        "plugin toolkit not initialized"},
    {APP_ERR_PLUGIN_TOOLKIT_NODELIST_NOT_EXIST - APP_ERR_PLUGIN_TOOLKIT_BASE,
        "plugin toolkit nodelist not exist"},
    {APP_ERR_PLUGIN_TOOLKIT_NODE_NOT_EXIST - APP_ERR_PLUGIN_TOOLKIT_BASE,
        "plugin toolkit node not exist"},
    {APP_ERR_PLUGIN_TOOLKIT_INVALID_MEMBERID - APP_ERR_PLUGIN_TOOLKIT_BASE,
    ...
    {APP_ERR_PLUGIN_TOOLKIT_METADATA_IS_NULL - APP_ERR_PLUGIN_TOOLKIT_BASE,
    ...
    {APP_ERR_PLUGIN_TOOLKIT_FLOW_ERROR - APP_ERR_PLUGIN_TOOLKIT_BASE,
        "plugin toolkit some error occurred.Element generating this error should post an error "
        "message"},
    {APP_ERR_PLUGIN_TOOLKIT_FLOW_NOT_SUPPORTED - APP_ERR_PLUGIN_TOOLKIT_BASE,
        "plugin toolkit operation is not supported"},
};

const std::map<int, std::string> APP_ERR_DEVICE_MANAGER_LOG_MAP = {
    {0, "Success"},
...
const std::map<int, std::string> APP_ERR_EXTRA_MAP = {
    ...
    {APP_ERR_OP_INPUT_NOT_MATCH - APP_ERR_EXTRA_BASE,
        "The input of a single operator does not match"},
    {APP_ERR_OP_OUTPUT_NOT_MATCH - APP_ERR_EXTRA_BASE,
        "The output of a single operator does not match"},
    {APP_ERR_OP_ATTR_NOT_MATCH - APP_ERR_EXTRA_BASE,
        "The attribute of the single operator does not match"},
    ...
    {APP_ERR_BIN_SELECTOR_NOT_REGISTERED - APP_ERR_EXTRA_BASE,
        "When the operator interface is compiled in binary selection mode, the operator has not "
        "registered a selector"},
    ...
    {APP_ERR_BIN_SELECTOR_ALREADY_REGISTERED - APP_ERR_EXTRA_BASE,
        "When the operator interface is compiled in binary selection mode, the operator is "
    ...
    {APP_ERR_NOT_STATIC_AIPP - APP_ERR_EXTRA_BASE,
        "The AIPP configuration information does not exist"},
};

const std::map<int, std::string> APP_ERR_INFER_MAP = {
...
const std::map<int, std::string> APP_ERR_STORAGE_MAP = {
    {0, "Exceeded the upper limit of storage"},
};
const std::map<int, std::string> APP_ERR_INTERNAL_MAP = {
    ...
    {APP_ERR_DRV_FAILURE - APP_ERR_INTERNAL_ERROR,
        "An internal DRV (Driver) error occurs in the system"},
...
#endif  // ACLRUNTIME_INCLUDE_ERRORCODE_H_
