...
class TestClass:
    ...
    def test_inference_normal_dynamic_shape_auto_set_dymshape_mode(self):
        ...
        self.create_npy_files_in_auto_set_dymshape_mode_input(
            auto_set_dymshape_mode_input_dir_path, shapes
        )
        ...
        for file in filelist:
            file_paths_list.append(
                os.path.join(auto_set_dymshape_mode_input_dir_path, file)
            )
        ...
        cmd = (
            "{} --model {} --device {} --outputSize {} --auto_set_dymshape_mode true --input {} --output {} \
    ...
    def test_general_inference_prformance_comparison_with_msame_dynamic_shape(self):
        ...
        msame_infer_log_path = os.path.join(
            output_path, output_dir_name, "msame_infer.log"
        )
    ...
    def test_pure_inference_normal_dynamic_shape_range_mode(self):
        ...
        cmd = (
            "{} --model {} --outputSize {} --dymShape_range {} --output {} --output_dirname {} > \
            {}".format(
                TestCommonClass.cmd_prefix,
                model_path,
                output_size,
        ...
        run_count, result_ok_num, shape_status = (
            self.get_dynamic_shape_range_mode_inference_result_info(log_path)
        )
    ...
    def test_pure_inference_normal_dynamic_shape_range_mode_3(self):
        ...
        dymshape_range_file = os.path.join(
            range_file_parent_path, "dymshape_range.info"
        )
    ...
    def test_general_inference_interface_dyshape_compare_tensor_npy(self):
        ...
        output_parent_path = os.path.join(self.model_base_path, "output")
        tensor_infer_result_file_path = os.path.join(
            output_parent_path, "tensor_infer_result.npy"
        )
        np.save(tensor_infer_result_file_path, tensor_outputs)

        # numpy interface
        ndata = ndata.numpy()
        session.infer([ndata], mode, custom_sizes=output_size)
        npy_infer_result_file_path = os.path.join(
            output_parent_path, "npy_infer_result.npy"
        )
    ...
    def test_general_inference_interface_dyshape_compare_tensor_discontinuous_tensor_continues(
        self,
    ):
        model_path = self.get_dynamic_shape_om_path()
        output_size = 100000
        ...
        tensor_infer_result1_path = os.path.join(
            output_parent_path, "first_tensor_infer_result.npy"
        )
        ...
        tensor_infer_result2_path = os.path.join(
            output_parent_path, "second_tensor_infer_result.npy"
        )
