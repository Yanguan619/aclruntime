...
class TestCommonClass:
    ...
    cmd_prefix = (
        ...
        + " "
        + os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "../ais_bench/__main__.py"
        )
    )
    base_path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "../test/testdata"
    )
    ...
    def get_cmd_prefix():
        ...
        return (
            sys.executable
            + " "
            + os.path.join(_current_dir, "../ais_bench/__main__.py")
        )
    ...
    def get_model_inputs_size(model_path):
        ...
        session = aclruntime.InferenceSession(
            model_path, TestCommonClass.default_device_id, options
        )
    ...
    def get_inference_execute_num(log_path):
        ...
        try:
            ...
            raise Exception(
                "grep action raises raise an exception: {}".format(e)
            ) from e
    ...
    def get_model_static_om_path(cls, batchsize, modelname):
        ...
        return os.path.join(
            ...
            "pth_{}_bs{}.om".format(modelname, batchsize),
        )

    @classmethod
    def get_legal_args(cls):
        current_directory = cls.get_basepath()
        model = os.path.join(current_directory, "resnet50/model/pth_resnet50_bs1.om")
