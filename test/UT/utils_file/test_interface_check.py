...
class TestClass:
    ...
    def rmforce(path):
        os.system(
            f"rm -rf {path}"
        )  # 这里之所以不用shutil.rmtree ，os.remove之类的是考虑到服务器会询问是否删除，干脆直接rm -rf了。

    @classmethod
    def check_illegal_fake_path_case(
        cls, func_to_test, fake_value, permission=0o750, is_exist=True
    ):
    ...
    def test_check_acl_json_path_legality(self):
        assert check_acl_json_path_legality("") is None
    ...
    def test_check_dict(self):
        ...
        dict_check = {"key1": "value1", "key2": "value2"}
        with pytest.raises(ValueError):
            check_dict(dict_check, max_len)
