...
def parse_bin_file(bin_file):
    ...
    log_msg = f"bin_file:{bin_file} format: {num_to_aclFormat.get(format_num)} dtype: {num_to_aclDataType.get(dtype_num)} shape: {shape}"
    ...
    logging.info(
        f"out npy dtype: {np_dtype} shape: {ndata.shape} out_file: {npy_file}\n"
    )
...
def get_args():
    ...
    parser.add_argument(
        "--input",
        "-i",
        help="exception cb bin file path.Support single file and folder path.",
    )
...
if __name__ == "__main__":
    ...
    if args.input is None or not os.path.exists(args.input):
        logging.info(
            "bad parameters. lack of input parameter or bin file is not exist."
        )
        sys.exit(1)
