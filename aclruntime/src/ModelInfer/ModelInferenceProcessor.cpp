...
APP_ERROR ModelInferenceProcessor::GetModelDescInfo() {
  ...
  for (size_t i = 0; i < numInputs; i++) {
    ...
    CHECK_RET_EQ(
        processModel->GetInTensorDesc(i, info.name, datatype, info.format,
  ...
  for (size_t i = 0; i < numOutputs; i++) {
    CHECK_RET_EQ(
        processModel->GetOutTensorDesc(i, info.name, datatype, info.format,
                                       info.shape, info.size),
        SUCCESS);
    info.realsize = info.size;
    info.datatype = (TensorDataType)datatype;
...
APP_ERROR ModelInferenceProcessor::Init(const std::string& modelPath,
  ...
  DEBUG_LOG(
      "[MEM_CHECK] ModelInferenceProcessor::Init start: RSS=%zuMB, model=%s",
  ...
  if (modelSize > 0 && modelData != nullptr) {
    ...
    DEBUG_LOG("[MEM_CHECK] Before LoadModelFromMem: RSS=%zuMB",
    ...
    DEBUG_LOG("[MEM_CHECK] After LoadModelFromMem: RSS=%zuMB",
              GetSystemMemoryUsedMB());
  } else {
    ...
    DEBUG_LOG("[MEM_CHECK] Before LoadModelFromFile: RSS=%zuMB",
              GetSystemMemoryUsedMB());
    CHECK_RET_EQ(
        processModel->LoadModelFromFile(modelPath, options->weightDir, options),
        SUCCESS);
    DEBUG_LOG("[MEM_CHECK] After LoadModelFromFile: RSS=%zuMB",
              GetSystemMemoryUsedMB());
  }
  ...
  DEBUG_LOG("[MEM_CHECK] Before AllocDymAIPPIndexMem: RSS=%zuMB",
            GetSystemMemoryUsedMB());
  CHECK_RET_EQ(AllocDymAIPPIndexMem(), APP_ERR_OK);
  DEBUG_LOG("[MEM_CHECK] After AllocDymAIPPIndexMem: RSS=%zuMB",
            GetSystemMemoryUsedMB());

  CHECK_RET_EQ(GetModelDescInfo(), APP_ERR_OK);

  DEBUG_LOG("[MEM_CHECK] Before AllocDyIndexMem: RSS=%zuMB",
            GetSystemMemoryUsedMB());
  CHECK_RET_EQ(AllocDyIndexMem(), APP_ERR_OK);
  DEBUG_LOG("[MEM_CHECK] After AllocDyIndexMem: RSS=%zuMB",
            GetSystemMemoryUsedMB());
  ...
  DEBUG_LOG("[MEM_CHECK] ModelInferenceProcessor::Init end: RSS=%zuMB",
            GetSystemMemoryUsedMB());
  return APP_ERR_OK;
}
...
const std::vector<Base::TensorDesc>& ModelInferenceProcessor::GetInputs()
    const {
  return modelDesc_.inTensorsDesc;
}

const std::vector<Base::TensorDesc>& ModelInferenceProcessor::GetOutputs()
    const {
...
APP_ERROR ModelInferenceProcessor::DestroyOutMemoryData(
...
APP_ERROR ModelInferenceProcessor::CreateOutMemoryData(
    std::vector<MemoryData>& outputs) {
  size_t size;
  ...
  for (size_t i = 0; i < modelDesc_.outTensorsDesc.size(); ++i) {
    ...
    DEBUG_LOG("Create OutMemory i:%zu name:%s size:%zu", i,
              modelDesc_.outTensorsDesc[i].name.c_str(), size);
    Base::MemoryData memorydata(size, MemoryData::MemoryType::MEMORY_DEVICE,
                                deviceId_);
    ...
    if (ret != APP_ERR_OK) {
      ...
      ERROR_LOG("memory data malloc failed.i:%zu name:%s size:%zu ret:%d", i,
                modelDesc_.outTensorsDesc[i].name.c_str(), size, ret);
      return ret;
    }
...
APP_ERROR ModelInferenceProcessor::AddOutTensors(
  ...
  bool is_dymshape = (dynamicInfo_.dynamicType == DYNAMIC_SHAPE);
  size_t realLen;
  for (const auto& name : outputNames) {
    auto it = modelDesc_.outnames2Index.find(name);
    if (it == modelDesc_.outnames2Index.end()) {
    ...
    if (index >= modelDesc_.outTensorsDesc.size() || index >= outputs.size()) {
      ERROR_LOG(
          "index: %zu of output name: %s should be smaller than outputs size!",
          index, name.c_str());
      return APP_ERR_ACL_RT_FAILURE;
    }
    if (processModel->GetCurOutputShape(index, is_dymshape, i64shape) !=
        SUCCESS) {
      // 针对于动态shape场景 如果无法获取真实的输出shape 先填写一个一维的值
      // 以便后续内存可以导出
      auto tmpDataType =
          static_cast<aclDataType>(modelDesc_.outTensorsDesc[index].datatype);
      i64shape.push_back(realLen / aclDataTypeSize(tmpDataType));
    }
    DEBUG_LOG(
        "AddOutTensors name:%s index:%zu len:%zu outdescsize:%zu shapesize:%zu",
        name.c_str(), index, realLen, modelDesc_.outTensorsDesc[index].size,
        i64shape.size());
    ...
    TensorBase outputTensor(outputs[index], isBorrowed, u32shape,
                            modelDesc_.outTensorsDesc[index].datatype,
                            contextIndex_);
...
APP_ERROR ModelInferenceProcessor::CheckInVectorAndFillBaseTensor(
    const std::vector<BaseTensor>& feeds, std::vector<BaseTensor>& inputs) {
  ...
  for (size_t i = 0; i < feeds.size(); ++i) {
    ...
    if (baseTensor.size != modelDesc_.inTensorsDesc[i].realsize) {
      ERROR_LOG("check i:%zu name:%s in size:%zu needsize:%zu not match", i,
                modelDesc_.inTensorsDesc[i].name.c_str(), baseTensor.size,
                modelDesc_.inTensorsDesc[i].realsize);
      return APP_ERR_ACL_FAILURE;
    }
...
APP_ERROR ModelInferenceProcessor::Inference(
    const std::vector<BaseTensor>& feeds, std::vector<std::string>& outputNames,
    std::vector<TensorBase>& outputTensors) {
  APP_ERROR ret;
  // create basetensors
...
APP_ERROR ModelInferenceProcessor::CheckInMapAndFillBaseTensor(
    ...
    std::vector<BaseTensor>& inputs) {
  if (feeds.size() != modelDesc_.inTensorsDesc.size()) {
    ERROR_LOG("intensors size:%zu need size:%zu not match", feeds.size(),
              modelDesc_.inTensorsDesc.size());
    return APP_ERR_ACL_FAILURE;
  }
  ...
  for (size_t i = 0; i < modelDesc_.inTensorsDesc.size(); ++i) {
    ...
    if (feeds.end() == iter) {
      ERROR_LOG("intensors i:%zu name:%s not find", i,
                modelDesc_.inTensorsDesc[i].name.c_str());
      return APP_ERR_ACL_FAILURE;
    }
...
APP_ERROR ModelInferenceProcessor::CheckInVectorAndFillBaseTensor(
    const std::vector<TensorBase>& feeds, std::vector<BaseTensor>& inputs) {
  for (size_t i = 0; i < feeds.size(); ++i) {
    BaseTensor baseTensor = {};
...
APP_ERROR ModelInferenceProcessor::Inference(
    const std::vector<TensorBase>& feeds, std::vector<std::string> outputNames,
    std::vector<TensorBase>& outputTensors) {
  APP_ERROR ret;
  // create basetensors
  std::vector<BaseTensor> inputs;
...
APP_ERROR ModelInferenceProcessor::DestroyInferCacheData() {
  ...
  processModel->DestroyInput(MemoryPolicy::RETAIN_MEMORY);
  processModel->DestroyOutput(MemoryPolicy::RETAIN_MEMORY);
  return APP_ERR_OK;
}

APP_ERROR ModelInferenceProcessor::UpdateInputsData(
    const std::vector<int>& inOutRelation, const bool mem_copy) {
  Result result;
  if (mem_copy) {
...
APP_ERROR ModelInferenceProcessor::SetInputsData(
    std::vector<BaseTensor>& inputs) {
  APP_ERROR ret;
  ...
  if (inputs.size() != modelDesc_.inTensorsDesc.size()) {
    WARN_LOG("intensors in:%zu need:%zu not match", inputs.size(),
             modelDesc_.inTensorsDesc.size());
    return APP_ERR_ACL_FAILURE;
  }

  if (dynamicInfo_.dynamicType != DYNAMIC_DIMS && dym_gear_count_ > 0) {
    WARN_LOG("check failed dym gearcount:%zu but dymtype:%d not set",
             dym_gear_count_, dynamicInfo_.dynamicType);
    return APP_ERR_ACL_FAILURE;
  }

  // add dynamic index tensor
  if (dynamicIndex_ != DYNAMIC_INDEX_NONE) {
...
APP_ERROR ModelInferenceProcessor::SetAippConfigData() {
  if (dyAippCfg->IsActivated() && dyAippCfg->ModelIsLegal()) {
    ...
    for (auto& aippSetIt : dymAIPPIndexSet_) {
      Result result =
          processModel->SetInputAIPP(aippSetIt.first, aippSetIt.second);
      if (result != SUCCESS) {
        ERROR_LOG("set input AIPP failed. index:%d result:%d ",
                  int(aippSetIt.first), result);
        return APP_ERR_ACL_FAILURE;
      }
    }
...
APP_ERROR ModelInferenceProcessor::GetOutputs(
    ...
    std::vector<TensorBase>& outputTensors) {
  for (const auto& name : outputNames) {
    if (modelDesc_.outnames2Index.count(name) == 0) {
      ERROR_LOG("outnames %s not valid", name.c_str());
      return APP_ERR_ACL_FAILURE;
    }
  }
...
APP_ERROR ModelInferenceProcessor::FirstInferenceInner(
    std::vector<BaseTensor>& inputs, std::vector<std::string> outputNames,
    std::vector<TensorBase>& outputTensors) {
...
APP_ERROR ModelInferenceProcessor::Execute() {
  ...
  auto& zp = sumaryInfo_.zero_point;
  float start_time =
      std::chrono::duration<float, std::milli>(start - zp).count();
  float end_time = std::chrono::duration<float, std::milli>(end - zp).count();
...
APP_ERROR ModelInferenceProcessor::InitSumaryInfo() {
  sumaryInfo_.zero_point = std::chrono::steady_clock::now();
  return APP_ERR_OK;
}
...
const InferSummaryInfo& ModelInferenceProcessor::GetSumaryInfo() const {
  return sumaryInfo_;
}

InferSummaryInfo& ModelInferenceProcessor::GetMutableSumaryInfo() {
  return sumaryInfo_;
}
...
APP_ERROR ModelInferenceProcessor::AllocDymAIPPIndexMem() {
  ...
  for (const auto& index : dymAIPPIndexList_) {
    ...
    int datatype;
    CHECK_RET_EQ(
        processModel->GetInTensorDesc(index, info.name, datatype, info.format,
                                      info.shape, info.size),
        SUCCESS);
    MemoryData memdata;
...
APP_ERROR ModelInferenceProcessor::AllocDyIndexMem() {
  ...
  int datatype;
  CHECK_RET_EQ(
      processModel->GetInTensorDesc(dynamicIndex_, info.name, datatype,
                                    info.format, info.shape, info.size),
      SUCCESS);
...
APP_ERROR ModelInferenceProcessor::FreeDymInfoMem() {
  dynamicInfo_.dyDims.reset();
  dynamicInfo_.dyShape.reset();
  return APP_ERR_OK;
}
...
APP_ERROR ModelInferenceProcessor::SetDynamicBatchsize(int batchsize) {
  ...
  CHECK_RET_EQ(processModel->CheckDynamicBatchSize(batchsize, is_dymbatch),
               SUCCESS);
  CHECK_RET_EQ(processModel->GetMaxBatchSize(dynamicInfo_.dyBatch.maxbatchsize),
               SUCCESS);
  ...
  for (size_t i = 0; i < modelDesc_.inTensorsDesc.size(); ++i) {
    ...
    if (find(tensorBegin, tensorEnd, -1) != tensorEnd) {
      modelDesc_.inTensorsDesc[i].realsize = modelDesc_.inTensorsDesc[i].size *
                                             ...
                                             dynamicInfo_.dyBatch.maxbatchsize;
    }
  }
...
APP_ERROR ModelInferenceProcessor::SetSrcImageSize(
    std::vector<int> srcImageSize) {
...
APP_ERROR ModelInferenceProcessor::SetPaddingParams(
    std::vector<int> padParams) {
  CHECK_RET_EQ(dyAippCfg->SetPaddingParams(padParams), APP_ERR_OK);
  return APP_ERR_OK;
}

APP_ERROR ModelInferenceProcessor::SetDtcPixelMean(
    std::vector<int> meanParams) {
  CHECK_RET_EQ(dyAippCfg->SetDtcPixelMean(meanParams), APP_ERR_OK);
  return APP_ERR_OK;
}

APP_ERROR ModelInferenceProcessor::SetDtcPixelMin(
...
APP_ERROR ModelInferenceProcessor::SetDynamicHW(int width, int height) {
  bool is_dymHW;
  std::pair<uint64_t, uint64_t> dynamicHW = {width, height};

  FreeDymInfoMem();

  CHECK_RET_EQ(processModel->CheckDynamicHWSize(dynamicHW, is_dymHW), SUCCESS);
  CHECK_RET_EQ(processModel->GetMaxDynamicHWSize(dynamicInfo_.dyHW.maxHWSize),
               SUCCESS);
  ...
  for (size_t i = 0; i < modelDesc_.inTensorsDesc.size(); ++i) {
    ...
    if (find(tensorBegin, tensorEnd, -1) != tensorEnd) {
      modelDesc_.inTensorsDesc[i].realsize =
          modelDesc_.inTensorsDesc[i].size * static_cast<size_t>(width) *
          static_cast<size_t>(height) / dynamicInfo_.dyHW.maxHWSize;
    }
  }
...
APP_ERROR ModelInferenceProcessor::SetDynamicDims(std::string dymdimsStr) {
  ...
  FreeDymInfoMem();
  if (dynamicInfo_.dyDims == nullptr) {
    dynamicInfo_.dyDims = std::make_unique<DyDimsInfo>();
  }
  ...
  Utils::SplitStringSimple(dymdimsStr, dynamicInfo_.dyDims->dym_dims, ';', ':',
                           ',');

  if (dym_gear_count_ == 0) {
    PROMPT_MSG(
        "the dynamic_dims parameter is not specified for model conversion");
    delete[] dims;
    return APP_ERR_ACL_FAILURE;
  }

  Result ret = processModel->CheckDynamicDims(dynamicInfo_.dyDims->dym_dims,
                                              dym_gear_count_, dims);
  if (ret != SUCCESS) {
    ERROR_LOG(
        "check dynamic dims failed, please set correct dymDims paramenter");
    delete[] dims;
    return APP_ERR_ACL_FAILURE;
  }
  ...
  ret = Utils::SplitStringGetNameDimsMulMap(dymdims_tmp, namedimsmap);
  if (ret != SUCCESS) {
    ERROR_LOG("split dims str failed");
    delete[] dims;
    return APP_ERR_ACL_FAILURE;
  }
  for (const auto& kv : namedimsmap) {
    auto it = modelDesc_.innames2Index.find(kv.first);
    if (it == modelDesc_.innames2Index.end()) {
      WARN_LOG("find no in names:%s", kv.first.c_str());
      continue;
    }
    size_t inindex = it->second;
    auto tmpDataType =
        static_cast<aclDataType>(modelDesc_.inTensorsDesc[inindex].datatype);
    modelDesc_.inTensorsDesc[inindex].realsize =
        static_cast<size_t>(kv.second) * aclDataTypeSize(tmpDataType);
  }
...
APP_ERROR ModelInferenceProcessor::SetDynamicShape(std::string dymshapeStr) {
  ...
  FreeDymInfoMem();
  if (dynamicInfo_.dyShape == nullptr) {
    dynamicInfo_.dyShape = std::make_unique<DyShapeInfo>();
  }

  std::map<std::string, std::vector<int64_t>> name2shapesmap;
  Result ret = processModel->CheckDynamicShape(dym_shape_tmp, name2shapesmap,
                                               dynamicInfo_.dyShape->dims_num);
  if (ret != SUCCESS) {
    ERROR_LOG("check dynamic shape failed");
    return APP_ERR_ACL_FAILURE;
  }

  dynamicInfo_.dyShape->dym_shape_map = name2shapesmap;

  // update realsize according real shapes
  std::map<std::string, int64_t> namedimsmap;
  ret = Utils::SplitStringGetNameDimsMulMap(dym_shape_tmp, namedimsmap);
  if (ret != SUCCESS) {
    ERROR_LOG("split dims str failed");
    return APP_ERR_ACL_FAILURE;
  }
...
APP_ERROR ModelInferenceProcessor::SetCustomOutTensorsSize(
...
APP_ERROR ModelInferenceProcessor::SetDymAIPPInfoSet() {
  ...
  for (auto& aippSetIt : dymAIPPIndexSet_) {
    Result ret =
        processModel->GetDymAIPPConfigSet(dyAippCfg, aippSetIt.second, MaxBS);
    DEBUG_LOG("AIPP config set index:%d\n", int(aippSetIt.first));
    if (ret != SUCCESS) {
      ERROR_LOG("set dynamic AIPP config failed. index: %d ret %d",
                int(aippSetIt.first), ret);
...
APP_ERROR ModelInferenceProcessor::SetDynamicInfo() {
  std::pair<uint64_t, uint64_t> dynamicHW;
  switch (dynamicInfo_.dynamicType) {
    case DYNAMIC_BATCH:
      CHECK_RET_EQ(
          processModel->SetDynamicBatchSize(dynamicInfo_.dyBatch.batchSize),
          SUCCESS);
      break;
    case DYNAMIC_HW:
      dynamicHW = {dynamicInfo_.dyHW.imageSize.width,
                   dynamicInfo_.dyHW.imageSize.height};
    ...
    case DYNAMIC_DIMS:
      if (dynamicInfo_.dyDims == nullptr) {
        ...
        CHECK_RET_EQ(
            processModel->SetDynamicDims(dynamicInfo_.dyDims->dym_dims),
            SUCCESS);
      }
      break;
    case DYNAMIC_SHAPE:
      if (dynamicInfo_.dyShape == nullptr) {
        ...
        CHECK_RET_EQ(
            processModel->SetDynamicShape(dynamicInfo_.dyShape->dym_shape_map,
                                          dynamicInfo_.dyShape->dims_num),
            SUCCESS);
      }
      break;
