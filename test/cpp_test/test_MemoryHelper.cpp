  ...
  void SetGlobalDefaultExpectations() {
    EXPECT_CALL(*mockAcl, aclrtMallocHost(_, _))
        .WillRepeatedly(Invoke([](void** ptr, size_t size) {
          *ptr = malloc(size);
          return (*ptr == nullptr && size > 0) ? APP_ERR_ACL_BAD_ALLOC
                                               : APP_ERR_OK;
        }));
    ...
    EXPECT_CALL(*mockAcl, aclrtFree(_)).WillRepeatedly(Invoke([](void* ptr) {
      free(ptr);
      return APP_ERR_OK;
    }));
    ...
    EXPECT_CALL(*mockAcl, acldvppFree(_)).WillRepeatedly(Invoke([](void* ptr) {
      free(ptr);
      return APP_ERR_OK;
    }));
    ...
    EXPECT_CALL(*mockAcl, aclrtMemset(_, _, _, _))
        .WillRepeatedly(
            Invoke([](void* ptr, size_t, int32_t value, size_t count) {
    ...
    EXPECT_CALL(*mockAcl, aclrtMemcpy(_, _, _, _, _))
        .WillRepeatedly(Invoke(
            [](void* dst, size_t dstMax, const void* src, size_t count, int) {
...
TEST_F(MemoryHelperTest, MxbsMallocAndCopySrcNull) {
  ...
  src.ptrData = nullptr;
  MemoryData dst(8, MemoryData::MEMORY_HOST_MALLOC);
  EXPECT_EQ(MemoryHelper::MxbsMallocAndCopy(dst, src),
