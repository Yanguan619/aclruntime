...
APP_ERROR ModelInferenceProcessor::Init(const std::string& modelPath,
    ...
    if (modelSize > 0 && modelData != nullptr) {
        ...
        CHECK_RET_EQ(processModel->LoadModelFromMem(modelData, modelSize),
                     SUCCESS);
    } else {
        ...
        CHECK_RET_EQ(processModel->LoadModelFromFile(
                         modelPath, options->weightDir, options),
                     SUCCESS);
    }
...
APP_ERROR ModelInferenceProcessor::AddOutTensors(
    ...
    for (const auto& name : outputNames) {
        ...
        if (index >= modelDesc_.outTensorsDesc.size() ||
            index >= outputs.size()) {
            ERROR_LOG(
                "index: %zu of output name: %s should be smaller than outputs "
                "size!",
                index, name.c_str());
        ...
        if (processModel->GetCurOutputShape(index, is_dymshape, i64shape) !=
            SUCCESS) {
            // 针对于动态shape场景 如果无法获取真实的输出shape
            // 先填写一个一维的值 以便后续内存可以导出
            auto tmpDataType = static_cast<aclDataType>(
                modelDesc_.outTensorsDesc[index].datatype);
            i64shape.push_back(realLen / aclDataTypeSize(tmpDataType));
        }
        DEBUG_LOG(
            "AddOutTensors name:%s index:%zu len:%zu outdescsize:%zu "
            "shapesize:%zu",
    ...
    for (size_t i = 0; i < feeds.size(); ++i) {
        ...
        if (baseTensor.size != modelDesc_.inTensorsDesc[i].realsize) {
            ERROR_LOG("check i:%zu name:%s in size:%zu needsize:%zu not match",
                      ...
                      baseTensor.size, modelDesc_.inTensorsDesc[i].realsize);
            return APP_ERR_ACL_FAILURE;
        }
...
APP_ERROR ModelInferenceProcessor::GetOutputs(
    ...
    APP_ERROR ret =
        AddOutTensors(outputsMemDataQue_, outputNames, outputTensors);
    if (ret != APP_ERR_OK) {
...
APP_ERROR ModelInferenceProcessor::AllocDymAIPPIndexMem() {
    ...
    for (const auto& index : dymAIPPIndexList_) {
        ...
        int datatype;
        CHECK_RET_EQ(
            processModel->GetInTensorDesc(index, info.name, datatype,
...
APP_ERROR ModelInferenceProcessor::SetDynamicBatchsize(int batchsize) {
    ...
    CHECK_RET_EQ(processModel->CheckDynamicBatchSize(batchsize, is_dymbatch),
                 SUCCESS);
    CHECK_RET_EQ(
        processModel->GetMaxBatchSize(dynamicInfo_.dyBatch.maxbatchsize),
        SUCCESS);
    ...
    for (size_t i = 0; i < modelDesc_.inTensorsDesc.size(); ++i) {
        ...
        if (find(tensorBegin, tensorEnd, -1) != tensorEnd) {
            modelDesc_.inTensorsDesc[i].realsize =
                modelDesc_.inTensorsDesc[i].size *
...
APP_ERROR ModelInferenceProcessor::SetDynamicHW(int width, int height) {
    ...
    CHECK_RET_EQ(processModel->CheckDynamicHWSize(dynamicHW, is_dymHW),
                 SUCCESS);
...
APP_ERROR ModelInferenceProcessor::SetDynamicDims(std::string dymdimsStr) {
    ...
    for (const auto& kv : namedimsmap) {
        ...
        auto tmpDataType = static_cast<aclDataType>(
            modelDesc_.inTensorsDesc[inindex].datatype);
...
APP_ERROR ModelInferenceProcessor::SetDynamicShape(std::string dymshapeStr) {
    ...
    Result ret = processModel->CheckDynamicShape(
        dym_shape_tmp, name2shapesmap, dynamicInfo_.dyShape->dims_num);
    if (ret != SUCCESS) {
...
APP_ERROR ModelInferenceProcessor::SetDymAIPPInfoSet() {
    ...
    for (auto& aippSetIt : dymAIPPIndexSet_) {
        Result ret = processModel->GetDymAIPPConfigSet(dyAippCfg,
                                                       aippSetIt.second, MaxBS);
...
APP_ERROR ModelInferenceProcessor::SetDynamicInfo() {
    ...
    switch (dynamicInfo_.dynamicType) {
        case DYNAMIC_BATCH:
            CHECK_RET_EQ(processModel->SetDynamicBatchSize(
                             dynamicInfo_.dyBatch.batchSize),
                         SUCCESS);
            break;
        case DYNAMIC_HW:
        ...
        case DYNAMIC_SHAPE:
            if (dynamicInfo_.dyShape == nullptr) {
                ...
                CHECK_RET_EQ(processModel->SetDynamicShape(
                                 dynamicInfo_.dyShape->dym_shape_map,
