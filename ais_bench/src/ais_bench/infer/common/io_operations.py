...
def get_narray_from_files_list(files_list, size, pure_data_type, no_combine_tensor_mode=False):
    ...
    for i, file_path in enumerate(files_list):
        logger.debug(
            "get tensor from filepath:{} i:{} of all:{}".format(file_path, i, len(files_list))
        )
...
def get_tensor_from_files_list(
    files_list, session, size, pure_data_type, no_combine_tensor_mode=False
):
...
# Obtain filesperbatch runcount information according to file information and input description information  # noqa: E501
# The strategy is as follows:  Judge according to the realsize and file size of input 0.  # noqa: E501
...
# Obtain tensor information and files information according to the input filelist.  # noqa: E501
...
def create_infileslist_from_fileslist(fileslist, intensors_desc, no_combine_tensor_mode=False):
    if len(intensors_desc) != len(fileslist):
        logger.error(
            "fileslist:{} intensor:{} not match".format(len(fileslist), len(intensors_desc))
        )
        raise RuntimeError()
    ...
    if os.path.isfile(inputs_list[0]):
        ...
        for dir_path in inputs_list:
            ...
            if not os.path.isdir(real_dir_path):
                logger.error(
                    "Invalid args. {} of input args is not a real dir path".format(real_dir_path)
                )
                raise RuntimeError()
    else:
...
def _parse_inputs_to_fileslist(inputs_list, intensors_desc):
    ...
    if os.path.isfile(inputs_list[0]):
        ...
        logger.debug(
            "create intensors list file type inlistcount:{} intensorcont:{} "
            "chunks:{} files_size:{}".format(inputlistcount, intensorcount, chunks, len(fileslist))
        )
        ...
        logger.debug(
            "create intensors list dictionary type inlistcount:{} "
            "intensorcont:{} files_size:{}".format(inputlistcount, intensorcount, len(fileslist))
        )
    else:
        logger.error(
            "create intensors list filelists:{} intensorcont:{} error create".format(
...
def create_infileslist_from_inputs_list(inputs_list, intensors_desc, no_combine_tensor_mode=False):
    fileslist = _parse_inputs_to_fileslist(inputs_list, intensors_desc)
    infileslist = create_infileslist_from_fileslist(
        fileslist, intensors_desc, no_combine_tensor_mode
    )
    if len(infileslist) == 0:
        logger.error(
            "create input fileslist from fileslist, input fileslist size: {}".format(
                len(infileslist)
            )
        )
        raise RuntimeError()
...
def create_pipeline_fileslist_from_inputs_list(inputs_list, intensors_desc):
    ...
    return list(zip(*fileslist))


def save_tensors_to_file(
    infiles_paths, outfmt, index, output_batchsize_axis, out_file_info: OutFileInfo
):
    ...
    for i, out in enumerate(out_file_info.outputs):
        ...
        if output_batchsize_axis >= len(ndata.shape):
            logger.error(
                "error i:{0} ndata.shape:{1} len:{2} <= "
                "output_batchsize_axis:{3}  is invalid".format(
                    i, ndata.shape, len(ndata.shape), output_batchsize_axis
                )
            )
            raise RuntimeError()
        if (
            files_count_perbatch == 1
            or ndata.shape[output_batchsize_axis] % files_count_perbatch == 0
        ):
            ...
            logger.error(
                "save out files error array shape:{} "
                "filesinfo:{} files_count_perbatch:{} "
                "ndata.shape {}:{}".format(
                    ndata.shape,
                    infiles_paths,
