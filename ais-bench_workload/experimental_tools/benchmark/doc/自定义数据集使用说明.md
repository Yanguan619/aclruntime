...
在 `csv` 和 `jsonl` 两种数据集文件格式下，均支持将最大输出长度的颗粒度设置到`每条请求`程度，通过在文件的每条对象中（`jsonl`格式文件）或 每行（`csv`格式文件）添加 `max_tokens` 字段以及对应的值。该字段暂时不支持性能压测场景。
...
- 数据集样例及对应性能测评结果示例图：
  - 数据集样例：
    ![custom_dataset_example.img](../img/自定义数据集/custom_dataset_example.png)

  - 性能测评结果示例图：
    ![custom_results_example.img](../img/自定义数据集/custom_results_example.png)
