...
# this file is as same as components/utils/file_opem_check.py,  # noqa: E501
# because benchmark might be install without ait
...
MAX_SIZE_LIMITED_MODEL_FILE = (
    100 * 1024 * 1024 * 1024
)  # 100G 超大模型文件，需要确定能处理大文件，可以根据实际要求变更
...
SOLUTION_BASE_LOC = (
    '"gitee repo: Ascend/tools, path:tools/ais_bench-workload/tool/ais_bench/README.md, '
    + "chapter:"
)
...
def solution_log(content):
    logger.log(
        SOLUTION_LEVEL, f"visit \033[1;32m {content} \033[0m for detailed solution"
    )  # green content
...
def is_legal_path_length(path):
    if len(path) > MAX_LINUX_ABS_PATH_LENGTH and not is_platform(
        "win"
    ):  # linux total path length limit
        logger.error(
            f"file total path's length out of range ({MAX_LINUX_ABS_PATH_LENGTH}), "
            + "please check the file(or directory) path"
        )
        solution_log(SOLUTION_BASE_LOC + PATH_LENGTH_SUB_CHAPTER)
        return False

    if len(path) > MAX_WIN_ABS_PATH_LENGTH and is_platform(
        "win"
    ):  # windows total path length limit
        logger.error(
            f"file total path's length out of range ({MAX_WIN_ABS_PATH_LENGTH}), "
            + "please check the file(or directory) path"
        )
    ...
    for dirname in dirnames:
        if len(dirname) > MAX_LINUX_BASE_NAME_LENGTH:  # linux single file path length limit
            logger.error(
                f"file base name length out of range ({MAX_LINUX_BASE_NAME_LENGTH}), "
                + "please check the file(or directory) path"
            )
            solution_log(SOLUTION_BASE_LOC + PATH_LENGTH_SUB_CHAPTER)
            return False
...
def is_match_path_white_list(path):
    if PATH_WHITE_LIST_REGEX.search(path) and not is_platform("win"):
        logger.error("path contains illegal char, legal chars include A-Z a-z 0-9 _ - / .")
...
class FileStat:
    def __init__(self, file) -> None:
        ...
        if not is_legal_path_length(self.file) or not is_match_path_white_list(self.file):
            raise OpenException("create FileStat failed")
    ...
    def check_linux_permission(self, perm=FilePermChoice.NONE):
        ...
        if self.is_softlink:
            logger.error(
                f"path :{self.file} is a soft link, not supported, "
...
def ms_open(
    ...
    if "r" in mode:
        ...
        if max_size != MAX_SIZE_UNLIMITED and max_size < file_stat.file_size:
            raise OpenException(
                f"The file size has exceeded the specifications and cannot be read. {file}"
            )

    if "w" in mode:
        if file_stat.is_exists and not file_stat.is_owner:
            raise OpenException(
                f"The file owner is inconsistent with the current process user "
                f"and is not allowed to write. {file}"
            )
    ...
    return os.fdopen(
        os.open(file, flags, mode=write_permission), mode, **kwargs
    )  # ms_open函数中，file在之前已经完成校验了
...
def check_path_legality(
    path,
    perm=FilePermChoice.WRITE,
    max_size=MAX_SIZE_LIMITED_CONFIG_FILE,
    is_file=True,
    suffix=None,
):
    try:
        ...
        raise ValueError("The path string is illegal. Please check.") from err
    if is_file != file_stat.is_file:
        if is_file:
            raise ValueError(
                f"The path:{path} is not a file."
            )  # check path string content when init FileStat
        else:
            raise ValueError(
