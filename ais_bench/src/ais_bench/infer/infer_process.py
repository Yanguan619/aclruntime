...
from ais_bench.infer.args_check import DYM_INFO_PATTERN, check_dym_str_format
...
def set_session_options(session, args):
    ...
    if not args.auto_set_dymshape_mode and not args.auto_set_dymdims_mode:
        if args.batchsize < 0:
            if not args.dym_batch and not args.dym_dims and not args.dym_shape:
                raise RuntimeError(
                    "dynamic batch om model detected, but dymbatch, dymdims or dymshape not set!"
                )
    ...
    if (args.aipp_config is not None) and (aipp_input_exist == 1):
        ...
        logger.error(
            f"don't support more than one dynamic aipp input in model, \
...
def set_dymshape_shape(session, inputs):
    ...
    if len(inputs) != len(intensors_desc):
        raise ValueError(
            f"input datas count{inputs} is not equal to model input tensors count{intensors_desc}"
        )
...
def warmup(session, args, intensors_desc, infiles):
    ...
    if len(infiles) != len(intensors_desc):
        raise ValueError(
            f"input files count{infiles} is not equal to model input tensors count{intensors_desc}"
        )
    for j, files in enumerate(infiles):
        if args.run_mode == "tensor":
            ...
            narray = get_narray_from_files_list(
                files, intensors_desc[j].realsize, args.pure_data_type, args.no_combine_tensor_mode
            )
            infeeds.append(narray)
    session.set_loop_count(1)
    # warmup
    for _ in range(args.warmup_count):
        run_inference(session, args, infeeds, out_array=True)
...
def infer_loop_tensor_run(session, args, intensors_desc, infileslist, output_prefix):
    for i, infiles in enumerate(
        tqdm(infileslist, file=sys.stdout, desc="Inference tensor Processing")
    ):
        intensors = []
        if len(infiles) != len(intensors_desc):
            raise ValueError(
                f"input files count{infiles} is not equal to "
                f"model input tensors count{intensors_desc}"
            )
        for j, files in enumerate(infiles):
            tensor = get_tensor_from_files_list(
                files,
...
def infer_loop_files_run(session, args, intensors_desc, infileslist, output_prefix):
    for i, infiles in enumerate(
        tqdm(infileslist, file=sys.stdout, desc="Inference files Processing")
    ):
        intensors = []
        if len(infiles) != len(intensors_desc):
            raise ValueError(
...
def infer_fulltensors_run(session, args, intensors_desc, infileslist, output_prefix):
    ...
    intensorslist = create_intensors_from_infileslist(
        infileslist, intensors_desc, session, args.pure_data_type, args.no_combine_tensor_mode
    )
    ...
    if len(infileslist) != len(outtensors):
        raise ValueError(
            f"input files count{infileslist} is not equal to out tensors count{outtensors}"
        )
...
def infer_loop_array_run(session, args, intensors_desc, infileslist, output_prefix):
    for i, infiles in enumerate(
        tqdm(infileslist, file=sys.stdout, desc="Inference array Processing")
    ):
        innarrays = []
        for j, files in enumerate(infiles):
            narray = get_narray_from_files_list(
                files, intensors_desc[j].realsize, args.pure_data_type
            )
    ...
    for option_cmd in ACL_JSON_CMD_LIST:
        ...
        if opt_value:
            if option_cmd == "output" and not is_legal_args_path_string(opt_value):
                raise Exception("output path in acl_json is illegal!")
...
def msprof_run_profiling(args, msprof_bin):
    if args.acl_json_path is not None:
        ...
        cmd = regenerate_cmd(args)
        msprof_cmd = (
            f'{msprof_bin} --output={args.output}/profiler --application="{cmd}" '
            f"--model-execution=on --sys-hardware-mem=on --sys-cpu-profiling=off "
            f"--sys-profiling=off --sys-pid-profiling=off "
            f"--dvpp-profiling=on --runtime-api=on --task-time=on --aicpu=on"
        )
...
def convert(tmp_acl_json_path, real_dump_path, tmp_dump_path):
    ...
    if tmp_dump_path is not None:
        check_path_legality(
            tmp_dump_path, FilePermChoice.WRITE, is_file=False
        )  # if not exist, won't except
        try:
    ...
    if tmp_acl_json_path is not None:
        check_path_legality(
            tmp_acl_json_path, FilePermChoice.WRITE, suffix=["json"]
        )  # if not exist, won't except
        try:
...
def main(args, index=0, msgq=None, device_list=None):
    ...
    session = init_inference_session(
        args, tmp_acl_json_path if tmp_acl_json_path is not None else acl_json_path
    )
        ...
        if args.output is not None:
            if args.output_dirname is None:
                ...
                output_prefix = os.path.join(
                    output_prefix, "device" + str(device_list[index]) + "_" + str(index)
                )
            else:
    ...
    if len(inputs_list) == 0:
        ...
        if not args.pipeline:
            ...
            pure_file = (
                ...
                else PURE_INFER_FAKE_FILE_RANDOM
            )
            for _ in intensors_desc:
                infileslist[0].append(pure_file)
    else:
        if not args.pipeline:  # check before np.load, not here
            infileslist = create_infileslist_from_inputs_list(
                inputs_list, intensors_desc, args.no_combine_tensor_mode
            )
            ...
            for files in infileslist:
                for file in files:
                    check_path_legality(
                        file, perm=FilePermChoice.READ, max_size=MAX_SIZE_LIMITED_NORMAL_FILE
                    )
    ...
    if args.pipeline:
        ...
        if run_mode_switch.get(args.run_mode) is not None:
            run_mode_switch.get(args.run_mode)(
                session, args, intensors_desc, infileslist, output_prefix
            )
        else:
    ...
    if multi_threads_mode:
        ...
        summary.npu_compute_time_list = [
            end_time - start_time for start_time, end_time in s.exec_time_list
        ]
    ...
    try:
        if args.energy_consumption:
            energy_consumption = (
                (float(end_energy_consumption) + float(start_energy_consumption)) / 2.0
            ) * (end_time - start_time)
            logger.info(f"NPU ID:{args.npu_id} energy consumption(J):{energy_consumption}")
    except AttributeError as err:
        logger.error(f"Attribute Access Error: {err}")
        raise RuntimeError(
            "Error accessing an attribute, please verify if the NPU ID is correct. "
        ) from err
    except Exception as err:
        logger.error(f"Unexpected Error: {err}")
        raise RuntimeError(
            "Energy consumption append an unexpected error occurred, "
...
def multidevice_run(args):
    ...
    for i, device in enumerate(device_list):
        ...
        p.apply_async(
            main, args=(cur_args, i, msgq, device_list), error_callback=print_subproces_run_error
        )
    ...
    while msgq.qsize() != 0:
        ...
        if isinstance(ret, list):
            logger.info(
                f"i:{ret[0]} device_{device_list[ret[0]]} throughput:{ret[1]} \
...
def args_rules(args):
    if args.profiler and args.dump:
        logger.error(
            "parameter --profiler cannot be true at the same time "
            "as parameter --dump, please check them!\n"
        )
    ...
    if args.output is None and args.output_dirname is not None:
        logger.error(
            "parameter --output_dirname cann't be used alone. "
...
def infer_process(args: AISBenchInferArgsAdapter):
    ...
    if args.profiler:
        ...
        if msprof_bin is None:
            ...
            logger.info(
                "find MSIT_NO_MSPROF_MODE set, continue use acl.json mode, "
                "result won't be parsed as csv"
            )
        else:
