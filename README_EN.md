...
1. **Install the aclruntime package.**
   ...
   ```bash
   pip3 install -v 'git+https://gitee.com/ascend/tools.git#egg=aclruntime&subdirectory=ais-bench_workload/tool/ais_bench/aclruntime'
   ```

   Note: If overwrite installation is adopted, add the **--force-reinstall** option to enable forcible installation, for example:

   ```bash
   pip3 install -v --force-reinstall 'git+https://gitee.com/ascend/tools.git#egg=aclruntime&subdirectory=ais-bench_workload/tool/ais_bench/aclruntime'
   ```
...
1. Clone or download tool package **tools-xxx.zip** from the open-source code repository at [Gitee](https://gitee.com/ascend/tools).
...
+ Profiler is a group of profile data collection configurations fixed in the program. The generated profile data is saved in the **profiler** folder under the directory specified by the **--output** option.
  ...
  - If **GE_PROFILING_TO_STD_OUT** is set to **1** in the environment, the **acl.json** configuration file is used by the **--profiler** option to collect profile data. If the collected profile data file is not automatically parsed, you need to parse it according to [Parsing the Profiling File](https://gitee.com/ascend/tools/blob/master/ada/doc/ada_pa.md).
...
For details about the code sample, see [https://gitee.com/ascend/tools/blob/master/ais-bench_workload/tool/ais_bench/test/interface_sample.py](https://gitee.com/ascend/tools/blob/master/ais-bench_workload/tool/ais_bench/test/interface_sample.py).
...
```
To convert an abnormal BIN file to an NPY file, use the conversion script **[convert_exception_cb_bin_to_npy.py](https://gitee.com/ascend/tools/tree/master/ais-bench_workload/tool/ais_bench/test/convert_exception_cb_bin_to_npy.py)**.
