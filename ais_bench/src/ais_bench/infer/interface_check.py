...
from ais_bench.infer.common.path_security_check import FilePermChoice, check_path_legality
...
def check_device_range_valid(value):
    ...
    if value < min_value or value > max_value:
        raise ValueError(
            f"device:{value} is illegal. legal value range is [{min_value}, {max_value}]"
        )
...
def check_in_out_list(in_out_list, inputs, outputs):
    if len(in_out_list) != len(inputs):
        raise RuntimeError("inputs' amount and length of in_out_list not matched!")
    for _, reused_index in enumerate(in_out_list):
        ...
        if reused_index < -1 or reused_index >= len(outputs):
            raise IndexError(
                f"in_out_list[{in_out_list}] out of range, length range is (-1, {len(outputs)})"
            )
...
def check_custom_size(value, mode="dymshape"):
    if mode not in ["static", "dymbatch", "dymhw", "dymdims", "dymshape"]:
        raise ValueError("infer mode is illegal, Please check.")
    ...
    if isinstance(value, list):
        ...
        for data in value:
            if data <= 0 or data > CUSTOME_SIZE_MAX_SIZE:
                raise ValueError(
                    f"value:{value} in custom size list is out of range. Please check."
                )
