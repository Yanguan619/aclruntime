...
uint8_t Utils::CreateRandomNum() {
    ...
    uint8_t randomByte;
    ssize_t rc = read(fd, &randomByte, sizeof(randomByte));
    if (rc != sizeof(randomByte)) {
        close(fd);
...
std::string Utils::printCurrentTime() {
    ...
    std::string pi =
        std::to_string(p->tm_year + 1900) + std::to_string(p->tm_mon + 1) +
        std::to_string(p->tm_mday) + "_" + std::to_string(p->tm_hour) + "_" +
        std::to_string(p->tm_min) + "_" + std::to_string(p->tm_sec) + "_" +
...
void Utils::SplitStringSimple(string str, vector<string>& out, char split1,
...
void Utils::SplitStringWithSemicolonsAndColons(string str, vector<string>& out,
...
void Utils::SplitStringWithPunctuation(string str, vector<string>& out,
                                       char split) {
    istringstream block(str);
    string cell;
...
Result Utils::SplitStringGetNameDimsMulMap(
    ...
    for (size_t i = 0; i < in_dym_shape_str.size(); ++i) {
        ...
        if (pos == in_dym_shape_str[i].npos) {
            ERROR_LOG("find no : split i:%zu str:%s", i,
                      in_dym_shape_str[i].c_str());
            return FAILED;
        }
        name = in_dym_shape_str[i].substr(0, pos);
        shape_str = in_dym_shape_str[i].substr(
            pos + 1, in_dym_shape_str[i].size() - pos - 1);
...
Result Utils::ReadBinFileToMemory(const std::string fileName, char* ptr,
    ...
    if (offset + binFileBufferLen > size) {
        ERROR_LOG("offset:%zu filesize:%zu > size:%zu invalid", offset,
                  binFileBufferLen, size);
        binFile.close();
        return FAILED;
    }
    ...
    DEBUG_LOG("Readbin file:%s offset:%zu len:%zu", fileName.c_str(), offset,
              binFileBufferLen);
...
Result Utils::FillFileContentToMemory(const std::string file, char* ptr,
                                      const size_t size, size_t& offset) {
...
std::string Utils::MergeStr(std::vector<std::string>& list,
...
std::string Utils::GetPrefix(const std::string& outputDir, std::string filePath,
    ...
    if (fileName.size() >= removeTail.size() &&
        fileName.compare(fileName.size() - removeTail.size(), removeTail.size(),
                         removeTail) == 0) {
...
std::string Utils::CreateDynamicShapeDims(const std::string& name,
...
Result Utils::TensorToNumpy(const std::string& outputFileName,
    ...
    if (output.GetDataType() == Base::TENSOR_DTYPE_FLOAT32) {
        cnpy::NpySave(outputFileName, static_cast<float*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_FLOAT16) {
        cnpy::NpySave(outputFileName, static_cast<aclFloat16*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_INT8) {
        cnpy::NpySave(outputFileName, static_cast<int8_t*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_INT32) {
        cnpy::NpySave(outputFileName, static_cast<int32_t*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_UINT8) {
        cnpy::NpySave(outputFileName, static_cast<uint8_t*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_INT16) {
        cnpy::NpySave(outputFileName, static_cast<int16_t*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_UINT16) {
        cnpy::NpySave(outputFileName, static_cast<uint16_t*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_UINT32) {
        cnpy::NpySave(outputFileName, static_cast<uint32_t*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_INT64) {
        cnpy::NpySave(outputFileName, static_cast<int64_t*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_UINT64) {
        cnpy::NpySave(outputFileName, static_cast<uint64_t*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_DOUBLE64) {
        cnpy::NpySave(outputFileName, static_cast<double*>(output.GetBuffer()), shape);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_BOOL) {
        cnpy::NpySave(outputFileName, static_cast<bool*>(output.GetBuffer()), shape);
    } else {
...
Result Utils::TensorToBin(const std::string& outputFileName,
                          Base::TensorBase& output) {
    if (access(outputFileName.c_str(), F_OK) == 0 &&
        remove(outputFileName.c_str()) != 0) {
        ERROR_LOG("Tensor to bin: existing file %s cannot be removed",
    ...
    outfile.write(reinterpret_cast<const char*>(output.GetBuffer()),
                  output.GetByteSize());
    outfile.close();
...
template <typename T>
static void SaveTxt(std::ofstream& outFile, const T* p, size_t size,
                    size_t rowCount) {
    if (p == nullptr) {
...
Result Utils::TensorToTxt(const std::string& outputFileName,
                          Base::TensorBase& output) {
    if (access(outputFileName.c_str(), F_OK) == 0 &&
        remove(outputFileName.c_str()) != 0) {
        ERROR_LOG("Tensor to txt: existing file %s cannot be removed",
                  outputFileName.c_str());
        return FAILED;
    }
    std::ofstream outFile;
    ...
    if (output.GetDataType() == Base::TENSOR_DTYPE_FLOAT32) {
        SaveTxt(outFile, static_cast<float*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_FLOAT16) {
        SaveTxt(outFile, static_cast<aclFloat16*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_INT8) {
        SaveTxt(outFile, static_cast<int8_t*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_INT32) {
        SaveTxt(outFile, static_cast<int32_t*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_UINT8) {
        SaveTxt(outFile, static_cast<uint8_t*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_INT16) {
        SaveTxt(outFile, static_cast<int16_t*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_UINT16) {
        SaveTxt(outFile, static_cast<uint16_t*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_UINT32) {
        SaveTxt(outFile, static_cast<uint32_t*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_INT64) {
        SaveTxt(outFile, static_cast<int64_t*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_UINT64) {
        SaveTxt(outFile, static_cast<uint64_t*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_DOUBLE64) {
        SaveTxt(outFile, static_cast<double*>(output.GetBuffer()), size, rowCount);
    } else if (output.GetDataType() == Base::TENSOR_DTYPE_BOOL) {
        SaveTxt(outFile, static_cast<bool*>(output.GetBuffer()), size, rowCount);
    } else {
        outFile.close();
...
bool Utils::TailContain(const std::string& str, const std::string& tail) {
    if (str.length() >= tail.length() &&
        str.compare(str.length() - tail.length(), tail.length(), tail) == 0) {
        return true;
    }
    return false;
}
...
bool Utils::IsValidInteger(const std::string& str) {
    ...
    // Check for empty string, characters left after conversion, and out of
    // range values
...
bool Utils::IsDymShapeValid(const std::string& str) {
    ...
    if (shapeValues.size() == 0 || shapeValues.size() > MAX_SHAPE_VALUES)
        return false;
    for (std::string value : shapeValues) {
        if (value.empty() || value.size() > MAX_SHAPE_VALUE_LENGTH)
            return false;
...
bool Utils::IsInputNameValidChar(const std::string& str) {
    ...
    for (size_t i = 0; i < str.size(); ++i) {
        auto it =
            std::find(extendedPattern.begin(), extendedPattern.end(), str[i]);
...
bool Utils::IsLegalDymString(const std::string& str) {
    ...
    if (inputInfoList.size() > INPUT_LIST_MAX_SIZE) {
        ERROR_LOG(
            "the count of input list parsed from dymshape string is over %zu",
            INPUT_LIST_MAX_SIZE);
        return false;
    }
    ...
    for (std::string infoStr : inputInfoList) {
        ...
        if (inputInfo.size() != EXPECTED_INPUT_INFO_SIZE) {
            ERROR_LOG(
                "the format of input info parsed from dymshape string is "
                "wrong!");
            return false;
        }
        ...
        if (inputName.length() == 0 ||
            inputName.length() > INPUT_NAME_LENGTH_MAX) {
            ERROR_LOG(
                "the length of input name parsed from dymshape string is "
                "output of "
                "[1, %zu]",
                INPUT_LIST_MAX_SIZE);
            return false;
        ...
        if (!IsInputNameValidChar(inputName)) {
            ERROR_LOG(
                "input name parsed from dymshape string contain illegal char!");
            return false;
        }
        ...
        if (!IsDymShapeValid(inputValue)) {
            ERROR_LOG(
                "the format of shape string parsed from dymshape string is "
                "illegal!");
            return false;
        }
    }
