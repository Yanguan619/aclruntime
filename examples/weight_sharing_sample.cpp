...
//
// 前置: OM 用 ATC --external_weight=1 编译, 权重剥离到同级 weight/ 目录的 weight_<hash> 文件。
// 编译: g++ -std=c++17 weight_sharing_sample.cpp \
//        -I/usr/local/Ascend/ascend-toolkit/latest/aarch64-linux/include \
//        -L/usr/local/Ascend/ascend-toolkit/latest/lib64 -lascendcl -o weight_sharing_sample
...
// 读 weight_dir 下所有权重文件上 Device, 只一份。返回 {文件名: (devPtr, 对齐后大小)}。
...
LoadSharedWeights(const std::string& weightDir) {
    ...
    for (const auto& name : names) {
        ...
        void* dev = nullptr;
        Check(aclrtMalloc(&dev, aligned, ACL_MEM_MALLOC_HUGE_FIRST), "aclrtMalloc " + name);
        Check(aclrtMemcpy(dev, aligned, buf.data(), raw, ACL_MEMCPY_HOST_TO_DEVICE),
              "aclrtMemcpy " + name);
...
static uint32_t LoadModel(const std::string& omPath,
    ...
    // 把每个外置权重的 Device 地址按文件名登记 (两个模型传同一个 devPtr -> 共享)
    for (const auto& [name, devsz] : w) {
        Check(aclmdlSetExternalWeightAddress(h, name.c_str(), devsz.first, devsz.second),
              "aclmdlSetExternalWeightAddress " + name);
    }

    // 加载方式 = 从文件加载
    size_t loadType = ACL_MDL_LOAD_FROM_FILE;
    Check(aclmdlSetConfigOpt(h, ACL_MDL_LOAD_TYPE_SIZET, &loadType, sizeof(loadType)),
    ...
    std::cout << "[model] loaded " << omPath << " -> modelId=" << modelId << std::endl;
    return modelId;
}

int main(int argc, char** argv) {
    std::string om = argc > 1 ? argv[1] : "/data/workspace/qwen2onnx/resnet50.om";
    ...
    std::cout << "[weight] " << weights.size() << " buffers on device (single copy)\n";
    ...
    aclmdlUnload(a);
    aclmdlUnload(b);
    for (const auto& [_, devsz] : weights)
        aclrtFree(devsz.first);
    aclFinalize();
