...
class DymAippManager:
    ...
    def load_aipp_config_content(self):
        ...
        if session_list.count(AIPP_HEAD_STR) != 1:
            logger.error(
                "nums of section aipp_op in .config file is not supported, please check it!"
            )
            raise ValueError("wrong aipp config file content!")
        option_list = self.cfg.options(AIPP_HEAD_STR)
        ...
        if (
            option_list.count("src_image_size_w") == 1
            and option_list.count("src_image_size_h") == 1
        ):
    ...
    def _get_int_safe(self, head_name: str, key_name: str):
        try:
            ...
            raise ValueError(
                "get int type value from aipp config file failed, "
                + f"head name:{head_name}, key name:{key_name}"
            ) from err
        return value
    ...
    def _get_float_safe(self, head_name: str, key_name: str):
        try:
            ...
            raise ValueError(
                "get float type value from aipp config file failed, "
                + f"head name:{head_name}, key name:{key_name}"
            ) from err
        return value
    ...
    def _aipp_set_input_format(self):
        ...
        if legal_image_format.count(input_format) == 1:
        ...
        if tmp_csc_switch == CSC_SWITCH_OFF:
            ...
            for option in options:
                tmp_csc_params.append(
                    0
                    if option_list.count(option) == 0
                    else self._get_int_safe(AIPP_HEAD_STR, option)
                )
    ...
    def _aipp_set_crop_params(self, option_list):
        ...
        if tmp_crop_switch == CROP_SWITCH_OFF:
            ...
            tmp_crop_params.append(
                0
                if option_list.count("load_start_pos_w") == 0
                else self._get_int_safe(AIPP_HEAD_STR, "load_start_pos_w")
            )
            tmp_crop_params.append(
                0
                if option_list.count("load_start_pos_h") == 0
                else self._get_int_safe(AIPP_HEAD_STR, "load_start_pos_h")
            )
            tmp_crop_params.append(
                0
                if option_list.count("crop_size_w") == 0
                else self._get_int_safe(AIPP_HEAD_STR, "crop_size_w")
            )
            tmp_crop_params.append(
                0
                if option_list.count("crop_size_h") == 0
                else self._get_int_safe(AIPP_HEAD_STR, "crop_size_h")
            )

            range_ok = True
            range_ok = range_ok and (
                LOAD_START_POS_W_MIN <= tmp_crop_params[1] <= LOAD_START_POS_W_MAX
            )
            range_ok = range_ok and (
                LOAD_START_POS_H_MIN <= tmp_crop_params[2] <= LOAD_START_POS_H_MAX
            )
    ...
    def _aipp_set_padding_params(self, option_list):
        ...
        if tmp_padding_switch == PADDING_SWITCH_OFF:
            ...
            tmp_padding_params.append(
                0
                if option_list.count("top_padding_size") == 0
                else self._get_int_safe(AIPP_HEAD_STR, "top_padding_size")
            )
            tmp_padding_params.append(
                0
                if option_list.count("bottom_padding_size") == 0
                else self._get_int_safe(AIPP_HEAD_STR, "bottom_padding_size")
            )
            tmp_padding_params.append(
                0
                if option_list.count("left_padding_size") == 0
                else self._get_int_safe(AIPP_HEAD_STR, "left_padding_size")
            )
            tmp_padding_params.append(
                0
                if option_list.count("right_padding_size") == 0
                else self._get_int_safe(AIPP_HEAD_STR, "right_padding_size")
            )

            range_ok = True
            for i in range(1, 5):
                range_ok = range_ok and (
                    PADDING_SIZE_MIN <= tmp_padding_params[i] <= PADDING_SIZE_MAX
                )
            if not range_ok:
    ...
    def _aipp_set_dtc_pixel_mean(self, option_list):
        ...
        tmp_mean_params.append(
            0
            if option_list.count("mean_chn_0") == 0
            else self._get_int_safe(AIPP_HEAD_STR, "mean_chn_0")
        )
        tmp_mean_params.append(
            0
            if option_list.count("mean_chn_1") == 0
            else self._get_int_safe(AIPP_HEAD_STR, "mean_chn_1")
        )
        tmp_mean_params.append(
            0
            if option_list.count("mean_chn_2") == 0
            else self._get_int_safe(AIPP_HEAD_STR, "mean_chn_2")
        )
        tmp_mean_params.append(
            0
            if option_list.count("mean_chn_3") == 0
            else self._get_int_safe(AIPP_HEAD_STR, "mean_chn_3")
        )
    ...
    def _aipp_set_dtc_pixel_min(self, option_list):
        ...
        tmp_min_params.append(
            0
            if option_list.count("min_chn_0") == 0
            else self._get_float_safe(AIPP_HEAD_STR, "min_chn_0")
        )
        tmp_min_params.append(
            0
            if option_list.count("min_chn_1") == 0
            else self._get_float_safe(AIPP_HEAD_STR, "min_chn_1")
        )
        tmp_min_params.append(
            0
            if option_list.count("min_chn_2") == 0
            else self._get_float_safe(AIPP_HEAD_STR, "min_chn_2")
        )
        tmp_min_params.append(
            0
            if option_list.count("min_chn_3") == 0
            else self._get_float_safe(AIPP_HEAD_STR, "min_chn_3")
        )
    ...
    def _aipp_set_pixel_var_reci(self, option_list):
        ...
        tmp_reci_params.append(
            0
            if option_list.count("var_reci_chn_0") == 0
            else self._get_float_safe(AIPP_HEAD_STR, "var_reci_chn_0")
        )
        tmp_reci_params.append(
            0
            if option_list.count("var_reci_chn_1") == 0
            else self._get_float_safe(AIPP_HEAD_STR, "var_reci_chn_1")
        )
        tmp_reci_params.append(
            0
            if option_list.count("var_reci_chn_2") == 0
            else self._get_float_safe(AIPP_HEAD_STR, "var_reci_chn_2")
        )
        tmp_reci_params.append(
            0
            if option_list.count("var_reci_chn_3") == 0
            else self._get_float_safe(AIPP_HEAD_STR, "var_reci_chn_3")
        )

        range_ok = True
        for i in range(0, 4):
            range_ok = range_ok and (
                PIXEL_VAR_RECI_CHN_MIN <= tmp_reci_params[i] <= PIXEL_VAR_RECI_CHN_MAX
            )
        if not range_ok:
