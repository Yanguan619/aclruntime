...
namespace Base {
void AutoSetDym(std::shared_ptr<Feeds> feeds, std::string type,
                std::string inputName, std::vector<size_t> &shape, bool last) {
    auto &target =
        type == "shape" ? feeds->autoDynamicShape : feeds->autoDynamicDims;
...
void PrepareInputData(std::vector<std::string> &files,
    ...
    if (files.size() != inputNames.size()) {
        throw std::runtime_error(
            "files not equal to input names count while prepare input data in "
            "pipeline.");
    }
    if (files.size() != session->GetInputs().size()) {
        throw std::runtime_error(
            "files not equal to model inputs count while prepare input data in "
            "pipeline.");
    }
    for (size_t i = 0; i < files.size(); i++) {
        if (pure_infer) {
            auto array = std::make_shared<cnpy::NpyArray>(
                CreatePureInferArray(files[i], session->GetInputs()[i]));
            feeds->arrayPtr->emplace_back(array);
        } else {
            if (Utils::TailContain(files[i], ".npy") ||
                Utils::TailContain(files[i], ".NPY")) {
                auto array =
                    std::make_shared<cnpy::NpyArray>(cnpy::NpyLoad(files[i]));
                feeds->arrayPtr->emplace_back(array);
            } else {
                auto array =
                    std::make_shared<cnpy::NpyArray>(cnpy::BinLoad(files[i]));
                feeds->arrayPtr->emplace_back(array);
            }
        }

        feeds->inputs->emplace_back(feeds->arrayPtr->back()->Data<void>(),
                                    feeds->arrayPtr->back()->NumBytes());
        if (autoDymShape) {
            AutoSetDym(feeds, "shape", inputNames[i],
                       feeds->arrayPtr->back()->shape, i == (files.size() - 1));
        }
        if (autoDymDims) {
            AutoSetDym(feeds, "dim", inputNames[i],
                       feeds->arrayPtr->back()->shape, i == (files.size() - 1));
        }
    }
}
...
void FuncPrepare(ConcurrentQueue<std::shared_ptr<Feeds>> &h2dQueue,
    ...
    for (size_t i = startIndex; i < n; i += numThreads) {
        ...
        if (inferOption->outputDir != "") {
            for (auto tail : {".npy", ".bin", ".NPY", ".BIN"}) {
                if (Utils::TailContain(files.front(), tail)) {
                    feeds->outputPrefix = Utils::GetPrefix(
                        inferOption->outputDir, files.front(), tail);
                    break;
                }
            }
        }
        ...
        feeds->arrayPtr =
            std::make_shared<std::vector<std::shared_ptr<cnpy::NpyArray>>>();
        PrepareInputData(files, session, feeds, inferOption->autoDymShape,
                         inferOption->autoDymDims, inferOption->pureInferMode,
                         inputNames);
        h2dQueue.push(feeds);
    }
...
void FuncPrepareBaseTensor(
    ...
    std::vector<std::string> &outputNames) {
    APP_ERROR ret = Base::TensorContext::GetInstance()->SetContext(
        session->GetDeviceId(), session->GetContextIndex());
    if (ret != APP_ERR_OK) {
    ...
    for (size_t i = 0; i < inputsList.size(); i++) {
        auto feeds = std::make_shared<Feeds>();
        feeds->inputs =
            std::make_shared<std::vector<Base::BaseTensor>>(inputsList[i]);
        feeds->outputNames = outputNames;
        if (inputNames.size() != shapesList[i].size()) {
            throw std::runtime_error(
                "input name count not equal to shapes count");
        }
        for (size_t j = 0; j < inputNames.size(); j++) {
            if (autoDymShape) {
                AutoSetDym(feeds, "shape", inputNames[j], shapesList[i][j],
                           j == (inputNames.size() - 1));
            }
            if (autoDymDims) {
                AutoSetDym(feeds, "dim", inputNames[j], shapesList[i][j],
                           j == (inputNames.size() - 1));
            }
        }
        h2dQueue.push(feeds);
...
void FuncH2d(ConcurrentQueue<std::shared_ptr<Feeds>> &h2dQueue,
    ...
    while (true) {
        ...
        for (auto &info : *(item->inputs)) {
            Base::MemoryData mem = Base::CopyMemory2DeviceMemory(
                info.buf, info.size, session->GetDeviceId());
...
void FuncCompute(ConcurrentQueue<std::shared_ptr<Feeds>> &computeQueue,
    ...
    while (true) {
        ...
        session->OnlyInfer(*(item->inputs), item->outputNames,
                           *(item->outputs));
...
void SaveOutput(std::shared_ptr<Feeds> item, std::string outFmt, size_t index) {
    ...
    if (outFmt == "NPY") {
        outputFileName += ".npy";
        if (Utils::TensorToNumpy(outputFileName, item->outputs->at(index)) ==
            UtilsResult::FAILED) {
            ERROR_LOG("%s save failed\n", outputFileName.c_str());
        }
    } else if (outFmt == "TXT") {
        outputFileName += ".txt";
        if (Utils::TensorToTxt(outputFileName, item->outputs->at(index)) ==
            UtilsResult::FAILED) {
            ERROR_LOG("%s save failed\n", outputFileName.c_str());
        }
    } else {
        outputFileName += ".bin";
        if (Utils::TensorToBin(outputFileName, item->outputs->at(index)) ==
            UtilsResult::FAILED) {
            ERROR_LOG("%s save failed\n", outputFileName.c_str());
        }
    }
