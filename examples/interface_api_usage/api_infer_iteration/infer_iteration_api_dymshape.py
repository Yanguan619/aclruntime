...
def infer_iteration_api_dymshape():
    device_id = 0
    data_dir = os.getenv("AISBENCH_INFER_DT_TESTDATA_PATH", "../../sampledata/")
    model_path = os.path.join(data_dir, "add_model/model/add_model_dymshape.om")
    ...
    outputs = session.infer_iteration(
        feeds, in_out_list, iteration_times, mode="dymshape", custom_sizes=100000
    )
    logger_print("outputs: %s" % outputs)

infer_iteration_api_dymshape()
