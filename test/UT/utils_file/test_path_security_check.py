...
class TestClass:
    ...
    def init(self):
        self.standard_file_path = os.path.join(
            TestCommonClass.base_path, "fake_model.json"
        )
    ...
    def test_is_legal_path_length_linux(self, monkeypatch):
        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.is_platform", lambda *arg: False
        )
        path = ""
        for _ in range(4097):
            path = path + "x"
        assert not is_legal_path_length(path)
        ...
        path = "xxx"
        assert is_legal_path_length(path)

    def test_is_legal_path_length_windows(self, monkeypatch):
        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.is_platform", lambda *arg: True
        )
        path = ""
        for _ in range(261):
            path = path + "x"
    ...
    def test_is_match_path_white_list_linux(self, monkeypatch):
        ...
        path = "/home&*/"
        assert not is_match_path_white_list(path)
    ...
    def test_is_match_path_white_list_windows(self, monkeypatch):
        ...
        assert not is_match_path_white_list(path)
        path = "D:\home"
        assert is_match_path_white_list(path)

    def test_is_legal_args_path_string(self, monkeypatch):
        assert is_legal_args_path_string("")

        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.is_legal_path_length",
            lambda *arg: False,
        )
        assert not is_legal_args_path_string("/home")
        monkeypatch.undo()

        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.is_match_path_white_list",
            lambda *arg: False,
        )
        assert not is_legal_args_path_string("/home")
        monkeypatch.undo()

        assert is_legal_args_path_string("/home")
    ...
    def test_file_stat_exception(self, monkeypatch):
        ...
        with pytest.raises(Exception) as e:
            ...
            if "create FileStat failed" not in str(e):
    ...
    def test_check_linux_permission(self, monkeypatch):
        ...
        assert not file_stat.check_linux_permission(perm=FilePermChoice.READ)

        file_stat = FileStat(self.standard_file_path)
        monkeypatch.setattr("os.path.islink", lambda *arg: True)
        assert not file_stat.check_linux_permission(perm=FilePermChoice.WRITE)
        monkeypatch.undo()
        ...
        assert file_stat.check_windows_permission(perm=FilePermChoice.READ)

    def test_check_windows_permission(self, monkeypatch):
        file_stat = FileStat(self.standard_file_path)
        file_stat.is_file_exist = False
        assert not file_stat.check_windows_permission(perm=FilePermChoice.READ)

        file_stat = FileStat(self.standard_file_path)
        monkeypatch.setattr("os.path.islink", lambda *arg: True)
        assert not file_stat.check_windows_permission(perm=FilePermChoice.WRITE)
        monkeypatch.undo()
    ...
    def test_ms_open_exist_dir(self, monkeypatch):
        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.FileStat.is_exists",
            lambda *arg: True,
        )
        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.FileStat.is_dir",
            lambda *arg: True,
        )
        with pytest.raises(Exception) as e:
            ms_open(self.standard_file_path)
            if "but it's a folder" not in str(e):
                pytest.fail(f"Do not catch expected err! Actual error is {str(e)}")

    def test_ms_open_softlink(self, monkeypatch):
        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.FileStat.is_softlink",
            lambda *arg: True,
        )
        with pytest.raises(Exception) as e:
            ms_open(self.standard_file_path, mode="r")
            if "Softlink is not allowed" not in str(e):
                pytest.fail(f"Do not catch expected err! Actual error is {str(e)}")
    ...
    def test_ms_open_read(self, monkeypatch):
        ...
        with pytest.raises(Exception) as e:
            ms_open(self.standard_file_path, mode="r")
            if "must have a size limit" not in str(e):
                pytest.fail(f"Do not catch expected err! Actual error is {str(e)}")

        with pytest.raises(Exception) as e:
            ms_open(self.standard_file_path, mode="r", max_size=1)
            if "The file size has exceeded" not in str(e):
                pytest.fail(f"Do not catch expected err! Actual error is {str(e)}")
    ...
    def test_ms_open_write(self, monkeypatch):
        ...
        with pytest.raises(Exception) as e:
            ...
            if "file owner is inconsistent" not in str(e):
                pytest.fail(f"Do not catch expected err! Actual error is {str(e)}")
        monkeypatch.undo()
