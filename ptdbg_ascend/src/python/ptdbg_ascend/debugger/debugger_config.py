...
class DebuggerConfig:
    ...
    def check(self):
        ...
        if self.hook_name not in ["dump", "overflow_check"]:
            raise ValueError("hook_name is not in ['dump', 'overflow_check']".format(self.hook_name))
        if self.rank is not None and not isinstance(self.rank, int):
            raise ValueError("rank {} is not int".format(self.rank))
        if not isinstance(self.step, list):
            raise ValueError("step {} is not list".format(self.step))
        for s in self.step:
            if not isinstance(s, int):
                raise ValueError("step element {} is not int".format(s))
        return True
