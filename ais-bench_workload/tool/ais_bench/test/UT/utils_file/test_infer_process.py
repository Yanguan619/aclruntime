...
class TestInferenceFunctions(unittest.TestCase):
    def setUp(self):
        ...
        self.real_args = AISBenchInferArgsAdapter(model=1, input_path=1, output=1, output_dirname=1, outfmt=1, loop=1,
                 debug=1, device=1, dym_batch=1, dym_hw=1, dym_dims=1,
                 dym_shape=1, output_size=1, auto_set_dymshape_mode=1,
                 auto_set_dymdims_mode=1, batchsize=1, pure_data_type=1,
                 profiler=1, dump=1, acl_json_path=1, output_batchsize_axis=1,
                 run_mode=1, display_all_summary=1, warmup_count=1, dym_shape_range=1, aipp_config=1,
                 energy_consumption=1, npu_id=1, backend=1, perf=1, pipeline=1, profiler_rename=1,
                 dump_npy=1, divide_input=1, threads=1)
    ...
    def test_init_inference_session(self, mock_set_session_options, mock_InferSession):
        ...
        mock_InferSession.assert_called_once_with(self.args.device, self.args.model, acl_json_path, self.args.debug, self.args.loop)
    ...
    def test_warmup(self, mock_run_inference, mock_get_narray_from_files_list, mock_get_tensor_from_files_list):
    ...
    @patch('ais_bench.infer.infer_process.get_narray_from_files_list')
    @patch('ais_bench.infer.infer_process.run_inference')
    def test_warmup1(self, mock_run_inference, mock_get_narray_from_files_list, mock_get_tensor_from_files_list):
        # Mock session and arguments
        session = MagicMock()
        intensors_desc = [MagicMock(realsize=1 * 224 * 224 * 3)]
        infiles = [['file1']]
    ...
    def test_run_inference(self, mock_set_dymdims_shape, mock_set_dymshape_shape):
        # Mock session and arguments
        inputs = [MagicMock()]
        outputs = run_inference(self.session, self.args, inputs)
    ...
    def test_run_pipeline_inference(self, mock_logger):
        ...
        # Call function
        run_pipeline_inference(self.session, self.args, infileslist, output_prefix, extra_session)
    ...
    @patch('ais_bench.infer.infer_process.get_tensor_from_files_list')
    def test_infer_loop_tensor_run(self, mock_get_tensor_from_files_list, mock_save_tensors_to_file, mock_run_inference, mock_tqdm):
        ...
        # Call function
        infer_loop_tensor_run(session, args, intensors_desc, infileslist, output_prefix='output')
    ...
    @patch('ais_bench.infer.infer_process.run_inference')
    @patch('ais_bench.infer.infer_process.save_tensors_to_file')
    def test_infer_loop_files_run(self, mock_save_tensors_to_file, mock_run_inference, mock_tqdm):
        # Mock session and args
        session = MagicMock()
        args = MagicMock()
        args.outfmt = 'npy'
        ...
        # Call function
        with self.assertRaises(ValueError):
            infer_loop_files_run(session, args, intensors_desc, infileslist, output_prefix='output')
    
    @patch('ais_bench.infer.infer_process.tqdm')
    @patch('ais_bench.infer.infer_process.run_inference')
    @patch('ais_bench.infer.infer_process.save_tensors_to_file')
    def test_infer_loop_files_run1(self, mock_save_tensors_to_file, mock_run_inference, mock_tqdm):
        # Mock session and args
        session = MagicMock()
        args = MagicMock()
        args.outfmt = 'npy'
    ...
    def test_infer_fulltensors_run(self, mock_create_intensors_from_infileslist,mock_save_tensors_to_file, mock_run_inference, mock_tqdm):
        # Mock session and args
        session = MagicMock()
        args = MagicMock()
        args.pure_data_type = 'zero'
        ...
        # Mock infileslist
        infileslist = [["file1.txt", "file1.txt"]]
        infer_fulltensors_run(session, args, intensors_desc, infileslist, output_prefix='output')
        mock_run_inference.assert_called_once()
        mock_save_tensors_to_file.assert_called_once()
    ...
    @patch('ais_bench.infer.infer_process.save_tensors_to_file')
    @patch('ais_bench.infer.infer_process.get_narray_from_files_list')
    def test_infer_loop_array_run(self, mock_get_narray_from_files_list, mock_save_tensors_to_file, mock_run_inference, mock_tqdm):
        session = MagicMock()
        args = MagicMock()
        args.pure_data_type = 'zero'
        args.outfmt = 'npy'
        ...
        infer_loop_array_run(session, args, intensors_desc, infileslist, output_prefix='output')
    ...
    def test_infer_pipeline_run(self, mock_run_pipeline_inference):
        ...
        infer_pipeline_run(session, args, infileslist, output_prefix='output', extra_session=extra_session)
    ...
    def test_get_file_name(self, mock_os_walk):
        ...
        self.assertEqual(res_file_path, ['/path/to/files/file1.txt', '/path/to/files/file2.txt'])

    @patch('ais_bench.infer.infer_process.ms_open')
    def test_get_legal_json_content(self, mock_ms_open):
        mock_ms_open.return_value.__enter__.return_value.read.return_value = '{"profiler": {"output": "/path/to/output"}}'
        acl_json_path = 'acl.json'
    ...
    def test_json_to_msprof_cmd(self, mock_get_legal_json_content):
        acl_json_path = 'acl.json'
        mock_get_legal_json_content.return_value = {'--acl_json_path': "xxxx", '--warmup_count': 1, '--profiler': 1}
    ...
    def test_msprof_run_profiling(self, mock_get_file_name, mock_json_to_msprof_cmd, mock_regenerate_cmd, mock_fcntl, mock_call, mock_Popen, mock_rename):
        args = MagicMock()
    ...
    @patch('ais_bench.infer.infer_process.regenerate_cmd')
    @patch('ais_bench.infer.infer_process.json_to_msprof_cmd')
    def test_msprof_run_profiling_no_rename(self, mock_json_to_msprof_cmd, mock_regenerate_cmd, mock_fcntl, mock_call, mock_Popen):
        args = MagicMock()
        args.acl_json_path = None
    ...
    def test_convert(self, mock_check_path_legality, mock_convert_helper, mock_move_subdir, mock_remove, mock_rmtree):
    ...
    def test_main_normal(self, mock_get_energy_consumption, mock_convert, mock_MemorySummary, mock_Summary, mock_infer_loop_tensor_run,
                         mock_infer_fulltensors_run, mock_infer_loop_files_run, mock_infer_loop_array_run,
                         mock_infer_pipeline_run, mock_warmup, mock_init_inference_session, mock_create_tmp_acl_json,
                         mock_get_acl_json_path, mock_logger):
        self.args.threads = 1
        session = MagicMock()
        mock_init_inference_session.return_value = session
        intensors_desc = [MagicMock()]
        session.get_inputs.return_value = intensors_desc
        infileslist = [[MagicMock()]]
        summary = MagicMock()
        ...
        mock_create_tmp_acl_json.return_value = ('tmp_acl.json', 'real_dump_path', 'tmp_dump_path')
    ...
    @patch('ais_bench.infer.infer_process.check_path_legality')
    def test_main_normal1(self, mock_check_path_legality, mock_create_pipeline_fileslist_from_inputs_list, mock_get_energy_consumption, mock_convert, mock_MemorySummary, mock_Summary, mock_infer_loop_tensor_run,
                         mock_infer_fulltensors_run, mock_infer_loop_files_run, mock_infer_loop_array_run,
                         mock_infer_pipeline_run, mock_warmup, mock_init_inference_session, mock_create_tmp_acl_json,
                         mock_get_acl_json_path, mock_logger):
        self.args.threads = 1
    ...
    def test_seg_input_data_for_multi_process(self, mock_isfile, mock_list_share, mock_get_acl_json_path, mock_init_inference_session, mock_get_fileslist_from_dir):
        # Mock inputs
    ...
    def test_acl_json_base_check_normal(self, mock_json, mock_ms_open):
        ...
        mock_ms_open.return_value.__enter__.return_value.read.return_value = '{"profiler": {"switch": "on"}}'
    ...
    @patch('ais_bench.infer.infer_process.ms_open', side_effect=Exception('Mocked exception'))
    ...
    def test_config_check_normal(self, mock_getsize, mock_splitext):
        # Mock config_path and os.path
        config_path = "path/to/config.config"
        mock_splitext.return_value = ("path/to/config", ".config")
        ...
        # Assert
        mock_splitext.assert_called_once_with(config_path)
        mock_getsize.assert_called_once_with(config_path)
    
    @patch('os.path.splitext')
    @patch('os.path.getsize')
    def test_config_check_normal1(self, mock_getsize, mock_splitext):
        # Mock config_path and os.path
        config_path = "path/to/config.config"
        mock_splitext.return_value = ("path/to/config", "nfig")
        mock_getsize.return_value = 1000
    ...
    @patch('os.path.splitext')
    @patch('os.path.getsize')
    def test_config_check_normal2(self, mock_getsize, mock_splitext):
        config_path = "path/to/config.config"
        mock_splitext.return_value = ("path/to/config", ".config")
    ...
    def test_infer_process_normal(self, mock_main, mock_multidevice_run, mock_dymshape_range_run,
                                  mock_msprof_run_profiling, mock_get_msprof_bin_path, mock_backend_run,
                                  mock_acl_json_base_check, mock_version_check, mock_args_rules):
