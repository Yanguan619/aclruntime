...
class TestClass:
    ...
    def test_pure_inference_normal_dynamic_dims(self):
        ...
        for _, dym_dims in enumerate(batch_list):
            cmd = "{} --model {} --device {} --dymDims {}".format(
                TestCommonClass.cmd_prefix,
                model_path,
                TestCommonClass.default_device_id,
                dym_dims,
    ...
    def test_general_inference_normal_static_batch(self):
        ...
        input_path = TestCommonClass.get_inputs_path(
            input_size,
            os.path.join(self.model_base_path, "input"),
    ...
    def test_general_inference_normal_dynamic_batch(self):
        ...
        for _, dys_batch_size in enumerate(batch_list):
            model_path = self.get_dynamic_batch_om_path()
            cmd = "{} --model {} --device {} --dymBatch {} --input {}".format(
                TestCommonClass.cmd_prefix,
                model_path,
                TestCommonClass.default_device_id,
                dys_batch_size,
    ...
    def test_general_inference_normal_run_mode(self):
        ...
        for _, run_mode in enumerate(run_modes):
            cmd = "{} --model {} --device {} --input {} --run_mode {}".format(
                TestCommonClass.cmd_prefix,
                model_path,
                TestCommonClass.default_device_id,
                input_path,
    ...
    def test_general_inference_normal_inference_time(self):
        ...
        input_size = TestCommonClass.get_model_inputs_size(static_model_path)[0]
        input_path = TestCommonClass.get_inputs_path(
            input_size, os.path.join(self.model_base_path, "input"), num_input_file
        )
        ...
        cmd = "{} --model {} --device {} --input {}  --debug true --output {}  --output_dirname {} > {}".format(
            TestCommonClass.cmd_prefix,
            model_path,
            TestCommonClass.default_device_id,
            input_path,
        ...
        # ignore of warmup inference time, get  inferening times from log file
        #  and  summary.json, compare them and assert
        infer_time_lists = []
        with open(log_path) as f:
            i = 0
            for line in f:
        ...
        summary_json_path = os.path.join(
            output_parent_path, "{}_summary.json".format(output_dirname)
        )
        with open(summary_json_path, "r", encoding="utf8") as fp:
            ...
            assert (
                math.fabs(np.percentile(time_array, 99) - json_percentile)
                <= TestCommonClass.EPSILON
            )
    ...
    def test_general_inference_normal_warmup_count(self):
        ...
        for i, warmup_num in enumerate(warmups):
            ...
            os.makedirs(output_path, 0o750)
            cmd = (
                "{} --model {} --device {} --input {}  --debug true --output {}  --output_dirname {} \
                --warmup_count {} > {}".format(
                    TestCommonClass.cmd_prefix,
                    model_path,
                    TestCommonClass.default_device_id,
    ...
    def test_pure_inference_normal_warmup_count_200(self):
        ...
        os.makedirs(output_path, 0o750)
        cmd = (
            "{} --model {} --device {} --debug true --output {}  --output_dirname {} --warmup_count {} \
            --loop {} > {}".format(
                TestCommonClass.cmd_prefix,
                model_path,
                TestCommonClass.default_device_id,
    ...
    def test_pure_inference_normal_pure_data_type(self):
        ...
        for pure_data_type in pure_data_types:
            ...
            os.makedirs(output_path, 0o750)
            cmd = (
                "{} --model {} --device {}  --debug true --output {}  --output_dirname {} --pure_data_type {} \
                --loop {} > {}".format(
                    TestCommonClass.cmd_prefix,
                    model_path,
                    TestCommonClass.default_device_id,
            ...
            for i, bin_file in enumerate(bin_files):
                ...
                if i > 0:
                    if pure_data_type == "zero":
                        ...
                        assert (
                            filecmp.cmp(first_output_bin_file_path, bin_file_path)
                            is False
                        )
    ...
    def test_general_inference_prformance_comparison_with_msame_static_batch(self):
        ...
        summary_json_path = os.path.join(
            output_path, "{}_summary.json".format(output_dir_name)
        )
    ...
    def test_pure_inference_batchsize_is_none_normal_static_batch(self):
        ...
        for i, batch_size in enumerate(batch_list):
            ...
            cmd = (
                "{} --model {} --device {} --output {} --output_dirname {} > {}".format(
                    TestCommonClass.cmd_prefix,
                    model_path,
                    TestCommonClass.default_device_id,
                    output_parent_path,
    ...
    def test_pure_inference_batchsize_is_none_normal_dynamic_batch(self):
        ...
        for i, dys_batch_size in enumerate(batch_list):
            ...
            model_path = self.get_dynamic_batch_om_path()
            cmd = "{} --model {} --device {}  --output {} --output_dirname {} --dymBatch {} > {}".format(
                TestCommonClass.cmd_prefix,
                model_path,
                TestCommonClass.default_device_id,
                output_parent_path,
    ...
    def test_pure_inference_batchsize_is_none_normal_dynamic_dims(self):
        ...
        for i, dym_dim in enumerate(dym_dims):
            ...
            cmd = (
                "{} --model {} --device {} --dymDims {} --output {} --output_dirname {} > \
                {}".format(
                    TestCommonClass.cmd_prefix,
                    model_path,
                    TestCommonClass.default_device_id,
    ...
    def test_pure_inference_batchsize_is_none_normal_dynamic_hw(self):
        ...
        for i, dym_hw in enumerate(hw_list):
            ...
            cmd = (
                "{} --model {} --device {} --dymHW {} --output {} --output_dirname {} > \
                {}".format(
                    TestCommonClass.cmd_prefix,
                    model_path,
                    TestCommonClass.default_device_id,
    ...
    def test_pure_inference_batchsize(self):
        ...
        for i, batch_size in enumerate(batch_sizes):
            ...
            cmd = "{} --model {} --batchsize {} --output {} --output_dirname {} > {}".format(
                TestCommonClass.cmd_prefix,
                model_path,
                para_batch_size,
                output_parent_path,
            ...
            batch_size_from_reference_result = (
                self.get_model_batchsize_from_inference_result(log_path)
            )
    ...
    def test_general_inference_interface_simple(self):
        ...
        infer_sample_output_path = os.path.join(
            self.model_base_path, "output", "infer_sample_output.bin"
        )
        ...
        if os.path.exists(output_path):
            shutil.rmtree(output_path)
        os.makedirs(output_path, 0o750)
        cmd = "{} --model {} --input {} --output {} --output_dirname {} --outfmt BIN".format(
            TestCommonClass.cmd_prefix,
            model_path,
            input_path,
            output_parent_path,
        ...
        ret = os.system(cmd)
        assert ret == 0
        output_npy_file_path = os.path.join(
            output_path, "{}_0.bin".format(output_dirname)
        )
    ...
    def test_general_inference_interface_dynamic_dims(self):
        ...
        infer_dynamic_dims_output_path = os.path.join(
            self.model_base_path, "output", "infer_dynamic_dims_output.bin"
        )
        out = np.array(outarray)
    ...
    def test_pure_inference_normal_static_batch_output_txt_file(self):
        ...
        cmd = "{} --model {} --device {} --outfmt 'TXT' --output {}  --output_dirname {}".format(
            TestCommonClass.cmd_prefix,
            model_path,
            TestCommonClass.default_device_id,
            output_parent_path,
    ...
    def test_general_inference_interface_normal_multi_device(self):
        ...
        cmd = "{} --model {} --device {} --input {} --debug=1 > {}".format(
            TestCommonClass.cmd_prefix, model_path, devices, input_path, log_path
        )
        logger.info("run cmd:{}".format(cmd))
        ret = os.system(cmd)
        assert ret == 0
    ...
    def test_general_inference_interface_same_multi_device_0_divide_input_for_odd_count(
        self,
    ):
    ...
    def test_general_inference_interface_same_multi_device_0_divide_input_for_even_count(
        self,
    ):
        """device 0,0 divide_inputs 1"""
        devices = "0,0"
    ...
    def test_pure_inference_profiler_normal(self):
        ...
        cmd = "{} --model {} --device {} --profiler True --output {} > {}".format(
            TestCommonClass.cmd_prefix,
            model_path,
            TestCommonClass.default_device_id,
            output_path,
