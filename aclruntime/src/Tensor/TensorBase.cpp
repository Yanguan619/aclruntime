...
namespace {
constexpr uint32_t ZERO_BYTE = 0;
constexpr uint32_t ONE_BYTE = 1;
constexpr uint32_t TWO_BYTE = 2;
...
uint32_t TensorBase::DataTypeByteSize(TensorDataType type) {
    switch (type) {
        ...
        case TENSOR_DTYPE_FLOAT16:
            return TWO_BYTE;
        case TENSOR_DTYPE_UINT32:
        case TENSOR_DTYPE_INT32:
        case TENSOR_DTYPE_FLOAT32:
            return FOUR_BYTE;
        case TENSOR_DTYPE_UINT64:
        case TENSOR_DTYPE_INT64:
        case TENSOR_DTYPE_DOUBLE64:
            return EIGHT_BYTE;
        default:
            return ZERO_BYTE;
    }
...
TensorBase::TensorBase(const MemoryData &memoryData, const bool &isBorrowed,
                       const std::vector<uint32_t> &shape,
                       const TensorDataType &type, const size_t contextIndex)
    : dataType_(type) {
    ...
    if (isBorrowed) {
        ...
        auto deleter = [buffer](void *p) {
            buffer.SetContext();
            Base::MemoryData memoryData(p, buffer.size, buffer.type,
                                        buffer.deviceId);
...
TensorBase::TensorBase(const std::vector<uint32_t> &shape,
                       ...
                       const MemoryData::MemoryType &bufferType,
                       const int32_t &deviceId, const size_t contextIndex)
    : dataType_(type) {
    shape_ = std::make_shared<TensorShape>(shape);
    uint32_t bytes = DataTypeByteSize(type);
...
TensorBase::TensorBase(const std::vector<uint32_t> &shape,
                       const TensorDataType &type, const int32_t &deviceId,
                       const size_t contextIndex) {
    shape_ = std::make_shared<TensorShape>(shape);
    uint32_t bytes = DataTypeByteSize(type);
    buffer_ =
        std::make_shared<TensorBuffer>(shape_->GetSize() * bytes, deviceId);
    buffer_->contextIndex = contextIndex;
}
...
APP_ERROR TensorBase::ToDevice(int32_t deviceId) {
    ...
    if (GetDeviceId() == deviceId &&
        buffer_->type == MemoryData::MemoryType::MEMORY_DEVICE) {
        return APP_ERR_OK;
    }
    ...
    TensorBuffer newBuffer(buffer_->size, MemoryData::MemoryType::MEMORY_DEVICE,
                           deviceId);
...
APP_ERROR TensorBase::ToDvpp(int32_t deviceId) {
    ...
    if (GetDeviceId() == deviceId &&
        buffer_->type == MemoryData::MemoryType::MEMORY_DVPP) {
        return APP_ERR_OK;
    }
    ...
    // device a to device b
    TensorBuffer newBuffer(buffer_->size, MemoryData::MemoryType::MEMORY_DVPP,
                           deviceId);
    ret = TensorBuffer::TensorBufferMalloc(newBuffer);
    if (ret != APP_ERR_OK) {
        ERROR_LOG("Tensor malloc buffer failed. ret=%d", ret);
...
size_t TensorBase::GetByteSize() const {
    ...
    return shape_->GetSize() * DataTypeByteSize(dataType_);
}
// tensor 的shape
...
// tensor 数据类型
TensorDataType TensorBase::GetDataType() const { return dataType_; }
...
APP_ERROR TensorBase::CheckBatchTensors(const std::vector<TensorBase> &inputs,
                                        const bool &checkFirstDim) {
    auto checkFunc = [checkFirstDim](const TensorBase &t1,
                                     const TensorBase &t2) {
        if (t1.GetShape().size() != t2.GetShape().size()) {
            ERROR_LOG("dimension is not match (%lu) vs (%lu)",
                      t1.GetShape().size(), t2.GetShape().size());
            return false;
        }
        if (t1.GetDeviceId() != t2.GetDeviceId()) {
            ERROR_LOG("deviceId is not match (%d) vs (%d)", t1.GetDeviceId(),
                      t2.GetDeviceId());
            return false;
        }
        if (t1.GetDataType() != t2.GetDataType()) {
            ERROR_LOG("data type is not match (%d) vs (%d)", t1.GetDataType(),
                      t2.GetDataType());
            return false;
        }
        if (t1.GetTensorType() != t2.GetTensorType()) {
            ERROR_LOG("memory type is not match (%d) vs (%d)",
                      t1.GetTensorType(), t2.GetTensorType());
            return false;
        }
        ...
        for (uint32_t i = startIndex; i < t1.GetShape().size(); i++) {
            ...
            ERROR_LOG("tensor shape is not match %s vs %s", shapeStr1.c_str(),
                      shapeStr2.c_str());
            return false;
        }
        return true;
...
APP_ERROR TensorBase::BatchConcat(const std::vector<TensorBase> &inputs,
                                  TensorBase &output) {
    // check input size
    if (inputs.size() == 0) {
    ...
    // malloc
    output = TensorBase(batchShape, inputs[0].GetDataType(),
                        inputs[0].GetTensorType(), inputs[0].GetDeviceId());
...
APP_ERROR TensorBase::BatchStack(const std::vector<TensorBase> &inputs,
                                 TensorBase &output) {
    // check
    if (inputs.size() == 0) {
        ERROR_LOG("input size(%zu)", inputs.size());
...
APP_ERROR TensorBase::BatchVector(const std::vector<TensorBase> &inputs,
                                  TensorBase &output, const bool &keepDims) {
    if (keepDims) {
...
std::string TensorBase::GetDesc() {
    ...
    return "<Tensor>\nshape:\t(" + shapeStr + ")\ndtype:\t" +
           GetTensorDataTypeDesc(GetDataType()) + "\ndevice:\t" +
...
MemoryData CopyMemory2DeviceMemory(void *ptr, uint64_t size, int32_t deviceId) {
    ...
    if (ret != APP_ERR_OK) {
        throw std::runtime_error(
            "copy mem to device failed! cause: malloc device memory data "
            "failed!");
    }
    ret = MemoryHelper::MxbsMemcpy(dst, src, dst.size);
    if (ret != APP_ERR_OK) {
        dst.memory_free(dst.ptrData);
        throw std::runtime_error(
            "copy mem to device failed! cause: memcpy failed!");
    }
    return dst;
}
}  // namespace Base
