...
public:
    MOCK_METHOD(bool, OpenFileIfstream,
                (const std::string&, std::ifstream&, std::ios_base::openmode),
                ());
    MOCK_METHOD(bool, OpenFileOfstream,
                (const std::string&, std::ofstream&, std::ios_base::openmode),
                ());
    MOCK_METHOD(bool, OpenFileOfstreamSimple,
...
namespace {
bool OpenFileIfstreamProxy(const std::string& path, std::ifstream& f,
                           std::ios_base::openmode mode) {
    if (g_fileOpener) return g_fileOpener->OpenFileIfstream(path, f, mode);
    return false;
}
bool OpenFileOfstreamProxy(const std::string& path, std::ofstream& f,
                           std::ios_base::openmode mode) {
...
namespace cnpy {
template <typename T>
void NpySave(const std::string& fname, T* data, std::vector<size_t>& shape,
             std::string mode = "w") {
...
protected:
    ...
    void TearDown() override { g_fileOpener = nullptr; }
};
...
TEST_F(UtilsTest, ReadBinFileToMemory_SuccessAndFail) {
    ...
    ASSERT_EQ(Utils::ReadBinFileToMemory("./test.bin", buf, 100, offset),
              SUCCESS);
    offset = 0;
    ASSERT_EQ(Utils::ReadBinFileToMemory("not_exist.bin", buf, 100, offset),
              FAILED);
    remove("test.bin");
}
...
TEST_F(UtilsTest, FillFileContentToMemory_SuccessAndFail) {
    ...
    f.close();
    chmod("test.bin", 0750);  // 设置权限为750
    ASSERT_EQ(Utils::FillFileContentToMemory("./test.bin", buf, 100, offset),
              SUCCESS);
    offset = 0;
    ASSERT_EQ(Utils::FillFileContentToMemory("not_exist.bin", buf, 100, offset),
              FAILED);
    remove("test.bin");
}
