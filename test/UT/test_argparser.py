...
class TestClass:
    ...
    def test_argparser_args(self, monkeypatch):
        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.FileStat.is_basically_legal",
            lambda *arg: True,
        )
        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.FileStat.is_legal_file_type",
            lambda *arg: True,
        )
        monkeypatch.setattr(
            "ais_bench.infer.common.path_security_check.FileStat.is_legal_file_size",
            lambda *arg: True,
        )
        for arg_name, value in ARGS_VALUE_DICT.items():
            monkeypatch.setattr(
                "sys.argv", ["python3", "--model", "xx.om"] + [arg_name, value]
            )
            args = get_args()
