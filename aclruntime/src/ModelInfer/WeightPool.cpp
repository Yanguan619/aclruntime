...
UtilsResult::Result WeightPool::Acquire(
  ...
  for (const std::string& name : names) {
    ...
    if (it != cache_.end()) {
      ...
      DEBUG_LOG("[%s] Allocating weight %s: raw=%zu, aligned=%zu, RSS=%zuMB",
                TAG_WEIGHT, name.c_str(), rawSize, aligned,
                GetSystemMemoryUsedMB());
      aclError ret =
          aclrtMallocAlign32(&devPtr, aligned, ACL_MEM_MALLOC_HUGE_FIRST);
      if (ret != ACL_SUCCESS) {
      ...
      ret =
          aclrtMemcpy(devPtr, aligned, data.data(),
      ...
      ret =
          aclmdlSetExternalWeightAddress(handle, name.c_str(), devPtr, aligned);
      if (ret != ACL_SUCCESS) {
        std::cerr << "[" << TAG_WEIGHT
                  << "] aclmdlSetExternalWeightAddress failed for " << name
                  << " ret=" << ret << std::endl;
        return UtilsResult::FAILED;
      }
      newCount += 1;
  ...
  INFO_LOG(
      ...
      TAG_WEIGHT, total, reusedCount, reusePct, FormatSize(reusedBytes).c_str(),
      newCount, newPct, FormatSize(newBytes).c_str());
