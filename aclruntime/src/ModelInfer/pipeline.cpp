...
void PrepareInputData(std::vector<std::string> &files,
  ...
  for (size_t i = 0; i < files.size(); i++) {
    ...
    if (autoDymShape) {
      AutoSetDym(feeds, "shape", inputNames[i], feeds->arrayPtr->back()->shape,
                 i == (files.size() - 1));
    }
    if (autoDymDims) {
      AutoSetDym(feeds, "dim", inputNames[i], feeds->arrayPtr->back()->shape,
                 i == (files.size() - 1));
    }
  }
}
    ...
    if (inferOption->outputDir != "") {
      for (auto tail : {".npy", ".bin", ".NPY", ".BIN"}) {
        if (Utils::TailContain(files.front(), tail)) {
          feeds->outputPrefix =
              Utils::GetPrefix(inferOption->outputDir, files.front(), tail);
          break;
        }
      }
    }
