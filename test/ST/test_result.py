...
class TestClass:
    ...
    def get_dynamic_batch_om_path(self):
        return os.path.join(
            TestCommonClass.get_basepath(),
            self.model_name,
            "model",
            "pth_resnet50_dymbatch.om",
        )
    ...
    def test_general_inference_normal_static_batch(self):
        ...
        output_path = os.path.join(
            TestCommonClass.get_basepath(), self.model_name, "output"
        )
        TestCommonClass.prepare_dir(output_path)
        ...
        input_path = TestCommonClass.get_inputs_path(
            input_size,
            os.path.join(
                os.path.join(TestCommonClass.get_basepath(), self.model_name), "input"
            ),
            output_file_num,
        )
        ...
        for _, batch_size in enumerate(batch_list):
            ...
            result_path = os.path.join(
                output_path, outval.split(":")[1].replace("\n", "")
            )
            result_paths.append(result_path)
            summary_json_name = result_path.split("/")[-1]
            summary_json_paths.append(
                os.path.join(output_path, "{}_summary.json".format(summary_json_name))
            )
    ...
    def test_pipeline_inference_normal_static_batch(self):
        ...
        output_file_num = 20
        output_path = os.path.join(
            TestCommonClass.get_basepath(), self.model_name, "output_pipeline"
        )
        TestCommonClass.prepare_dir(output_path)
        log_path = os.path.join(output_path, "log.txt")
        ...
        summary_json_path = os.path.join(
            output_path, "{}_summary.json".format(summary_json_name)
        )
    ...
    def test_multi_threads_inference_normal_static_batch(self):
        ...
        for threads in threads_list:
            ...
            cmd = (
                "{} --model {} --device {} --output {} --debug True --pipeline True --threads {}\
                --input {} > {}".format(
                    TestCommonClass.cmd_prefix,
                    model_path,
                    TestCommonClass.default_device_id,
    ...
    def test_general_inference_normal_dynamic_batch(self):
        ...
        for _, dys_batch_size in enumerate(batch_list):
            cmd = (
                "{0} --model {1} --device {2} --debug True --dymBatch {3} --input {4} \
                --output {5} > {6}".format(
                    TestCommonClass.cmd_prefix,
                    model_path,
                    TestCommonClass.default_device_id,
    ...
    def test_general_inference_with_dump_npy(self):
        ...
        input_dir = os.path.join(
            TestCommonClass.get_basepath(), self.model_name, "input"
        )
        input_path = TestCommonClass.get_inputs_path(
            input_size, input_dir, output_file_num
        )
        if os.path.exists(os.path.join(output_path, "dump")):
            shutil.rmtree(os.path.join(output_path, "dump"))

        cmd = (
            "{} --model {} --device {} --output {} --debug True --dump True --dump_npy True\
            --input {} > {}".format(
                TestCommonClass.cmd_prefix,
                model_path,
                TestCommonClass.default_device_id,
        ...
        assert (
            dump_subdirs[0] + "_npy" == dump_subdirs[1]
            or dump_subdirs[1] + "_npy" == dump_subdirs[0]
        )
