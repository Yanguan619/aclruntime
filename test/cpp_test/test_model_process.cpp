...
public:
    explicit SetDebugLogGuard() { Base::LogCtrl::SetLogLevel(LOG_DEBUG_LEVEL); }
    ~SetDebugLogGuard() { Base::LogCtrl::SetLogLevel(LOG_INFO_LEVEL); }
};
...
protected:
    void SetUp() override {
        ...
        EXPECT_EQ(modelProcess->str2aclAippInputFormat["YUV420SP_U8"],
                  ACL_YUV420SP_U8);
        EXPECT_EQ(modelProcess->str2aclAippInputFormat["XRGB8888_U8"],
                  ACL_XRGB8888_U8);
        EXPECT_EQ(modelProcess->str2aclAippInputFormat["RGB888_U8"],
                  ACL_RGB888_U8);
        EXPECT_EQ(modelProcess->str2aclAippInputFormat["YUV400_U8"],
                  ACL_YUV400_U8);
...
#undef SET_DEFAULT_EXPECT
    ...
    aclmdlDesc* CreateModelDescSuccess(uint32_t modelId = expectedModelId) {
        ...
        EXPECT_CALL(*mockAcl, aclmdlCreateDesc()).WillOnce(Return(fakeDesc));
    ...
    void SetupCompleteModel(size_t numInputs = 2,
                            const vector<const char*>& inputNames = {
                                "input1", "input2"}) {
        // 模拟加载模型成功
        LoadModelSuccess();
    ...
    void SetupMockModelDescription(aclmdlDesc* fakeDesc, size_t numInputs,
                                   size_t numOutputs) {
        EXPECT_CALL(*mockAcl, aclmdlGetInputDims(fakeDesc, _, _))
            .Times(numInputs)
            .WillRepeatedly(Invoke([numInputs](const aclmdlDesc*, size_t index,
        ...
        EXPECT_CALL(*mockAcl, aclmdlGetOutputDims(fakeDesc, _, _))
            .Times(numOutputs)
            .WillRepeatedly(Invoke([numOutputs](const aclmdlDesc*, size_t index,
...
TEST_F(ModelProcessTest, TestLoadModelFromFile_Success) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlLoadFromFile(validModelPath.c_str(), _))
        .WillOnce(
            DoAll(SetArgPointee<1>(expectedModelId), Return(ACL_SUCCESS)));
...
TEST_F(ModelProcessTest, TestCreateDesc_Success) {
    ...
    // 验证日志
    EXPECT_TRUE(logOutput.find("create model description success") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCreateDesc_CreateFailed) {
    ...
    // 设置创建描述失败
    EXPECT_CALL(*mockAcl, aclmdlCreateDesc()).WillOnce(Return(nullptr));
    ...
    // 验证错误日志
    EXPECT_TRUE(logOutput.find("ERROR") != string::npos);
    EXPECT_TRUE(logOutput.find("create model description failed") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCreateDesc_GetDescFailed) {
    ...
    EXPECT_TRUE(logOutput.find("Failed to get model description") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestGetDynamicGearCount_Success) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetInputDynamicGearCount(fakeDesc, -1, _))
        .WillOnce(
            DoAll(SetArgPointee<2>(expectedGearCount), Return(ACL_SUCCESS)));
...
TEST_F(ModelProcessTest, TestGetDynamicGearCount_Failure) {
    ...
    // 验证错误日志
    EXPECT_TRUE(logOutput.find("get input dynamic gear count failed") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestGetDynamicIndex_DynamicTensorExists) {
    ...
    EXPECT_CALL(*mockAcl,
                aclmdlGetInputIndexByName(
                    ...
                    NotNull()))
        .WillOnce(DoAll(SetArgPointee<2>(expectedIndex), Return(ACL_SUCCESS)));
...
TEST_F(ModelProcessTest, TestGetDynamicIndex_GetNameFailed) {
    ...
    // 验证错误日志
    EXPECT_TRUE(logOutput.find("get input name by index failed") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestGetDynamicIndex_GetIndexFailed) {
    ...
    // 验证错误日志
    EXPECT_TRUE(logOutput.find("get input index by name failed") !=
                string::npos);
    EXPECT_TRUE(logOutput.find("Failed to get dynamic tensor index") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestGetDynamicIndex_NoModelDesc) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetNumInputs(nullptr)).WillOnce(Return(0));
...
TEST_F(ModelProcessTest, TestGetDynamicIndex_NoInputs) {
    ...
    // 设置输入数量为0
    EXPECT_CALL(*mockAcl, aclmdlGetNumInputs(fakeDesc)).WillOnce(Return(0));
...
TEST_F(ModelProcessTest, TestCheckDynamicShape_InputCountMismatch) {
    ...
    testing::internal::CaptureStdout();
    Result result =
        modelProcess->CheckDynamicShape(dymShape, shapeMap, dimsNum);
    string logOutput = testing::internal::GetCapturedStdout();
...
TEST_F(ModelProcessTest, TestCheckDynamicShape_InvalidDimensionFormat) {
    ...
    EXPECT_EQ(result, FAILED);
    EXPECT_TRUE(logOutput.find("ERROR") != string::npos);
    EXPECT_TRUE(logOutput.find("dim of dymshape string is illegal") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCheckDynamicShape_MissingColon) {
    ...
    EXPECT_TRUE(
        logOutput.find(
            "the dymShape parameter set error, please check input name") !=
        string::npos);
}
...
TEST_F(ModelProcessTest, TestSetDynamicShape_DimNumMismatch) {
    ...
    // 添加销毁期望
    EXPECT_CALL(*mockAcl, aclmdlDestroyDataset(
                              reinterpret_cast<const aclmdlDataset*>(0x1111)))
        .WillOnce(Return(ACL_SUCCESS));
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("dims num size: 3 not equal to input num 2") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestSetDynamicShape_GetInputNameFailed) {
    ...
    EXPECT_CALL(*mockAcl,
                aclmdlGetInputNameByIndex(modelProcess->modelDesc_, 0))
        .WillOnce(Return(nullptr));
...
TEST_F(ModelProcessTest,
       TestSetDynamicShape_CreateTensorDescOrSetDatasetTensorFailed) {
    SetupCompleteModel(1, {"input1"});
    SetupModelProcessInput();
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("acl set dataset TensorDesc failed") !=
                string::npos);
    EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
}
...
TEST_F(ModelProcessTest, TestSetDynamicShape_SingleInputSuccess) {
    ...
    EXPECT_CALL(*mockAcl,
                aclmdlSetDatasetTensorDesc(modelProcess->input_, fakeDesc, 0))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestGetMaxDynamicHWSize_NoDynamicHW) {
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(
        logOutput.find("the dynamic_image_size parameter is not specified") !=
        string::npos);
}
...
TEST_F(ModelProcessTest, TestCheckDynamicHWSize_Success) {
    ...
    EXPECT_TRUE(logOutput.find("check dynamic image size success") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCheckDynamicHWSize_GetDynamicHWFailed) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetDynamicHW(fakeDesc, -1, _))
        .WillOnce(
            DoAll(SetArgPointee<2>(dynamicHW), Return(ACL_ERROR_FAILURE)));
    EXPECT_CALL(*mockAcl, aclGetRecentErrMsg())
...
TEST_F(ModelProcessTest, TestCheckDynamicHWSize_DynamicHWNotFound) {
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("the dymHW parameter is not correct") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestSetDynamicHW_Success) {
    ...
    EXPECT_CALL(*mockAcl,
                aclmdlSetDynamicHWSize(expectedModelId,
                                       reinterpret_cast<aclmdlDataset*>(0x1234),
                                       0, 256, 256))
        .WillOnce(Return(ACL_SUCCESS));
    EXPECT_CALL(*mockAcl, aclmdlDestroyDataset(
                              reinterpret_cast<const aclmdlDataset*>(0x1234)))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestSetDynamicHW_Failed) {
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("acl set dynamicHW size failed") !=
                string::npos);
    EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
}
...
TEST_F(ModelProcessTest, TestCheckDynamicBatchSize_InvalidBatch) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetDynamicBatch(fakeDesc, _))
        .Times(2)
        .WillRepeatedly(
            DoAll(SetArgPointee<1>(batchInfo), Return(ACL_SUCCESS)));
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("the dymBatch parameter is not correct") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCheckDynamicBatchSize_NoDynamicBatch) {
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(
        logOutput.find("the dynamic_batch_size parameter is not specified") !=
        string::npos);
}
...
TEST_F(ModelProcessTest, TestGetMaxBatchSize_Success) {
    ...
    EXPECT_TRUE(logOutput.find("get max dynamic batch size success") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestSetDynamicBatchSize_Success) {
    ...
    EXPECT_CALL(*mockAcl,
                aclmdlDestroyDataset(
                    reinterpret_cast<const aclmdlDataset*>(inputDataset)))
        .WillOnce(Return(ACL_SUCCESS));

    // 设置期望
    EXPECT_CALL(*mockAcl,
                aclmdlSetDynamicBatchSize(expectedModelId, inputDataset, 0, 8))
        .WillOnce(Return(ACL_SUCCESS));
    ...
    // 验证结果
    EXPECT_EQ(ret, SUCCESS);
    EXPECT_TRUE(logOutput.find("set dynamic batch size success") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestSetDynamicBatchSize_Failed) {
    ...
    // 设置期望
    EXPECT_CALL(*mockAcl,
                aclmdlSetDynamicBatchSize(expectedModelId, inputDataset, 0, 16))
        .WillOnce(Return(ACL_ERROR_BAD_ALLOC));
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("acl set dynamic batch size failed") !=
                string::npos);
    EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
}
...
TEST_F(ModelProcessTest, TestGetCurOutputDimsMul_Success) {
    ...
    aclmdlIODims ioDims;
    ioDims.dimCount = 3;
    int64_t dimsArray[ACL_MAX_DIM_CNT] = {1, 2,
                                          3};  // 假设 ACL_MAX_DIM_CNT 足够大
...
TEST_F(ModelProcessTest, TestGetCurOutputDimsMul_Failed) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("acl get current output dims failed ret") !=
                string::npos);
    EXPECT_TRUE(logOutput.find("maybe the modle has dynamic shape") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCheckDynamicDims_DimCountMismatch) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("the dymDims parameter is not correct") !=
                string::npos);
...
TEST_F(ModelProcessTest, TestCheckDynamicDims_InvalidInteger) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("dim of dymdims string is illegal") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCheckDynamicDims_DimsNotMatching) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("the dynamic_dims parameter is not correct") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestSetDynamicDims_Success) {
    ...
    // 设置模拟期望 - 验证传入的 dims
    EXPECT_CALL(*mockAcl,
                aclmdlSetInputDynamicDims(expectedModelId, inputDataset, 0, _))
        .WillOnce(
            [](uint32_t, aclmdlDataset*, size_t, const aclmdlIODims* dims) {
...
TEST_F(ModelProcessTest, TestSetDynamicDims_InvalidInteger) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("Invalid input for conversion: abc") !=
                string::npos);
}

TEST_F(ModelProcessTest, TestSetDynamicDims_OutOfRange) {
    // 准备测试数据
    vector<string> dymDims = {"1", "2",
                              "99999999999999999999"};  // 超出 long 范围
    ...
    EXPECT_TRUE(
        logOutput.find(
            "Out of range input for conversion: 99999999999999999999") !=
        string::npos);
}
...
TEST_F(ModelProcessTest, TestSetDynamicDims_SetDimsFailed) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("acl set input dynamic dims failed") !=
                string::npos);
    EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
}
...
TEST_F(ModelProcessTest, TestGetDymHWInfo) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetDynamicHW(fakeDesc, _, _))
        .WillOnce(
            Invoke([](const aclmdlDesc*, int profileIndex, aclmdlHW* hwInfo) {
                hwInfo->hwCount = 1;
                hwInfo->hw[0][0] = 128;
                hwInfo->hw[0][1] = 128;
                return ACL_SUCCESS;
    ...
    EXPECT_TRUE(logOutput.find("please set correct dynamic batch size") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestDestroyDesc_WhenModelDescIsNull) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlDestroyDesc(nullptr)).Times(0);
...
TEST_F(ModelProcessTest, TestCreateDymInput_Success) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlCreateDataset()).WillOnce(Return(fakeDataset));
    EXPECT_CALL(*mockAcl,
                aclmdlDestroyDataset(
                    reinterpret_cast<const aclmdlDataset*>(fakeDataset)))
        .WillOnce(Return(ACL_SUCCESS));

    // 3. 分配设备内存
    void* fakeBuffer = reinterpret_cast<void*>(0x1000);
    EXPECT_CALL(*mockAcl,
                aclrtMalloc(_, buffer_size, ACL_MEM_MALLOC_HUGE_FIRST))
    ...
    EXPECT_TRUE(logOutput.find("add input_ at CreateDymInput +1") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCreateDymInput_CreateDatasetFailed) {
    ...
    // 创建数据集返回空指针
    EXPECT_CALL(*mockAcl, aclmdlCreateDataset).WillOnce(Return(nullptr));
...
TEST_F(ModelProcessTest, TestCreateDymInput_CreateDataBufferFailed) {
    ...
    // 5. 预期释放资源
    EXPECT_CALL(*mockAcl, aclrtFree(fakeBuffer)).WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestCreateDymInput_AddDatasetBufferFailed) {
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_EQ(modelProcess->input_, fakeDataset);
    EXPECT_TRUE(logOutput.find("add input dataset buffer failed") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestUpdateInputsReuse_RelationSizeMismatch) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetDatasetNumBuffers).WillRepeatedly(Return(0));
...
TEST_F(ModelProcessTest, TestUpdateInputsReuse_OutputIndexOutOfRange) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("outputdata index out of range") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestUpdateInputsReuse_SizeMismatch) {
    ...
    // 模拟缓冲区获取
    EXPECT_CALL(*mockAcl, aclmdlGetDatasetBuffer(
                              reinterpret_cast<aclmdlDataset*>(0x1111), 0))
        .WillRepeatedly(Return(fakeInputBuffer));
    EXPECT_CALL(*mockAcl, aclmdlGetDatasetBuffer(
                              reinterpret_cast<aclmdlDataset*>(0x2222), 0))
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(
        logOutput.find("current inputSize and last outputSize not matched") !=
        string::npos);
}
...
TEST_F(ModelProcessTest, TestUpdateInputsReuse_UpdateDataBufferFailed) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetDatasetBuffer(
                              reinterpret_cast<aclmdlDataset*>(0x1111), 2))
        .WillRepeatedly(Return(fakeInputBuffer2));
    EXPECT_CALL(*mockAcl, aclmdlGetDatasetBuffer(
                              reinterpret_cast<aclmdlDataset*>(0x2222), 1))
    ...
    // 执行测试 -
    // 动态索引在1，所以关系向量索引0对应输入0，索引1跳过，索引2对应输入2
    testing::internal::CaptureStdout();
    ...
    // 错误日志
    EXPECT_TRUE(
        logOutput.find("new input buffer update from last output failed") !=
        string::npos);
}
...
TEST_F(ModelProcessTest, TestUpdateInputsReuse_SuccessWithDynamicIndex) {
    ...
    // 设置更新操作成功
    EXPECT_CALL(*mockAcl,
                aclUpdateDataBuffer(fakeInputBuffer0, fakeAddrOut0, bufferSize))
        .WillRepeatedly(Return(ACL_SUCCESS));
    EXPECT_CALL(*mockAcl,
                aclUpdateDataBuffer(fakeInputBuffer2, fakeAddrOut1, bufferSize))
        .WillRepeatedly(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestCreateInput_SuccessWithExistingDataset) {
    ...
    EXPECT_CALL(*mockAcl,
                aclmdlAddDatasetBuffer(modelProcess->input_, fakeDataBuffer))
        .WillRepeatedly(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestUpdateInputsMemcpy_SuccessWithDynamicIndex) {
    ...
    // 设置内存复制成功
    EXPECT_CALL(*mockAcl, aclrtMemcpy(fakeAddrIn0, bufferSize, fakeAddrOut0,
                                      bufferSize, ACL_MEMCPY_DEVICE_TO_DEVICE))
        .WillRepeatedly(Return(ACL_SUCCESS));
    EXPECT_CALL(*mockAcl, aclrtMemcpy(fakeAddrIn2, bufferSize, fakeAddrOut1,
                                      bufferSize, ACL_MEMCPY_DEVICE_TO_DEVICE))
        .WillRepeatedly(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestUpdateInputsMemcpy_SuccessWithoutDynamicIndex) {
    ...
    EXPECT_CALL(*mockAcl, aclrtMemcpy(fakeAddrIn0, bufferSize, fakeAddrOut0,
                                      bufferSize, ACL_MEMCPY_DEVICE_TO_DEVICE))
        .WillRepeatedly(Return(ACL_SUCCESS));
    EXPECT_CALL(*mockAcl, aclrtMemcpy(fakeAddrIn1, bufferSize, fakeAddrOut1,
                                      bufferSize, ACL_MEMCPY_DEVICE_TO_DEVICE))
        .WillRepeatedly(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestUpdateInputsMemcpy_MemcpyFailed) {
    ...
    // 模拟内存复制失败
    EXPECT_CALL(*mockAcl, aclrtMemcpy(fakeAddrIn, bufferSize, fakeAddrOut,
                                      bufferSize, ACL_MEMCPY_DEVICE_TO_DEVICE))
        .WillRepeatedly(Return(ACL_ERROR_BAD_ALLOC));
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("acl memcpy from last output failed") !=
                string::npos);
    EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
}
...
TEST_F(ModelProcessTest, TestUpdateInputsMemcpy_OutputIndexOutOfRange) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("find outputdata index out of range") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCreateZeroInput_Success) {
    ...
    for (size_t i = 0; i < numInputs; i++) {
        ...
        EXPECT_CALL(*mockAcl,
                    aclrtMalloc(_, inputSizes[i], ACL_MEM_MALLOC_HUGE_FIRST))
            .WillOnce(DoAll(SetArgPointee<0>(fakeBuffer), Return(ACL_SUCCESS)));

        // 非动态张量，需要memset
        EXPECT_CALL(*mockAcl,
                    aclrtMemset(fakeBuffer, inputSizes[i], 0, inputSizes[i]))
            .WillOnce(Return(ACL_SUCCESS));

        aclDataBuffer* fakeDataBuffer =
            reinterpret_cast<aclDataBuffer*>(0x2000 + i * 0x1000);
    ...
    // 验证结果
    EXPECT_EQ(ret, SUCCESS);
    EXPECT_TRUE(logOutput.find("add input_ at CreateZeroInput +1") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestDestroyInput_FreeMemory) {
    ...
    vector<void*> memoryBuffers = {reinterpret_cast<void*>(0x4000),
                                   reinterpret_cast<void*>(0x5000),
                                   reinterpret_cast<void*>(0x6000)};

    for (size_t i = 0; i < bufferCount; i++) {
        EXPECT_CALL(*mockAcl, aclmdlGetDatasetBuffer(
                                  reinterpret_cast<aclmdlDataset*>(0x1111), i))
    ...
    // 期望销毁数据集
    EXPECT_CALL(*mockAcl,
                aclmdlDestroyDataset(reinterpret_cast<aclmdlDataset*>(0x1111)))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestDestroyInput_GetBufferFailed) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetDatasetBuffer(
                              reinterpret_cast<aclmdlDataset*>(0x1111), 1))
    ...
    EXPECT_CALL(*mockAcl,
                aclGetDataBufferAddr(reinterpret_cast<aclDataBuffer*>(0x2000)))
    ...
    EXPECT_CALL(*mockAcl,
                aclDestroyDataBuffer(reinterpret_cast<aclDataBuffer*>(0x2000)))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestCreateOutput_SuccessWithModelOutputSize) {
    ...
    for (size_t i = 0; i < outputNum; ++i) {
        void* fakeBuffer = reinterpret_cast<void*>(0x1000 + i * 0x1000);
        EXPECT_CALL(*mockAcl,
                    aclrtMalloc(_, outputSizes[i], ACL_MEM_MALLOC_HUGE_FIRST))
            .WillOnce(DoAll(SetArgPointee<0>(fakeBuffer), Return(ACL_SUCCESS)));
...
TEST_F(ModelProcessTest, TestCreateOutput_SuccessWithCustomOutputSize) {
    ...
    for (size_t i = 0; i < outputNum; ++i) {
        ...
        EXPECT_CALL(*mockAcl, aclrtMalloc(_, _, ACL_MEM_MALLOC_HUGE_FIRST))
            .WillRepeatedly(
                DoAll(SetArgPointee<0>(fakeBuffer), Return(ACL_SUCCESS)));
...
TEST_F(ModelProcessTest, TestCreateOutput_AddDatasetBufferFailed) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("can't add data buffer, create output failed") !=
                string::npos);
    EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
}
...
TEST_F(ModelProcessTest, TestExecute_Success) {
    ...
    EXPECT_CALL(
        *mockAcl,
        aclmdlExecute(expectedModelId, reinterpret_cast<aclmdlDataset*>(0x1111),
                      reinterpret_cast<aclmdlDataset*>(0x2222)))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestExecute_Failed) {
    ...
    EXPECT_TRUE(
        logOutput.find("modelId is " + std::to_string(expectedModelId)) !=
        string::npos);
    EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
}
...
TEST_F(ModelProcessTest, TestUnload_SuccessWithModelDesc) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlUnload(expectedModelId))
        .WillOnce(Return(ACL_SUCCESS));
    EXPECT_CALL(*mockAcl,
                aclmdlDestroyDesc(reinterpret_cast<aclmdlDesc*>(0x1234)))
        .WillOnce(Return(ACL_SUCCESS));
    ...
    EXPECT_TRUE(
        logOutput.find("model Id is " + std::to_string(expectedModelId)) !=
        string::npos);
}
...
TEST_F(ModelProcessTest, TestUnload_UnloadFailedButModelDescDestroyed) {
    ...
    EXPECT_TRUE(
        ...
        string::npos);
    EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
    EXPECT_TRUE(logOutput.find("unload model success") !=
                string::npos);  // 代码中会打印成功
}
...
TEST_F(ModelProcessTest, TestGetCurOutputShape_DynamicShapeSuccess) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetDatasetTensorDesc(
                              reinterpret_cast<aclmdlDataset*>(0x2222), index))
    ...
    for (size_t i = 0; i < dimNums; ++i) {
        EXPECT_CALL(*mockAcl, aclGetTensorDescDimV2(fakeTensorDesc, i, _))
            .WillOnce(
                DoAll(SetArgPointee<2>(dimValues[i]), Return(ACL_SUCCESS)));
    }
...
TEST_F(ModelProcessTest, TestGetCurOutputShape_StaticShapeFailed) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("aclmdlGetCurOutputDims failed") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestGetInTensorDesc_Success) {
    ...
    // 执行测试
    Result ret = modelProcess->GetInTensorDesc(index, name, outDataType,
                                               outFormat, shape, outSize);
...
TEST_F(ModelProcessTest, TestGetOutTensorDesc_Success) {
    ...
    // 执行测试
    Result ret = modelProcess->GetOutTensorDesc(index, name, outDataType,
                                                outFormat, shape, outSize);
...
TEST_F(ModelProcessTest, TestGetOutTensorLen_DynamicShapeSuccess) {
    ...
    aclTensorDesc* fakeTensorDesc = reinterpret_cast<aclTensorDesc*>(0x3000);
    EXPECT_CALL(*mockAcl,
                aclmdlGetDatasetTensorDesc(modelProcess->output_, index))
        .WillOnce(Return(fakeTensorDesc));
...
TEST_F(ModelProcessTest, TestCreateOutput_WithParams_CreateDatasetFailed) {
    ...
    // 验证结果
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(logOutput.find("can't create dataset, create output failed") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestCreateOutput_WithParams_CreateBufferFailed) {
    ...
    EXPECT_EQ(ret, FAILED);
    EXPECT_TRUE(
        logOutput.find("can't create data buffer, create output failed") !=
        string::npos);
    EXPECT_TRUE(logOutput.find(errorMsg) != string::npos);
}
...
TEST_F(ModelProcessTest, TestCheckDymAIPPInputExist_Success) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlGetAippType(expectedModelId, 0, _, _))
        .WillOnce(DoAll(SetArgPointee<2>(ACL_DATA_WITH_DYNAMIC_AIPP),
                        SetArgPointee<3>(0), Return(ACL_SUCCESS)));
...
TEST_F(ModelProcessTest, TestSetInputAIPP_Success) {
    ...
    EXPECT_CALL(
        *mockAcl,
        aclmdlSetInputAIPP(expectedModelId, modelProcess->input_, 0, dummyAipp))
...
TEST_F(ModelProcessTest, TestSetAIPPSrcImageSize_FailedWithException) {
    ...
    testing::internal::CaptureStdout();
    EXPECT_THROW(
        { modelProcess->SetAIPPSrcImageSize(config, dummyAipp); }, const char*);
    string logOutput = testing::internal::GetCapturedStdout();

    EXPECT_TRUE(logOutput.find("acl set AIPP SrcImage size failed") !=
                string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPInputFormat_FailedWithException) {
    ...
    testing::internal::CaptureStdout();
    EXPECT_THROW(
        { modelProcess->SetAIPPInputFormat(config, dummyAipp); }, const char*);
    string logOutput = testing::internal::GetCapturedStdout();

    EXPECT_TRUE(logOutput.find("acl set AIPP input format failed") !=
                string::npos);
}

// ===================== SetAIPPCscParams测试 =====================
// GMock 的 MOCK_METHOD 宏支持的参数数量是有限制的。MOCK_METHOD
// 宏最多支持10个参数的函数模拟。 而需要mock的 aclmdlSetAIPPCscParams
// 有17个参数，故略过
...
TEST_F(ModelProcessTest, TestSetAIPPRbuvSwapSwitch_Success) {
    ...
    // 验证
    EXPECT_EQ(ret, SUCCESS);
    EXPECT_TRUE(logOutput.find("aclmdlSetAIPPRbuvSwapSwitch") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPRbuvSwitch_Failure) {
    ...
    EXPECT_TRUE(logOutput.find("acl set AIPP swap switch params failed") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPAxSwapSwitch_Success) {
    ...
    EXPECT_EQ(ret, SUCCESS);
    EXPECT_TRUE(logOutput.find("aclmdlSetAIPPAxSwapSwitch") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPAxSwapSwitch_Failure) {
    ...
    EXPECT_TRUE(logOutput.find("acl set AIPP Ax swap switch params failed") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPDtcPixelMean_CustomValue_Success) {
    ...
    EXPECT_CALL(*mockAcl,
                aclmdlSetAIPPDtcPixelMean(fakeAipp, 10, 20, 30, 40, 1))
        .WillOnce(Return(ACL_SUCCESS));
    ...
    EXPECT_EQ(ret, SUCCESS);
    EXPECT_TRUE(logOutput.find("dtcPixelMeanChn0: 10 dtcPixelMeanChn1: 20") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPDtcPixelMean_DefaultValue_Success) {
    ...
    EXPECT_EQ(ret, SUCCESS);
    EXPECT_TRUE(logOutput.find("dtcPixelMeanChn0: 0 dtcPixelMeanChn1: 0") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPDtcPixelMean_Failure) {
    ...
    EXPECT_TRUE(logOutput.find("acl set AIPP Dtc pixel mean params failed") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPDtcPixelMin_CustomValue_Success) {
    ...
    // 设置期望行为（batchIndex=1 应使用第二个值）
    EXPECT_CALL(*mockAcl,
                aclmdlSetAIPPDtcPixelMin(fakeAipp, 10.0, 20.0, 30.0, 40.0, 1))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestSetAIPPDtcPixelMin_Failure) {
    ...
    EXPECT_TRUE(logOutput.find("acl set AIPP dtc pixel min params failed") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPPixelVarReci_CustomValue_Success) {
    ...
    // 设置期望行为（batchIndex=1 应使用第二个值）
    EXPECT_CALL(*mockAcl,
                aclmdlSetAIPPPixelVarReci(fakeAipp, 10.0, 20.0, 30.0, 40.0, 1))
        .WillOnce(Return(ACL_SUCCESS));
    ...
    EXPECT_EQ(ret, SUCCESS);
    EXPECT_TRUE(logOutput.find("dtcPixelVarReciChn0: 10.0") !=
                std::string::npos);
...
TEST_F(ModelProcessTest, TestSetAIPPPixelVarReci_DefaultValue_Success) {
    ...
    EXPECT_EQ(ret, SUCCESS);
    EXPECT_TRUE(logOutput.find("dtcPixelVarReciChn0: 0.0") !=
                std::string::npos);
    EXPECT_TRUE(logOutput.find("dtcPixelVarReciChn1") != std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPPixelVarReci_Failure) {
    ...
    EXPECT_TRUE(logOutput.find("acl set AIPP pixel variance params failed") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPCropParams_CustomValue_Success) {
    ...
    // 设置期望行为（batchIndex=1 应使用第二个值）
    EXPECT_CALL(*mockAcl,
                aclmdlSetAIPPCropParams(fakeAipp, 10, 20, 30, 40, 50, 1))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestSetAIPPCropParams_DefaultValue_Success) {
    ...
    EXPECT_CALL(*mockAcl, aclmdlSetAIPPCropParams(fakeAipp, _, 0, 0,
                                                  Base::CROP_SIZE_W_DEFAULT,
                                                  Base::CROP_SIZE_H_DEFAULT, 0))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestSetAIPPCropParams_Failure) {
    ...
    EXPECT_TRUE(logOutput.find("acl set AIPP crop params failed") !=
                std::string::npos);
}
...
TEST_F(ModelProcessTest, TestSetAIPPPaddingParams_CustomValue_Success) {
    ...
    // 设置期望行为（batchIndex=1 应使用第二个值）
    EXPECT_CALL(*mockAcl,
                aclmdlSetAIPPPaddingParams(fakeAipp, 10, 20, 30, 40, 50, 1))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestSetAIPPPaddingParams_DefaultValue_Success) {
    ...
    // 设置期望行为（使用默认值0,0,0,0,0）
    EXPECT_CALL(*mockAcl,
                aclmdlSetAIPPPaddingParams(fakeAipp, 0, 0, 0, 0, 0, 0))
        .WillOnce(Return(ACL_SUCCESS));
...
TEST_F(ModelProcessTest, TestSetAIPPPaddingParams_Failure) {
    ...
    EXPECT_TRUE(logOutput.find("acl set AIPP padding params failed") !=
                std::string::npos);
}
}  // namespace AISBench_test
