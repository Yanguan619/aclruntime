...
std::string Utils::printCurrentTime() {
  ...
  std::string pi = std::to_string(p->tm_year + 1900) +
                   std::to_string(p->tm_mon + 1) + std::to_string(p->tm_mday) +
                   "_" + std::to_string(p->tm_hour) + "_" +
                   std::to_string(p->tm_min) + "_" + std::to_string(p->tm_sec) +
                   "_" + std::to_string(tv.tv_usec);
  return pi;
}
...
bool Utils::IsValidInteger(const std::string& str) {
  ...
  // Check for empty string, characters left after conversion, and out of range
  // values
...
bool Utils::IsLegalDymString(const std::string& str) {
  ...
  if (inputInfoList.size() > INPUT_LIST_MAX_SIZE) {
    ERROR_LOG("the count of input list parsed from dymshape string is over %zu",
              INPUT_LIST_MAX_SIZE);
    return false;
  }
  ...
  for (std::string infoStr : inputInfoList) {
    ...
    if (inputInfo.size() != EXPECTED_INPUT_INFO_SIZE) {
      ERROR_LOG(
          "the format of input info parsed from dymshape string is wrong!");
      return false;
    }
    ...
    if (inputName.length() == 0 || inputName.length() > INPUT_NAME_LENGTH_MAX) {
      ERROR_LOG(
          "the length of input name parsed from dymshape string is output of "
          "[1, %zu]",
          INPUT_LIST_MAX_SIZE);
      return false;
    }
    ...
    if (!IsDymShapeValid(inputValue)) {
      ERROR_LOG(
          "the format of shape string parsed from dymshape string is illegal!");
      return false;
    }
  }
  return true;
