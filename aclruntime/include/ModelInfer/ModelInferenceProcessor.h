...
#ifndef MODEL_INFERENCE_PROCESSOR_H
#define MODEL_INFERENCE_PROCESSOR_H
...
struct InferSummaryInfo {
    ...
    std::chrono::steady_clock::time_point zero_point = std::chrono::steady_clock::now();
};
...
private:
    ...
    APP_ERROR AddOutTensors(std::vector<MemoryData>& outputs, const std::vector<std::string>& outputNames,
        std::vector<TensorBase>& outputTensors);
    ...
    APP_ERROR Execute();
    APP_ERROR GetOutputs(const std::vector<std::string>& outputNames, std::vector<TensorBase> &outputTensors);
