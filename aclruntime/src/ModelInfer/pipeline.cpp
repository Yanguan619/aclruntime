...
namespace Base {
    ...
    void FuncCompute(ConcurrentQueue<std::shared_ptr<Feeds>> &computeQueue,
                     ...
                     Base::PyInferenceSession* session, InferSummaryInfo* summaryInfo)
{
        APP_ERROR ret = Base::TensorContext::GetInstance()->SetContext(session->GetDeviceId(),
                                                                       session->GetContextIndex());
        if (ret != APP_ERR_OK) {
    ...
    void SaveOutput(std::shared_ptr<Feeds> item, std::string outFmt, size_t index)
    {
        ...
        if (outFmt == "NPY") {
            outputFileName += ".npy";
            if (Utils::TensorToNumpy(outputFileName, item->outputs->at(index)) == UtilsResult::FAILED) {
                ERROR_LOG("%s save failed\n", outputFileName.c_str());
            }
        } else if (outFmt == "TXT") {
            outputFileName += ".txt";
            if (Utils::TensorToTxt(outputFileName, item->outputs->at(index)) == UtilsResult::FAILED) {
                ERROR_LOG("%s save failed\n", outputFileName.c_str());
            }
        } else {
            outputFileName += ".bin";
            if (Utils::TensorToBin(outputFileName, item->outputs->at(index)) == UtilsResult::FAILED) {
                ERROR_LOG("%s save failed\n", outputFileName.c_str());
            }
        }
    }
