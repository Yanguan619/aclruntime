...
class InferSession:
    ...
    def infer_pipeline(self, feeds_list, mode="static", custom_sizes=100000):
        ...
        for feeds in feeds_list:
            ...
            for feed in feeds:
                if isinstance(feed, np.ndarray):
                    ...
                    infer_input = (
                        np.ascontiguousarray(infer_input)
                ...
                basetensor = aclruntime.BaseTensor(
                    infer_input.__array_interface__["data"][0], infer_input.nbytes
                )
        ...
        if (
            self.infer_mode_switch.get(mode) is not None
            and mode != "dymshape"
            and mode != "dymdims"
        ):
            ...
            if isinstance(custom_sizes, int):
                ...
                raise RuntimeError(
                    "custom_sizes:{} type:{} invalid".format(custom_sizes, type(custom_sizes))
                )
        ...
        outputs = self.session.run_pipeline(
            self.outputs_names, inputs_list, shapes_list, mode == "dymshape", mode == "dymdims"
        )
    ...
    def infer_iteration(
        self, feeds, in_out_list=None, iteration_times=1, mode="static", custom_sizes=100000
    ):
        """
        Parameters:
            feeds: input datas
...
class MultiDeviceSession:
    def __init__(
        self, model_path: str, acl_json_path: str = None, debug: bool = False, loop: int = 1
    ):
        check_model_path_legality(model_path)
        check_acl_json_path_legality(acl_json_path)
        check_loop_size(loop)
    ...
    def _run_multidevice(self, device_feeds, subprocess_func, log_label):
        ...
        if subprocess_num > MAX_TOTAL_PROCESS_COUNT:
            raise RuntimeError(
                f"subprocess count over max permitted count: {MAX_TOTAL_PROCESS_COUNT}"
            )
        ...
        for device_id, feeds in device_feeds.items():
            ...
            for feed in feeds:
                p.apply_async(
                    subprocess_func,
                    args=(outputs_queue, device_id, feed),
                    error_callback=self.print_subprocess_run_error,
                )
        p.close()
        p.join()
        result = 0 if 2 * len(device_feeds) == outputs_queue.qsize() else 1
        logger.info(f"multidevice {log_label} end qsize:{outputs_queue.qsize()} result:{result}")
        outputs_dict = {}
        self.summary.clear()
    ...
    def infer(self, device_feeds: dict, mode="static", custom_sizes=100000):
        ...
        check_custom_size(custom_sizes, mode)
        return self._run_multidevice(
            device_feeds, lambda q, d, f: self.subprocess_infer(q, d, f, mode, custom_sizes), "run"
        )
    ...
    def infer_pipeline(self, device_feeds_list: dict, mode="static", custom_sizes=100000):
        ...
        return self._run_multidevice(
            device_feeds_list,
            lambda q, d, f: self.subprocess_infer_pipeline(q, d, f, mode, custom_sizes),
            "run pipeline",
        )
    ...
    def subprocess_infer_pipeline(
        self, outputs_queue, device_id, feeds_list, mode="static", custom_sizes=100000
    ):
        sub_session = InferSession(
            device_id=device_id,
            model_path=self.model_path,
    ...
    def subprocess_infer_iteration(
        self, outputs_queue, device_id, feeds, infer_iteration_content: InferIterationContent
    ):
        sub_session = InferSession(
            device_id=device_id,
            model_path=self.model_path,
        ...
        if infer_iteration_content.in_out_list is not None:
            check_in_out_list(
                ...
                sub_session.get_inputs(),
                sub_session.get_outputs(),
            )
        start_time = time.time()
        outputs = sub_session.infer_iteration(
            feeds,
            infer_iteration_content.in_out_list,
