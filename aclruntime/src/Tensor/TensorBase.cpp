...
APP_ERROR TensorBase::CheckBatchTensors(const std::vector<TensorBase> &inputs,
  ...
  auto checkFunc = [checkFirstDim](const TensorBase &t1, const TensorBase &t2) {
    if (t1.GetShape().size() != t2.GetShape().size()) {
      ERROR_LOG("dimension is not match (%lu) vs (%lu)", t1.GetShape().size(),
                t2.GetShape().size());
      return false;
    }
    ...
    if (t1.GetTensorType() != t2.GetTensorType()) {
      ERROR_LOG("memory type is not match (%d) vs (%d)", t1.GetTensorType(),
                t2.GetTensorType());
      return false;
    }
...
MemoryData CopyMemory2DeviceMemory(void *ptr, uint64_t size, int32_t deviceId) {
  ...
  if (ret != APP_ERR_OK) {
    throw std::runtime_error(
        "copy mem to device failed! cause: malloc device memory data failed!");
  }
