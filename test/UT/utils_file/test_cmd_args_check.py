...
class TestClass:
    def test_args_check(self):
        ...
        assert not str2bool(args.auto_set_dymdims_mode)
        assert check_batchsize_valid(str(args.batchsize)) == args.batchsize
        assert (
            check_nonnegative_integer(str(args.output_batchsize_axis))
